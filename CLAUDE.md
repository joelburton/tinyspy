...
| [plans/stale-build.md](plans/stale-build.md) | **Not scheduled** — a tab notices its build has been replaced and asks the person to refresh. Written after the 2026-09-18 incident where a tab open across a deploy started two real crossword games while being told both had failed. The stamp is emitted by the vite build so no deploy step can skip it; `reloadOnStaleChunk` already covers the other half (a chunk that is gone) |
