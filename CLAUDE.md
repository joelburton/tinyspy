...
| [plans/error-system.md](plans/error-system.md) | **THE ACTIVE SPRINT (2026-08-26)** — the results/rejections/faults redesign. The SHAPE now lives in [docs/envelopes.md](docs/envelopes.md), which outranks this file; what the plan owns is §4 (still open), §6 (process) and §7 (the roster, 148 of 157 as of 2026-09-01 — **9 open rows = 5 conversions**; EVERY RPC is converted, leaving one read plus the cross-cutting four, which empty `ERROR_COPY` and let the old system be deleted. Twelve rows were ADDED that day by auditing the SQL — believe the SQL, not the roster). `ERROR_COPY` shrinks to environmental failures only. The CSS sprint is paused behind it. §5 is a worked psychicnum example |
