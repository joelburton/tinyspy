...
| [plans/fault-presentation.md](plans/fault-presentation.md) | **DONE 2026-09-01** — fault PRESENTATION moved from `dbFetch` (which knows only the URL) to the three wrappers (which hold the answer), plus a `presentFaults` opt-out for sites with their own logic (`useGameTimer`, `useDefinition`). §2 is the measured research: what each layer can actually see. Kept only for that; its checklist is complete |
