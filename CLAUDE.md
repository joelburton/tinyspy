...
| [docs/keyboard-shortcuts.md](docs/keyboard-shortcuts.md) | Every key the app listens for: how a keystroke is routed (the window dispatcher + its focused-field / floating-panel / modifier gates), the global shell shortcuts (`` ` ``, `/`, `?`, `~`, `⇧<`, `+`, `⌥+`, `⌥⌫`), menu/dialog/chat keys, the club + home lists, then the **per-game board keys** for all fourteen |
