...
| [plans/error-system.md](plans/error-system.md) | **THE ACTIVE SPRINT (2026-08-26)** — the results/rejections/faults redesign. The SHAPE now lives in [docs/envelopes.md](docs/envelopes.md), which outranks this file; what the plan owns is §4 (still open), §6 (process) and §7 (the roster, 121 of 157 as of 2026-09-01 — **36 open rows = 32 conversions**; eleven of sixteen games done, with crosswords, scrabble, letterboxed, strands and psychicnum left, then the cross-cutting four. Twelve rows were ADDED that day by auditing the SQL — believe the SQL, not the roster). `ERROR_COPY` shrinks to environmental failures only. The CSS sprint is paused behind it. §5 is a worked psychicnum example |
...
| [plans/deno-callers.md](plans/deno-callers.md) | **BUILT; 4 of its 10 call sites are on it** — the error sprint's FOURTH quadrant: the browser has three wrappers for talking to the server, and an edge function calling an RPC had none. One Deno `runRpc`; the row-returning board-builder calls are deliberately left alone (§3). §6 lists each remaining call site with the RPC it waits on — **read it before converting any RPC an edge function calls**, because the relay above one swallows the SUCCESS otherwise |
