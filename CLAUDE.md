...
| [plans/actions.md](plans/actions.md) | **The design for every key and command in the app — BUILT (all sixteen games converted 2026-09-10).** One registry holds an action's fixed half (name `act-xxx`, glyph, chords, tone, confirmation); a page or component binds the live half (what it does, a state callback → active · hidden · disabled + live label); menu rows, buttons, tooltips, one app-root key dispatcher and a generated help list all read the same bound action. ALL keys go through it, entry included; components register their own. Being retired: what it decided lives in `src/common/actions/doc.md` (what a command IS) and `docs/keyboard-shortcuts.md` (what each key DOES) |
