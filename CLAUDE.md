...
| [docs/button-taxonomy.md](docs/button-taxonomy.md) | **A PLAN, to be built and deleted**, and the NEXT thing after the chrome-tone work: inverting the `<button>` default. The bare element is currently the FILLED accent button, which is why **29 rules exist purely to undo it** and why a blanket hover has never been safe (the standing comment in theme.css promises a `.button` class). Carries the **fourteen kinds** (piece · key · action · form · trigger · choice · toggle · tab · row · handle · dismiss · textlink · surface · float) sorted into four families by *what feedback they should give* — accidental / game pieces / keyboard / general — plus the two rules that fall out: **pieces use depth for hover and darkening for press** (the darken can't move to hover: reduced-motion and touch would lose their only feedback), and **general buttons use colour only, no motion, no shadow** (the load-bearing half of *depth belongs to game pieces*). Plus the measured caveat that `border-radius` must stay in the neutral base (19 rules silently rely on it), and the one-commit-over-102-buttons migration |
| [docs/colors-refinement.md](docs/colors-refinement.md) | **A PLAN, to be built and deleted**, and the NEXT thing after connections (before the remaining twelve tile-feedback conversions — the reasoning is in the doc): the color system put on a grid. Bucket-prefixed names (`outcome-*` how a move/game went · `gamelist-*` a game as an object in a list · `chrome-*` controls · `view-*` what you are looking at · per-game **brand** colors), five ROLE-named variants on every outcome family (`-ink` text+thin lines · `-fill` pieces + log bars · `-edge` the border on a filled piece · `-terminal-frame` · `-wash`), and the rules: **every color has a name** (the magic-number rule — no literal or `color-mix()` at a use site, machine-guarded), alias-on-dependency vs copy-on-coincidence, hand-picked where perception beats math, no cross-bucket borrowing, no geometry. Carries the 2026-08-17 audit (every hex, every consumer, the dead tokens, the two stray reds, the 95 unnamed color declarations) and a two-commit work list — renames first, values second |
