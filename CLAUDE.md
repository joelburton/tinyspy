...
| [plans/found-words.md](plans/found-words.md) | **All four code steps SHIPPED 2026-09-21; open only for Joel's read.** The found-words family's folders redrawn by what is shared rather than by history: `shared/found-words` is the three games that accumulate a list of found words (plus wordiply, for the submit engine alone) and `shared/bee-games` is what only the hive and the wheel share. Boggle joined the family — the merge, the rows call, the typed-word rule and the play-surface scaffolding — rather than waiting for its own area, and its duplicate of each is gone. The leaderboard read is `common/game-page/readLeaderboard`, generic with no default, at twelve call sites across six games. All seven open calls are answered and recorded in the plan. **Nothing was stamped** (call 6). The durable parts are already in [docs/common-folders.md](docs/common-folders.md) and the two folders' `doc.md`; the file is kept until Joel has read it, then deleted |
| [plans/spectating.md](plans/spectating.md) | **PROPOSED, nothing built, nothing decided** — opened by Joel 2026-09-22 at wordle's F-12. A club member can already open a game they are not seated in; four games show a "Watching" notice three different ways and twelve show nothing. The file inventories what each layer does for a watcher today (SQL gates, the pause rule, the shell, the club page, chat) and lists the seven questions a design has to settle, all Joel's. Cross-cutting, so nobody's area: read it before deciding a spectator question from inside one game |
| [plans/turn-bell.md](plans/turn-bell.md) | **PROPOSED, nothing built; its questions answered** — asked for by Joel 2026-09-23. A bell when the turn becomes yours, on the same edge as the yellow turn frame, for every game where the turn passes. An **"Enable sounds"** profile setting (`sounds_enabled`, default ON, existing accounts included via `add column … not null default true`) governs EVERY sound, the win jingle too, checked inside `common/sounds/playSound`. The turn edge is factored out of `useTurnStartFlash`, and the bell is one call in `GamePage` — nine games move the shared `current_turn_user_id`, so they ring with no game code; scrabble compete (its own seat) and codenamesduet (on being given a clue) ring from their own. psychicnum is where it is proven |
| [plans/waffle-stored-colors.md](plans/waffle-stored-colors.md) | **PROPOSED, nothing built** — raised by Joel 2026-09-22. waffle's swap log records two positions and no colors, so the turn-history viewer recolors each past board on the FE, which is why `_solution_for` hands the coop client the answer DURING PLAY. Storing a 25-char `colors` on the event row instead takes the secret off the client mid-game, retires the TypeScript copy of `common.wordle_colors`, and makes waffle agree with wordle, which has always stored its colors per guess. The board is deliberately NOT stored — replaying it needs no secret. **The plan turns on the backfill for existing games**: without it the FE keeps the port for old games and none of that arrives. Three open questions at the end are Joel's |
| [plans/codenamesduet-events.md](plans/codenamesduet-events.md) | **BUILT through Step 5 on branch `codenamesduet-events`, not deployed** (2026-09-23); Step 6, the rehearsal and the deploy, is left. codenamesduet's `clues` + `guesses` become one `codenamesduet.events` table in the standard shape — kinds `clue` · `guess` · `pass` · `hint`, payload columns named for the kind that owns them, a CHECK tying each kind to its columns. A pass and an AI hint are recorded for the first time; the turn state stays on the game row; the log groups events by turn so it looks the same. The game gains a `lib/answer.ts`, and the header's peer phrases move into it. A forward migration backfills prod, inferring past passes. The `codenamesduet` area pauses at Step 5 for it |
...
- **Spectators are friends too.** A club member can open a game they are not seated in and watch it — what they see and may do is not yet designed (see [plans/spectating.md](plans/spectating.md)). Presence-pause counts the game's ROSTER: it fires the moment a player isn't connected, because someone-missing means the call has stalled, and a watcher's coming and going changes nothing.
...
- **No "find an open game" listings, no public lobby, no random pairings, no leaderboards-among-strangers.** Watching is for club members only.
