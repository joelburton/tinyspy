...
| [plans/tab-rings.md](plans/tab-rings.md) | **Our version of tabbing — RUNNING NOW as one sprint, nested inside app-audit (Joel, 2026-09-10).** Tab moves within a ring of stops a surface declared and never reaches the URL bar; crosswords is the only genuine exception. The mechanism is built (`useTabRing`); Part II is the build order — five steps, a stop after each, one decision (D1, a panel's ring declared or discovered) to take before step 3. Supersedes "convert per area": the `lists`, `forms` and `floating-panels` areas audit their subjects rather than also converting them |
