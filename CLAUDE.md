...
| [plans/envelope-layering.md](plans/envelope-layering.md) | **The layering fix, mostly shipped** — one author for "nothing answered", `dbResult` split so the imports point down, `ReadFailure` deleted. §6 is what it deliberately does NOT fix; §9 records the design rejected and why. Delete it when §8's last step lands |
