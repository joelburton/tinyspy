...
| [plans/found-words.md](plans/found-words.md) | **Drafted 2026-09-21, not started; no step runs until Joel says so.** The found-words family's folders redrawn by what is shared rather than by history: `shared/word-hunt` becomes `shared/found-words` and takes the family's types and typed-word look from `bee-games`; the leaderboard reader goes common; boggle adopts the shared merge, rows call, stylesheet and reader now rather than at its area; `word-list` stays the panel and only the panel. Six open calls at the top are Joel's. Deleted when it ships |
