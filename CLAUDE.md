...
| [docs/realtime-lost-events.md](docs/realtime-lost-events.md) | The **lost-event** failure mode: `SUBSCRIBED` is only the join ack — every fresh channel has a measured ~2–3s **deaf window** until the `system` "Subscribed to PostgreSQL" message, and a `postgres_changes` event committed inside it is LOST, not late (so a longer timeout is the wrong fix). The deterministic repro, the delivery-window probe, the diagnosis recipe, and the always-on `[rt]` console instrumentation (`realtimeDiag.ts`) that leaves the evidence trail in real browsers |
...
| [docs/keyboard-shortcuts.md](docs/keyboard-shortcuts.md) | Every key the app listens for: how a keystroke is routed (the window dispatcher + its focused-field / floating-panel / modifier gates), the global shell shortcuts (`` ` ``, `/`, `?`, `~`, `⇧<`, `+`, `⌥+`, `⌥⌫`), menu/dialog/chat keys, the club + home lists, then the **per-game board keys** for all fifteen |
...
The original target was ~7–8 games; fifteen are live today (codenamesduet, connections, psychicnum, spellingbee, bananagrams, waffle, wordle, stackdown, scrabble, boggle, crosswords, wordwheel, wordiply, strands, letterboxed); psychicnum is a deliberately minimal toy whose job is to exercise the multi-game architecture with the smallest possible game-logic surface.
