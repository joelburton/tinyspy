...
| [plans/envelope-rollout.md](plans/envelope-rollout.md) | **Short-lived** — the ordering for the prep the roster needs first: reconcile the docs, shrink the over-explaining comments, the shared machinery, retro-fix what was converted before the call-site rules existed. **Step 5 is the live one** — the per-entry retro-fix list, which the roster in `error-system.md` waits on. Delete it when that list is empty |
