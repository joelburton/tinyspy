...
| [docs/games/monkeygram.md](docs/games/monkeygram.md) | MonkeyGram (Bananagrams-style) — **v1 complete; v2 in progress (peel built, dump next).** Per-player **player board** = a fixed 25×25 arena (a 625-char string) navigated with zoom + scroll + Center-and-fit; snapshot-on-unmount persistence; owner-only RLS; the keyboard-cursor rules. v2: the hand is **derived** (`board` FE-owned + `tiles` server-owned split, `hand = tiles − placed`, live `tiles` subscription) so **peel** can grow every player's holdings at once; the shared ⟲ **ShuffleButton**; the hidden `pool` (the bunch). Board feel settled in the throwaway `monkeygram-ui/` prototype. |
