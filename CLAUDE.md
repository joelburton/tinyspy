...
| [docs/realtime-lost-events.md](docs/realtime-lost-events.md) | The **lost-event** failure mode: a channel reports `SUBSCRIBED` and then never delivers `postgres_changes` (the tenant stops itself with no connected users; the first channel after it reboots is deaf). Why it is LOST, not slow — so a longer timeout is the wrong fix — plus the deterministic repro, the diagnosis recipe, and what is *not* established |
