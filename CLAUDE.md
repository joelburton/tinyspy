...
| [docs/colors-refinement.md](docs/colors-refinement.md) | **A PLAN, to be built and deleted** (sequenced AFTER connections' tile-feedback conversion): the **outcome palette** put on a grid — five variants (`-very-dark` terminal frame · `-dark` text + pill border · `-fill` pieces + turn-log bars · `-edge` the border on a filled piece · `-pale`) reserved on six families (won · lost · near · warning · neutral · **unplayed**), with the things that are NOT outcomes split out under their own names: the two **flag** colors (open / live, plus the crossword-picker exception), the three **chrome** colors (caution · destructive · fault — coincidentally equal to outcome hexes, deliberately not kept in lockstep), and the two **view-state** frames. Carries the 2026-08-17 audit it is built on (every hex, every consumer, the dead tokens, the two stray reds) and the eleven-item work list |
