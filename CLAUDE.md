...
| [plans/feedback-system.md](plans/feedback-system.md) | **What the feedback system IS today**, at a high level — the flow from a wrapper's `Envelope` to words on screen: four origins, six message shapes, two slots, four channels chosen by severity. Written 2026-09-04 as the first read for the `feedback` area, because the system spans six areas and no area's audit could see it whole. Describes, does not propose; §10 lists what is good and should survive a redesign |
| [plans/feedback-design.md](plans/feedback-design.md) | **The TARGET for the feedback system** — what feedback-system.md's subject becomes. Every statement is marked DECIDED / PROPOSED / OPEN; kept current as the design is agreed, and the `feedback` area builds to it. Started 2026-09-04 |
| [plans/actions.md](plans/actions.md) | **The target for every key and command in the app — DECIDED throughout, NOT YET BUILT.** One registry holds an action's fixed half (name `act-xxx`, glyph, chords, tone, confirmation); a page or component binds the live half (what it does, a state callback → active · hidden · disabled + live label); menu rows, buttons, tooltips, one app-root key dispatcher and a generated help list all read the same bound action. ALL keys go through it, entry included; components register their own. Runs NOW, nested inside app-audit (menus and PlayAreas wait behind it). Came out of the `keyboard` area 2026-09-09; building starts when Joel says so, §9 has the order |
