...
| [plans/error-system.md](plans/error-system.md) | **THE ACTIVE SPRINT (2026-08-26)** — the results/rejections/faults redesign, and the ONLY spec for it. RPCs return a JSONB envelope; helpers keep raising and each RPC catches at its own boundary; the player-facing sentence is written at the raise by the RPC author, who also states its kind. `ERROR_COPY` shrinks to environmental failures only. The CSS sprint is paused behind it. §5 is a worked psychicnum example |
