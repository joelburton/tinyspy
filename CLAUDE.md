...
| [plans/events.md](plans/events.md) | **THE NEXT WORK, and the first of three that ship together** — one `events` table per game, a `bigint identity` key, a mandatory `kind`, `created_at`, a stored `took_turn`, and the end of all five `seq` columns. Ten games; codenamesduet deliberately out. Start there: §6 is the per-game table, §9 is the backfill and how it gets tested, §13 is the one decision still open |
| [plans/scrabble-ai-players.md](plans/scrabble-ai-players.md) | **Second of the three** — scrabble's three bots become real users (`ada-bot`, `bjarne-bot`, `claude-bot`) with `profiles.ai_member`, seated in `common.game_players` but never in `clubs_members`. It is what lets `events.user_id` be `not null`, and it retires 81 lines of AI special-casing in scrabble's SQL |
| [plans/event-log.md](plans/event-log.md) | **Third of the three** — the frontend: the `event-log` → `event-log` vocabulary sweep, then turn history that works under any filter and in compete (the old `history-always-available` area, which is now its phase B). The number becomes the ordinal of the row in the log you are looking at; the link becomes the row's id |
