...
| [plans/found-words.md](plans/found-words.md) | **IN FLIGHT, started 2026-09-21; each step runs only at Joel's word.** The found-words family's folders redrawn by what is shared rather than by history. **Step 1 shipped**: the folder is `shared/found-words` (was `word-hunt`) and holds the family's row and word types, the typed-word look, and the submit engine, now `useFoundWordSubmit`. Still to come: the leaderboard reader goes common (step 2); boggle adopts the shared merge, rows call, typed-word rule and stylesheet now rather than at its area (steps 3–4); `word-list` stays the panel and only the panel. Four of seven open calls are answered; calls 4, 6 and 7 are still Joel's. Deleted when it ships |
