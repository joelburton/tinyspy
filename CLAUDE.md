...
| [plans/event-log.md](plans/event-log.md) | **THE LIVE ONE, third of three** — the frontend: the `turn-log` → `event-log` vocabulary sweep (A), turn history re-keyed so it works under any filter and in compete (B, the old `history-always-available` area), wordiply's viewer (C). All three are built; **§D's verification is what is left**, and it includes an e2e, so ask first. The number is the ordinal of the row in the log you are looking at; the link is the row's id |
