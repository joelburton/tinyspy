...
| [plans/events.md](plans/events.md) | **BUILT — the first of three that ship together** — one `events` table per game, a `bigint identity` key, a mandatory `kind`, `created_at`, a stored `took_turn`, and the end of all five `seq` columns. Ten games; codenamesduet deliberately out. **The shape itself is now [docs/supabase.md](docs/supabase.md#every-games-log-is-gameevents)**; what the plan still owns is the sweep — §6's per-game table, §9's backfill and §10's phase-by-phase record of how each game went |
| [plans/scrabble-ai-players.md](plans/scrabble-ai-players.md) | **BUILT — second of the three** — scrabble's three bots are real users now (`ada-bot`, `bjarne-bot`, `claude-bot`) with `profiles.ai_member`, seated in `common.game_players` but never in `clubs_members`. It is what lets `events.user_id` be `not null`, and it retired the AI special-casing in scrabble's SQL. §6 (retiring `scrabble.games.current_seat`) is explicitly optional and not done |
| [plans/event-log.md](plans/event-log.md) | **BUILT, third of three** — the frontend: the `turn-log` → `event-log` vocabulary sweep (A), turn history re-keyed so it works under any filter and in compete (B, the old `history-always-available` area), wordiply's viewer (C), and §D's verification including the realtime e2e. What the sprint still owes is the setgame bug `e2e/setgame-flash` caught, filed in `src/setgame/todo.md`. The number is the ordinal of the row in the log you are looking at; the link is the row's id, and the banner shows back the number that was clicked |
...
| [plans/react-context.md](plans/react-context.md) | **ONLY A CONVERSATION — NOT ACTIONABLE, NOT APPROVED.** A 2026-09-07 question about `useContext` for the game shell and the answer given, kept so the thread can resume. Nothing in it is a decision; do not build from it or fold it into an area |
| plans/playarea-readability.md | **DO NOT READ UNTIL JOEL SAYS TO. IDEAS ONLY — NOT CURRENT, NOT APPROVED.** Six recommendations from a 2026-09-07 conversation on making the per-game `PlayArea.tsx` files easier to understand (loader/loaded split, one section order, a centralized menu, `buildOver` to `lib/`, a comment pass). Listed so its presence in `plans/` isn't mistaken for an oversight |
