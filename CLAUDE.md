...
| [docs/scrabble-ai-strength.md](docs/scrabble-ai-strength.md) | The scrabble AI **player** (strength control) — the `choosePlay` policy brain + `playSelfGame` coop self-play (`src/scrabble/lib/policy.ts`), the five strength knobs (vocabCap / scoreFraction / useLeave + the fallibility knobs bingoMissProb / equityNoise), and the paired-seed measurement harness (`npm run scrabble:selfplay`) that tunes the 5-level ladder. Distinct from the always-best suggester; in-game opponent UI still deferred |
