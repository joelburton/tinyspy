...
/**
 * Smoke: boggle's "Print board (PDF)" menu item generates + downloads a real PDF
 * (brand MothCubes). jsPDF's runtime is unreachable by the mocked component tests, so
 ...
 */
test.describe('boggle — print board', () => {
  test('the Print menu item downloads a non-empty PDF', async ({ browser }) => {
    const club = await createSoloClub('bgp')
    const { id, gametype } = await createBoggleGame(club)
