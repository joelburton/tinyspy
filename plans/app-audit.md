...
| 33 | `manifest` | `manifest` · `gametypes.ts` | **OPEN 2026-09-14.** Thirteen findings in `plans/areas/manifest.md`, none worked. the registry and the manifest contract every game fills in. `gametypes.ts` is the registry's list, the one file allowed to import every game; it was `root-files`' until that row folded into this one (Joel, 2026-09-05), once `boot` had taken the other two root files |
