...
**`game-lib` is READ THROUGH, with FIVE findings open from its closing
...
contract several versions old. **59 IDs issued, 54 settled** — 57 statused in the area file, two
...
**Resequenced 2026-09-02** — five areas were added at the top as the walk
...
| `gameManifest.ts` · `src/gametypes.ts` · test | 3 | 1,059 | **nobody, until now** |
...
**Why it runs at 3 and not beside the other two.** `common/lib/gameManifest.ts` is
