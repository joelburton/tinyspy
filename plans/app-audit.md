...
will not write reloads uncounted, accepted as too rare (recorded in `boot/doc.md` →
...
| 6c | the z- layers | **DONE 2026-08-25.** `src/common/core-css/base.css` → THE Z- LAYERS and [docs/ui.md → Floating panels](../docs/ui.md#floating-panels--five-families-one-shell) |
...
subject, because a one-file area helps nobody. **A folder is named by two rows
when one of its files belongs to another area's subject**, and then both rows
say so, which keeps it answerable by looking: `themes`, because `loadTheme.ts`
is `boot`'s, and `shared/wordle-style`, because `pdfTiles.ts` is `pdf`'s.
...
| 35 | `board-marks` | `board-marks` (+ setgame's `lib/flash.ts` pair, by Joel's word) | **RE-OPENED 2026-09-20** (Joel) and the work is **DONE; the area awaits its re-bless.** `useFlash`, `useMark` and `useAnnouncedMark` became one `useMark`, reversing F-board-marks-14 and revisiting F-15 — both recorded in the area file, and the plan that carried it is deleted. Sixteen call sites across ten games in six commits (`8402f1f9`..`4877c3f8`); `word-list` (row 42) has resumed. **No file was re-stamped** — this retro-fixed what the area itself built, so the blessing below stands until Joel re-reads. **What changed the app:** a mark's three decisions — what it carries, whether it announces itself, when it ends — are arguments on one call instead of a choice between three imports with three return shapes; the timers moved into an effect keyed on the raise, so `show` and `clear` are legal during render and connections' hand-split raise-then-beats is gone along with its `setTimeout`, its second flash instance and its `verdictSeq`; letterboxed converted at last, on `NO_TIMER`, the clockless lifetime F-14 had to leave it behind for — a unique symbol, not a `null`, so the vocabulary's word is the only way to ask for it. **The lesson, and it is about tests, not marks:** seven of the converted marks had NO spec — make the mark never draw and every suite stayed green — so the conversion's real risk was invisible, and wordiply's audience split could have shipped inverted. Six now have one, each planted-verified; scrabble's three are in `board-marks/todo.md` because reaching them needs a staged-and-committed move no scrabble spec has ever built. A second lesson from Joel at the close: the doc grew while the code shrank, because merging three hooks turns their differences into arguments and it is tempting to write the arguments out in the doc — the docstring is what a caller reads, and `doc.md` came back to the length it started at. · **CLOSED 2026-09-16, blessed** (Joel: *"bless the files in this area and close the area"*): fifteen files `cs-blessed-board-marks`; `base.css`'s flash lines were read here and keep `cs-audited-corecss`. The marks a board wears for a beat, and every mark's lifetime. Renamed from `move-flash` 2026-09-16 (F-12): a move's attention flash is one of the things in it, and by the end of the area the newest were about a refused WORD. Twenty-three findings in `plans/areas/board-marks.md`, all worked: fifteen from the reading and the board work, seven from the closing re-read, one from Joel at the close. What changed the app: `useMoveAttention` (the attention mark end to end), `useChangeCause` answering in three parts (setgame on the shared parts), `useMark` and `useAnnouncedMark` (a tagged mark, and the announce-then-answer sequence), no default duration anywhere, one clock per mark (`feedbackTiming.ts` publishes the durations, no stylesheet declares them), and eight games' marks brought onto the vocabulary — a record players will feel, kept in the area file's "Games this area changed". The re-read's lesson, for the sixth area running: six of its seven were the area's own fixes writing stale claims next door — and every effect the area wrote was a bare arrow, which Joel caught, so the effect-name grep now sits beside the docstring-marker pass. **Read [tile-feedback.md](tile-feedback.md) at every game area** — this folder is the mechanism that pass is about |
...
| 46 | `info-sheet` | `info-sheet` | **CLOSED 2026-09-19, blessed** (Joel: *"bless the files in this area, close the area, and commit"*); sixteen files `cs-blessed-info-sheet`. Audited 2026-09-18 (Joel: *"this is the roster. do the audit"*); `turnText` stays `cs-blessed-feedback` as evidence. The info column: its mobile sheet, its switch, the bordered panel its readouts wear, the whose-turn line (`TurnStatusLine`, moved in from `turn-log` 2026-09-12), and its action row (`InfoActionsRow`, moved in from `game-page` 2026-09-15). Seventeen findings in `plans/areas/info-sheet.md`. **The prose pass landed 2026-09-18** — the marker pass, the switch docstring arguing from a sheet width the same commit abolished, "used to" in seven files, the consumer rosters in five files and two docs (each now naming its condition), the stylesheet crediting a hook that renders nothing of it, the `@/` import inside `common/`, and a `doc.md` grown from one paragraph to an intro plus a Details with the render tree (off `INTROS_OWED`); it swept twelve files outside the folder that still opened their mobile comment on the deleted "Game info" menu item or the retired `wide` prop. **F-11 then ruled and shipped (b) the same day** — the info column's own stylesheet, `info-sheet/infoCol.module.css`: the column box and its eight row classes left `game-page/playArea.module.css` on the line that a game's PlayArea root div keeps its classes (`.layout` / `.mobileFill` / `.responsiveInfoCol`, the column-width clamp among them) and the column keeps its own; eighteen files repointed one import, no class renamed, no pixel moved. **F-12 ruled and shipped (a)** the same day: the row's `Exclude<Outcome, 'error'>` is gone, an `.outcome_error` rule inks the seventh word, and `Exclude<Outcome, …>` now appears nowhere in the repo — the three surfaces that show an outcome agree about the vocabulary. **F-15 ruled and shipped** the same day: the info column's unnamed `0.95rem` became **`--font-size-packed`** in `base.css` — a role name beside the font ramp rather than a step on it, for text where vertical space is the scarce thing — and the folder's other fits took their tokens, which emptied three `DECLARED_AHEAD` entries and turned up a false claim in blessed `StandardButton.module.css` (an info-column button has always been 1rem; the column sets no font-size). **F-16 ruled (2), no change to the CSS:** `infoPanel.headerRow` stays rather than becoming `core-css`'s `.heading-with-controls`, because that pattern is a global class whose readers are pages while these two are shared components — the reason now sits on the rule, and `patterns/heading.css` and `docs/ui.md` stop claiming the info column's two rows as instances of it, which they were not. **Nothing left in the area needs a decision, and the one "measured bug" was not one.** F-7 is WITHDRAWN on a false premise: the terminal turn line's character is a literal **U+00A0**, written as the character itself, so the line is 17px and the four places saying it holds its height were right all along. The audit had read it as U+0020 — which it is indistinguishable from in every view of a source file — and its measurement script then reproduced 0px from a scaffold typed by hand. A claim about a character is a claim about bytes. That line now says what the character is; Joel's ruling that *"changing things at terminal is ok"* stands but is kept local, so the code is unchanged. **F-14 worked:** `OpponentStrip.metricLabel` is required, the migration excuse it was optional for having finished — all thirteen callers already passed one — and the render's `&&` guard went with it. **F-13 worked too:** `useIsInfoSheetOpen`, the rename `docs/code-conventions.md` had scheduled for whenever this folder was next worked on — its "one folder still spells it the other way" paragraph is deleted with it. **The closing re-read landed 2026-09-18, F-18 to F-26**, all prose and all worked: eight of the nine were the area's own work next door — two docstrings pointing at the intro for an argument the harvest had put under Details, the row's comment still calling its classes the scaffold's after F-11 moved them, F-12's rewrite calling `noted` a move judgment, F-3's "used to" in two more files, F-4's roster shape one section down in `playarea.md`, F-11's width line told one-sidedly in two files, a tally in the todo, and F-2's vanished menu item and ✕ in sixteen e2e sites (fourteen files) the sweep had counted as two, every one quoting a `GamePage` comment that no longer exists. The `doc.md` harvest is checked: each ruling has a durable home. Twenty-six findings, all worked, ruled or withdrawn. Open in its `todo.md`: the shared `<InfoCol>` and `composes:`, which wait for a game or two (Joel) |
| 47 | `pdf` | `pdf` · `shared/wordle-style/pdfTiles.ts` | **CLOSED 2026-09-19, blessed** (Joel: *"bless all files in this area, close it, and commit"*): fourteen files `cs-blessed-pdf` — the eleven agreed at the opening and three the area wrote (`fakeJsPdf.ts`, `columns.test.ts`, `pdfTiles.test.ts`). Opened and closed the same day, ahead of its position, because `psychicnum` paused pre-close on it. Printing a board — the frame, the three body families, the marks, and the colored letter tiles the hidden-target games print (Joel: the tiles are printing, so they are read here). Eighteen findings in `plans/areas/pdf.md`: seventeen from the reading, all ruled and shipped one commit apiece, and one from the closing re-read, ruled no-change. **What changed the docs:** `docs/pdf.md` collapsed into the folder's `doc.md` — brief, a `game · family` roster in place of the per-game table, the setgame hatch story to setgame's doc, the Setup-rows section to `setup-form/doc.md`, seventy-odd cites repointed; the printed ✓/✗ are a find and a miss, never an outcome word. **What changed the code:** the header's height is stated once (`PrintDoc.contentTop`) and the Setup block's height is asked, not re-derived (`setupLineCount` + `setupBlockHeight`); `drawSetup`'s width is required, so every family wraps a long value; the track family's page-wide recap is guarded once (`drawSetupBelow`) and a word-list section never opens below the sheet — the area's one bug; the marks restore the line width they found; the printed log's heading is the game's on-screen word; six knobs no caller turned are gone (`cols` kept, and the `SetupRow` re-export kept because the finding's "one reader" was eleven); one fake jsPDF for the folder, with specs for the tracks, the tiles and the header. The closing re-read found fourteen, all prose and most the day's own findings next door — F-13's words in the doc it wrote, F-6's shape in the test the prose pass never opened, F-7's census in two test headers, F-1's sweep repointing two game-doc cites without reading the sentence. Handed on: setgame's `margin + 46` and its printed "Turns" against an on-screen tally, psychicnum's stale "every rect" comment |
|    | **The shared families** | | |
| 48 | `dict-trie` | `shared/dict-trie` | the shared dictionary trie |
| 49 | `rank-ladder` | `shared/rank-ladder` | **CLOSED 2026-09-21, blessed.** the Start..Genius ladder, its bar and its stat grid. Ten files `cs-blessed-rank-ladder` — the eight in the folder plus the two `rank_idx_test.sql` the area wrote; opened, read, worked and closed in one day. **Fourteen findings in `plans/areas/rank-ladder.md`, all worked** — twelve from the read and two from the closing re-read. **What changed the app:** the rank squares stopped drawing their own tooltip and ask the shared host instead, which grew one kind of carrier for them — `data-tooltip-on="readout"` reverses all three of the host's control-premised timings (instant on hover, a TAP reveals, a press leaves it up), because nobody crosses a readout on the way to pressing something. Joel raised it out of a duration I had put up for a ruling: *"why would the tooltip here be any different from other tooltips?"* — and caught, before it shipped, that moving to the host as-is would have replaced today's mobile tap with a 450ms hold. **What changed the database:** `_rank_idx` was byte-identical in two game schemas with no pgTAP at all, and is now one `common._rank_idx` with a test per caller, following `common.wordle_colors` exactly. That move was INCOMPLETE and a `gen-types` run found it — deleting a `create or replace` from `supabase/sql/` does not drop the object, so both per-schema copies were still live and would have survived on prod forever; both files now carry the retiring `drop function if exists`. **What the CSS became:** every size in the readout is an `em` rather than a ramp step, so the grid scales with `--font-size-packed` in the mobile status bar and holds its proportions — Joel's correction, after I argued for a bespoke rem on the grounds that an em would draw two different sizes, which is the point. Eleven pending `vocabularies` rows are down to three, each a recorded decision. **The lesson, for the third area running: plant AFTER the fix, not only before.** Three times here a plant that PASSED after the work was written showed the prose wrong rather than the code — a test header claiming integer math prevents a drift that does not exist, a comment calling an assertion "the clamp" that an unclamped index passes, and a finding that said no cascade contest was happening when one was, just not the named one. **And three of the folder's comments described a cascade wrongly in one day**, all found by reading declarations in source order rather than by any check. **The closing re-read earned its place again** — both of its findings were created by this area's own fixes: a rule that ended up in three homes each pointing at another, and F-4's roster surviving four lines below F-4's fix, which is the third area running to produce that exact signature. **Handed on:** six lowercase `freebee` sites in other areas' files (the retired codename, not the live `FreeBee` brand — the capitalization separates them) |
...
| 51 | `wordle-style` | `shared/wordle-style` | **CLOSED 2026-09-22, blessed.** Four files `cs-blessed-wordle-style`; opened, read, worked and closed in one day. The per-letter color codes of the hidden-target games. On SCREEN only: `pdfTiles.ts` is `pdf`'s, so this folder is named by two rows. **Four roster files** — the mapper and its spec, plus the two `colors_test.sql` that pin `common.wordle_colors`, taken in the way `rank-ladder` took its two `rank_idx_test.sql`; `supabase/sql/common.sql` is read as a function slice and stays `cs-unmet`, being shared by every area with SQL in it. **Twelve findings in `plans/areas/wordle-style.md`, all settled** — seven from the read, five from the closing re-read, one closed with no change. **The area's subject turned out to be a duplication it did not own.** `common.wordle_colors` had a second implementation in TypeScript, inside waffle, so the turn-history viewer could color a past board; the two were held together only by test vectors copied by hand from pgTAP, and the copying had already failed — five of the oracle's twelve, with the unpinned line provable by planting. Copying the rest was the fix here (F-2). Joel then took it further and **waffle's swap log now stores each board's colors**, which deleted the port, the vectors and the reason `_solution_for` hands the coop client the answer mid-game; that work shipped to prod the same day with a backfill over 830 rows. **What changed in the folder:** the guard over the stylesheets stopped being a hand-written list of four and now DISCOVERS every sheet defining a judged class, keyed on those rather than on `blank` because boggle spends that word elsewhere, with the keyboard exempt from `blank` for a reason a type states (`Exclude<TileColor, 'blank'>`); a planted stylesheet in a game that never touched the palette fails it three ways, and the old list passed it. The mapper's docstring lost a refactor story and a call-site count and hands the argument for the naming to `docs/tokens.md` → The buckets, which was already making it. **The closing re-read earned its place for the seventh area running** — four of its five findings were the day's own prose: a count that rotted the day it was written, in the file written to replace a finding about counts; two sentences describing the guard four lines below the paragraph announcing it had changed; a thirty-line docstring marker sitting on `process.cwd()`; and `common.wordle_colors` still crediting the TypeScript port that had been deleted hours earlier by work this area's evidence started. **Handed on:** nothing — `todo.md` is empty |
| 52 | `onscreen-keyboard` | `shared/onscreen-keyboard` | **CLOSED 2026-09-22, blessed.** the on-screen QWERTY for a game whose letters land on the board. Three files `cs-blessed-onscreen-keyboard` — the component, its stylesheet, and the spec this area wrote; opened, read, worked and closed in one day. **Eleven findings in `plans/areas/onscreen-keyboard.md`, all worked** — eight from the read, three from the closing re-read. **What the folder turns on is that its caps are two different things**: the 26 letters are KEYS that hand back a character (an action apiece would be a registry entry per keycap), while ⌫ and Enter are the GAME's own bindings worn through `actionSurface` — one binding drawn twice, which is why Enter can sit gray over an empty guess while every letter beside it is live. **The finding with a consequence was a consumer's:** `gameOver` withdraws the keyboard while keeping its box, and the stylesheet argues inside a drawn box that layout must never move on a state change — but wordiply never passed it, branching on `isTerminal` and unmounting instead, which is `display: none` by another road and about 12.4rem of column gone on the frame a player starts reading their verdict. Fixed forward, and pinned: the first spec for it asserted the keyboard was still in the document, which PASSES with `gameOver` dropped entirely, so it asserts the withdraw class too. **The folder had no spec of its own, and the gap was the reason it exists** — the e2e locates caps to read their color and every typing test in both games drives PHYSICAL keys, so nothing had tapped one and `onKey` wired to the wrong letter would have left the suite green. Four plants against the new spec all bite. **Five pending `vocabularies` rows became three converted and two recorded**, both recorded ones being PAIRS chosen against each other — the two gaps, and the two cap sizes; that `0.85rem` equals `--font-size-2` is a coincidence of arithmetic (Joel), reached by fitting a word to a key. The disabled fade took `--chrome-disabled-opacity` and got lighter, following the decision the rule above had already made about a key being a control. **The closing re-read caught the day's own prose for the fourth area running:** the spec written an hour earlier opened by rostering the coverage it was joining and narrating what came before it — F-4's fault plus archaeology, in the file written after fixing F-4. **Handed on:** nothing — `todo.md` is empty |
| 53 | `grid-and-drag` | `shared/grid-and-drag` | dragging a tile to the right place on the grid |
| 54 | `bee-games` | `shared/bee-games` | **CLOSED 2026-09-21, blessed.** what spellingbee and wordwheel share and nothing else does: the hook factory, the board header it returns, the compete leaderboard row, and their board geometry. The name is KEPT (Joel, 2026-09-21). Four files `cs-blessed-bee-games`; opened, read, worked and closed in one day. **Eleven findings in `plans/areas/bee-games.md`, all worked** — seven from the read, four from the closing re-read. **The opening found six files to DELETE rather than to roster** (`ade3a835`): the FE letter-mask layer in both games, proposed for this roster because the two copies were byte-identical, and investigated instead — `git log -S` shows no component ever imported it, in either game, since 2026-06-18. It was a solver's helper layer ported into an app that never filters a dictionary, because the board builder ships finished word lists; the masks that ARE live are the builders' own, on `bigint` to match the Postgres columns, and both now say why they belong there. **What changed the app, and it was not this folder's bug:** accepting a game invitation from a game page left the previous game's board on screen for a round trip, with a typed word validated against the previous game's shipped list. The invitation toast is mounted at the root, `join` changes only the route's params, `GamePage` keys the surface on `restarts` and every game's `useGame` refetches on `gameId` while keeping what it holds — so the GATE had to notice, and `exists` is no longer state: it is derived from `{ id, exists }`, because the reset this wanted is a lint error here (`react-hooks/set-state-in-effect`) and deriving cannot lag the prop anyway. One line, sixteen games. Its spec was written red first and asserts the property — a different `gameId` REMOUNTS the play surface. **Prose:** the owed intro (why one hook body is right when the two games disagree about the rule of their own game — the multiset rule is spent in an edge function, so a data hook sees the same shape either way), the fork ruling collapsed from four homes to one, the factory's archaeology and its line count gone, the leaderboard row saying what it is FOR. **Tests:** the failure path went from nothing to four cases, 7 → 11. Two plants had shown it open; the FOURTH case exists because a plant got through after the fix — reversing the header/rows priority passed, since no case set both slots. The lesson is recorded there: a plant that passes after the fix says the case list was wrong. **The closing re-read earned its place again** — four more, three of them the prose pass not reaching a sibling, and two in files it had no reason to open (the SPEC's header, and a game doc two directories away that kept a claim the docstring had just lost and added a reader that never existed). Its fourth was a docstring that OVERCLAIMED: `GameSchema` is every schema the client knows, not the two bee games'. **Not stamped by this area:** `game-page`'s three files keep `cs-blessed-game-page` (the re-bless is Joel's), both games' `hooks/useGame.ts` stay `cs-unmet` for their own areas |
| 55 | `found-words` | `shared/found-words` | **CLOSED 2026-09-21, blessed.** the games that accumulate a list of found words — spellingbee, wordwheel, boggle, and wordiply for the submit engine alone. Eleven files `cs-blessed-found-words`; opened, read, worked and closed in one day, which is what a folder created the day before allows. **Twenty-one findings in `plans/areas/found-words.md`, all worked** — fifteen from the read, six from the closing re-read. **What changed the code:** the engine's four answers have a name (`WordSubmitAnswer`), and the three games' answer tables are keyed by it rather than by a local copy, so a fifth answer goes red in three places instead of silently missing a key; `WordEntry` became `LegalWord`, since `WordEntry` is the typed-word CONTROL one folder over and a reader met both in the same file; the merge and the rows call take the folder's own types, dropping a generic that only ever carried one instantiation and four structural re-spellings of two types declared one file away; one map instead of two in the found-row dedup, which killed a `??` arm that could not run; `--avail-h` composes its three terms instead of hand-summing them to `5rem` with a "keep in sync" instruction, proved no-op by re-seeding board-geometry's baseline on the pre-change tree (21 of 21 within 0.5px); and `--board-col-gap` is named in the blessed `game-page` scaffold, on `.layout` rather than `.boardCol` because the element doing the arithmetic is the ANCESTOR — declared on the descendant it would have resolved to nothing and voided the whole declaration. **Tests:** four cases pin what nothing did (the `recordReject` split wordiply's turn economy rests on, `onAnswer` for every answer, the normalize-but-recall-raw rule, the commit that throws), each planted-verified; one case that any implementation passed is deleted. **The area's real find is about the day before it:** eleven of the first fifteen were prose written or moved into the folder by `plans/found-words.md` the previous day — a restructure's tail is prose, and its own check pass read the plan's bullets against the code rather than the docstrings against the callers. **The closing re-read earned its place again** — six more, five of them this area's own fixes not reaching a sibling: the FILE docstring kept the roster F-4 had just removed from five members below it, `revealWords.ts` kept the same count four lines under the lede that fixed it, neither stylesheet had been read for a finding written as a docstring finding, and the spec kept a claim F-3 had retired that morning. The sixth was the new intro's own count. **Handed on:** `doc.md`'s LEDE names the three games, the same fault the stylesheets lost — it predates the area and is Joel's call; bananagrams' and boggle's `SetupForm.module.css` describe a `setupForm.module.css` that exists under no name; `docs/games/bananagrams.md:64` cites the same nonexistent `code-review §1.4` this folder's two cites were removed for. **Beyond the area, from its stale-path sweep:** the pre-reorg prefix `common/components/` is gone from ten game docs and from `cssTokens.test.ts`, where it had been a DEAD exemption — the guard has been stricter than written and passed only because the button folder's files happen not to trip it |
|    | **The games** | | |
| 53 | per game, one area each | `src/<game>/` | **Sixteen areas**, keyed by CODENAME. **THREE passes back to back** (Joel, 2026-09-19, at psychicnum): first the **restructure** — [playarea-readability.md](playarea-readability.md) applied step by step, each step a commit he reads, **and with it the STYLESHEET SPLIT** (Joel, 2026-09-22, at connections): **one CSS module per component, named for it** — `Board.tsx` reads `Board.module.css`, `BoardCol.tsx` reads `BoardCol.module.css` — so `PlayArea.module.css` holds only what `PlayArea.tsx` itself wears, which for a game that draws its board elsewhere is the `.layout` rule declaring `--info-col-width` and nothing else. **The split is by IMPORTER and it needs no judgment**: a module two components import gets split, the partition falls out of which component reads which class, and a class two components both read is the exception — it stays in a module both can reach. Rule bodies and `/* @@ */` markers move verbatim; only the file headers are rewritten, since a header that mapped three regions has three files to describe now. It belongs to the restructure rather than to the audit for the restructure's own reason: the split rewrites every header it makes, so a prose pass ahead of it polishes prose about to be thrown away — then the **audit**, React, SQL and CSS together — **which includes, at every game, the `AnswerMessage` conversion** (Joel, 2026-09-19: *"this definitely part of the audits we'll do for games"*): the game's own answers — correct, wrong, near, already guessed, and their peer lines — built in its `lib/answer.ts` as the `{ outcome, text }` pair `common/feedback/doc.md` describes, so the pill, the log bar and the peer line read one table; psychicnum's `answerMessage()` / `peerAnswerMessage()` is the shape, and outcome-fix left every other game with only the `ANSWER_OUTCOME` table — then the **tile-feedback** pass against [tile-feedback.md](tile-feedback.md). The restructure goes first because the audit's prose pass would otherwise polish comments and docstrings that the split and the comment pass rewrite or delete, which is what psychicnum's step 6 then did to a great many of them. `psychicnum` first, as the control: the deliberately minimal toy, so what it settles is about the shape of a game area rather than about the game — and what it settled is written into [docs/playarea.md → The shape of a game's PlayArea.tsx](../docs/playarea.md), which the other fifteen conform to as each opens. **`psychicnum` is CLOSED 2026-09-19, blessed** (Joel: *"bless all files in the psychicnum area, then close it"*): thirty-nine files `cs-blessed-psychicnum` — the stamped files of `src/psychicnum/`, its three SQL files and its seven pgTAP files. All three passes in one day: the restructure in six steps, the audit (twenty findings in `plans/areas/psychicnum.md`, all ruled and shipped one commit apiece), and the tile-feedback pass to tf2; then the closing re-read (sixteen more, all prose), the `doc.md` harvest, and `docs/games/psychicnum.md` deleted into `src/psychicnum/doc.md`. Paused pre-close so `pdf` could be read first — its printer builds on that folder — and unpaused the same day when `pdf` closed, with the one thing `pdf` handed it worked. The full e2e suite ran green (245) before the blessing. **`connections` is CLOSED 2026-09-19, blessed** (Joel: *"mark all files in this area blessed and close it"*): fifty-six files `cs-blessed-connections` — the roster's fifty-one and five the area stamped as it worked (the presence-pause spec, F-5's and F-22's migrations, common's locally-terminal test, the importer). The second game, in psychicnum's shape and one day: the restructure in eight steps (the doc's RPC and FE-submission sections and the AnswerMessage conversion moved earlier, from psychicnum's commit history), the audit (twenty-two findings in `plans/areas/connections.md`, all shipped one commit apiece), tile-feedback to tf2 (confirmed, not converted), then the closing re-read (eighteen: five ruled, thirteen prose) and a comment pass against what psychicnum settled. `docs/games/connections.md` deleted into `src/connections/doc.md`, which carries the components diagram. Closed without an e2e run. **The stylesheet split landed 2026-09-22, after the close** — the board and the commit row came out of `PlayArea.module.css` into `Board.module.css` and `BoardCol.module.css` — and it is where the restructure's split rule above came from, Joel having asked for it at every game. **`wordle` is CLOSED 2026-09-22, blessed** (Joel: *"mark all files in this area as blessed, then close the area and commit"*): forty-eight files `cs-blessed-wordle` — the roster's forty-six, the pgTAP fixture (F-16), and `shared/wordle-style/tileColors.module.css`, which this area created inside a closed folder and so read here. The third game, in the two earlier games' shape and one day: the restructure in eight steps with the stylesheet split already in place, the audit (twenty-four findings in `plans/areas/wordle.md` — five from the todo and the prose pass, twelve from the read, seven from tile-feedback; twenty shipped, four ruled no-change), tile-feedback to tf2 (the board already conformed; what changed was a hook the game had written for itself where `useMark` existed), then the closing re-read (eight, all prose, all shipped). `docs/games/wordle.md` deleted into `src/wordle/doc.md`. **What changed the app:** a race's winner celebrates; a race the clock ends is written by the one finisher that writes every race's ending, so its status carries `winner_guesses` like a natural finish and the verdict tells a racer still guessing that time ran out; the verdict reads `status.reason` rather than the browser's clock, so an all-conceded race says so; `create_game` refuses a one-player race (`PN498`); the board's height reserve is composed from the four terms it stands for; two render-phase guards and two specs defending against a restart the remount already handles are gone; `WORD_LENGTH` has one home; the Help text stopped assuming two players. **What the read established, for the next game that asks:** wordle is EXEMPT from the identity dot — the event log is one aligned row per guess with the guesser's dot, in the board's own order (Joel: *"the event log is fine, so it's exempt from this requirement"*); its unit of attribution is a ROW, which the rule's geometry has no answer for; and the two dims compounding to 27.8% were looked at and kept. **The lesson, twice over:** the refusal mark had no test at all — disabling the ring left 117 tests green — and the first draft of the test written for it would have passed the plant, because it pinned the very outcome a defaulting mark lands on; and the re-read's eight were the area's own fixes standing next door, for the eighth area running — F-8 moved the finisher and left six pointers naming the function it moved out of, one of them in the test F-8 wrote. **Handed on:** nine games' copies of the end-for-all item (home: `game-page/todo.md`), three manifests claiming a two-player check their servers lack (setgame, stackdown, waffle), `plans/spectating.md` opened for the four games that show a watcher a notice three ways, and `deferred.md`'s per-game table headline, which says "only these games have open items" over three rows whose todos hold none. The full e2e suite ran green (247) after pass 3; nothing since touched behavior. **`spellingbee` is CLOSED 2026-09-23, blessed** (Joel: *"bless the files in this area. then close the area. then commit."*): forty-three files `cs-blessed-spellingbee` — the forty agreed at the opening, `Letter.module.css` from the stylesheet split, and `lib/terminal.ts` with its test from Step 6. The fourth game, and the first with an edge function on its roster, whose read took the ordinary shape plus three checks of its own: what it trusts against what `create_game` re-checks, which refusals the player can act on, and that it BOOTS, which `deno check` cannot prove. The restructure in eight steps with the stylesheet split; the audit (seventeen findings in `plans/areas/spellingbee.md`, nine from the todo and the prose pass, eight from the read; sixteen shipped, one ruled no-change); tile-feedback to tf2 (six proposals, four shipped); then Joel's shake change and the closing re-read (five, all shipped). **What changed the app:** a race's winner celebrates; a word typed into a game a friend just deleted is the shared race, not a BUG modal — and **the ruling is roster-wide**, written into `docs/envelopes.md`, with the thirty-three other functions that still fault one entry in `docs/deferred.md`; a refused word shakes its own hexes rather than the hive, and its hexes stroke the shared verdict edge; the tapped hex no longer stays raised on a touchscreen; Help stopped promising a pangram a custom board need not have, and explains bonus words and the two modes; `create_game` answers a non-numeric band in the envelope (`PN499`, `PN500`); every per-player result is `{ won }`; the leaderboard query is one helper; both shuffles are the shared one. **What the read established, for the next game that asks:** the khaki selected edge is the word-finding family's, written into tile-feedback.md → The channels; the bee center colors are brand tokens; an SVG hex cannot wear `.tileFace` and is recorded as unable, not owed. **The lesson:** the audit's first grep counted nine functions answering a deleted game as a fault, and there were thirty-four — a count taken from one phrasing of a message misses every other phrasing; and the re-read's five were this area's own fixes not reaching a sibling (F-16 cut the coop results and left the compete ones), for the ninth area running. **Handed on:** the thirty-three deleted-game conversions (`deferred.md`), three Soon entries this area added to `wordwheel/todo.md` for its twins of spellingbee's fixes (the leaderboard helper, the status-merge comments, the unread result keys) beside the shuffles entry it already had, and wordwheel's ungated hover, which stays in `deferred.md`. Closed without an e2e run, and nothing seen on a device. **`codenamesduet` is CLOSED 2026-09-23, blessed** (Joel: *"bless all the area's files, then close the area, then commit."*): seventy-seven files `cs-blessed-codenamesduet` — `src/codenamesduet/`, its SQL, both migrations, the edge function, thirteen pgTAP files, the `codenamesduet-events` e2e spec, and `common/info-sheet/InfoDisclosure` with its spec and stylesheet, which this area created inside a closed folder and so read here. The fifth game, and the one whose log was rebuilt mid-restructure: the restructure paused before Step 5 for `plans/codenamesduet-events.md`, which turned `clues` + `guesses` into one `codenamesduet.events` table in the standard shape, logging a pass and an AI hint for the first time, and went to prod the same day with 68 past passes inferred by its backfill (the plan was deleted at the harvest; `doc.md` and the migration's comments hold what it learned). Then the restructure's other steps (the stylesheet split found nothing to split), the audit (twenty-eight findings in `plans/areas/codenamesduet.md`: twenty-one shipped, six ruled no change, and F-14's three Soon items worked), tile-feedback from tf0 to tf2 (seven proposals, all ruled, the shake added by Joel), and the closing re-read (nineteen, eighteen shipped, one ruled no change). **What changed the app:** the first clue-giver picker offers only the selected players; sudden death has no clue seat, so a clue, a pass and an AI ask there are refused in their own words (PN502–PN504); every sudden-death guess is a turn and a row of the log, and the history link is an event id; a clue given exactly as the AI suggested it is marked in the log, and a partner's asking is narrated; the bell rings for both players, on the clue to give and the clue to guess from; the key card is a disclosure in the info column; `replay_board` takes the row lock; the AI is shown the board words it is told to avoid; the count is one digit; the PDF and the readout count turns as the screen does, and a sudden-death ending no longer reads "11/9 turns"; Help is right in five places; and the deleted-game sweep taken from this area's F-12 made every move path in the roster answer a missing game as the shared race. **Handed on:** the blessed files this area edited in closed areas — psychicnum's, connections' and wordle's SQL and tests (the deleted-game sweep), wordle's `Board` (the history flip fix), `common/info-sheet/MobileStatusBar.tsx` (R-12) — keep their old stamps, and their re-blessing is Joel's. The nine codenamesduet e2e tests ran green after the harvest; the full suite did not run. **Not deployed:** the SQL, edge functions and FE since the events deploy |
