...
| 55 | `found-words` | `shared/found-words` | **OPEN 2026-09-21; the audit READ is DONE the same day.** Eleven files `cs-met-found-words`; fifteen findings in `plans/areas/found-words.md`, nothing in the code moved at the read. Five are the prose pass, three are shape with a decision in them, the rest a dead branch, two tests (one confirmed by planting: four hook branches have no spec), the stylesheet, and stale paths in two game docs. the games that accumulate a list of found words — spellingbee, wordwheel, boggle, and wordiply for the submit engine alone. Renamed from `word-hunt` 2026-09-21 and given the family's row and word types, the typed-word look and (step 4) its play-surface scaffolding. The roster took `useFoundWordSubmit.ts` off `cs-fixed-outcome-fix` and `wordListRows.ts` + its test off `cs-unmet`, the pair `word-list` wrote and left for this area (its F-31) |
|    | **The games** | | |
