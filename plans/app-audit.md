...
| 54 | `bee-games` | `shared/bee-games` | **OPEN 2026-09-21; the audit READ is DONE the same day.** what spellingbee and wordwheel share and nothing else does: the hook factory, the board header it returns, the compete leaderboard row, and their board geometry. The name is KEPT (Joel, 2026-09-21, [found-words.md](found-words.md) call 5). Four files `cs-met-bee-games`; **seven findings in `plans/areas/bee-games.md`**, nothing in the code moved at the read. **Six files were deleted before the roster was agreed** (`ade3a835`): the FE letter-mask layer in both games, proposed for this roster and investigated instead — `git log -S` shows no component ever imported it, in either game, since 2026-06-18. **One finding is a reachable BUG whose cause is upstream** (F-7): accepting an invitation from a game page changes the route's params without unmounting, `GamePageGate` never resets `exists` to 'checking', and the play surface is keyed on `restarts` — so for one round trip the player is on game B's URL looking at game A's board and word lists. Every game's `useGame` has that shape; the one-line fix is in blessed `game-page` |
