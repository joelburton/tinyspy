...
| 51 | `wordle-style` | `shared/wordle-style` | the per-letter color codes of the hidden-target games. On SCREEN only: `pdfTiles.ts` is `pdf`'s, so this folder is named by two rows |
