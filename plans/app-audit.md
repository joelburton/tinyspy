...
| 45 | `reveal` | `reveal` | **OPEN — audited 2026-09-18** (Joel: *"this is a tiny section, just do the audit"*), two files `cs-audited-reveal`. Showing the answer after the end: `useSolutionReveal`, the personal, temporary, unpersisted display choice, and `solvedByMe`, the predicate that starts a solver's answer shown. The design shipped 2026-08-15 and is documented in [common.md → Revealing the solution](../docs/common.md#revealing-the-solution) and [ui.md → Terminal results](../docs/ui.md#terminal-results--the-moment-vs-the-record); this row used to say the area "has to build" it, three weeks after it had. Ten findings in `plans/areas/reveal.md`: six prose (a 52-line docstring over 9 lines of code, carrying the design, the history, two censuses and a link to a memory file; two docs saying "nothing autoreveals" written the day the clear-win default landed), the row itself, two decisions — six copies of the same `describe()` in the clear-win games, and "solution" vs "answer" as the control's noun — and Joel's own: `reset` and `hide` have had no caller since the restart key (2026-09-15) unmounts the surface, so they go. **The prose pass ran 2026-09-18** (F-1 to F-7): the hook's docstring is twelve lines and points at the folder's new `doc.md`, which carries the design, the caller table and the per-game reasons (`common/reveal` off `INTROS_OWED`). Its lesson is the sweep it turned into — *"nothing autoreveals, a win included"* was written in ten more places than the two the audit named, in six game docs and four components, every one of them false for its own game, and one of them (strands' PlayArea) contradicted by the paragraph six lines below it. **F-10 followed the same day**: `reset` and `hide` had had no caller since the restart key (2026-09-15), so `SolutionReveal` is `{ revealed, toggle, impliedBySolve }` and the hook is six lines. Its test case was deleted rather than rewritten — the remount is `GamePage`'s to pin, and it already does, by counting mounts — and the reasoning it carried moved to `doc.md`. `onRestarted`, a callback that exists nowhere in `src`, went with it from five sentences and an e2e comment, one of which credited it for a re-hide that `_target_for` does |
