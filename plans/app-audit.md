...
| 42 | `word-list` | `word-list` | **OPEN 2026-09-20**, paused the same day behind the re-opened `board-marks` (row 35) and **resumed** once that work landed. Seven files `cs-audited-word-list`, seventeen findings in `plans/areas/word-list.md`, none worked; the prose pass is the next step. The alphabetical finds readout — common, not a family's: it takes its rows as a prop |
...
| 55 | `found-words` | `shared/found-words` | the games that accumulate a list of found words — spellingbee, wordwheel, boggle, and wordiply for the submit engine alone. Renamed from `word-hunt` 2026-09-21 and given the family's row and word types, the typed-word look and (step 4) its play-surface scaffolding |
|    | **The games** | | |
