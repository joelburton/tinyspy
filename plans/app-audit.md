...
| 3 | rebuild `/palette` | **swatch half DONE 2026-08-20.** The in-situ half — every variant rendered DOING ITS JOB, ink as text, bar in a list row, fill on a tile — is DEFERRED, not skipped: it was built by hand, was wrong about the pill in three ways at once, and got reverted. It returns when the demos can render the REAL components (`<FeedbackPill>`, `<Dot>`, `<TurnLogOutcomeBar>`, the shared `.tile`), which may mean after those components' areas. An invented example is worse than none — it manufactures evidence about the stylesheet. `/palette` and `/font` are otherwise OUT of the sprint (§3) |
...
| 39 | `outcome-fix` | no folder — every game's move path: the `useGame` row seam, `lib/evaluate` / `lib/history`, `GameTurnLog.tsx`, the board's verdict mark, the feedback pill, and each game's PDF | **OPEN — all eleven games worked (twelve commits, `1e6d65e5`..`9a207b74`), the closing re-read done 2026-09-17.** Every game decides a move's outcome once, in `lib/answer.ts` (codenamesduet's turn in `lib/turnOutcome.ts`; waffle needs none), the pill reads `res.outcome`, rows index the table, and each half is pinned. Seven games' SQL changed a word. The re-read found seventeen (F-5 to F-21 in `plans/areas/outcome-fix.md`): the doc half is fixed; the code half is open — psychicnum's spoiler comment still says `warning`, connections' `verdict.tone` the plan promised at step 8, the "help"-for-hint sites the sweep missed, stale outcome claims per game, strands' pill indexing the table instead of the envelope (a decision), stackdown's ambiguous ring in `error` ink under a `warning` pill (a decision), five peer-line `'won'` literals, unpinned envelope words in three games and the unpinnable "no outcome" cases, no `answer.test.ts` for spellingbee/boggle/wordiply, the archaeology the area wrote into its comments (one date wrong). Five todos it filed are open. **Not a blessing area** (Joel, 2026-09-17): its files belong to other areas and should not have been stamped for it. Was: **one move, one outcome, determined once** — scheduled 2026-09-16 out of `turn-log` F-17, which is SKIPPED because this area catches it. Joel's rule: *"the outcome should be determined ONCE in ONE PLACE. that place is often directly from the server… the server's outcome, if provided, is always right"* — and where the FE decides instead (no server row, an FE-only refusal), that answer is still the one every consumer reads. **It should be impossible for a turn-log entry to say `near` when the determined outcome was `lost`.** Two shapes to trace, and the second is the cause of the first: (a) **a consumer re-deriving the verdict** — psychicnum's log computes `isReveal ? 'near' : g.is_correct ? 'won' : 'lost'` in its JSX, setgame reads a row's *kind* as a verdict (`kind === 'claim' ? 'won' : 'near'`), stackdown and letterboxed map `kind === 'hint'` to `near` in a helper beside the log; (b) **a narrowed type upstream that made the right word unsayable** — connections' `GuessOutcome = Extract<Outcome, 'won' \| 'near' \| 'lost'>` with `OUTCOME_FOR_RESULT` and a reverse `RESULT_FOR_OUTCOME`, already on record in [outcomes.md](../docs/outcomes.md) as an open narrowing. **The rule the plan states rather than asks:** the seam stores `Outcome`, whole — a game that only ever produces three of them still types the field as `Outcome`, because the field is "what this move was", not "what this game currently happens to answer". connections has the PLACEMENT right (one conversion at the row seam, consumers reading it, and its docstring names the PDF as a fourth consumer) and the type wrong, which is `TurnOutcome`'s disease one level down. **The four games' wrong `near` on hint and reveal rows is the proof this area worked** (Joel) — they are not fixed beforehand. Ruled at the plan (2026-09-16): a hint is `warning`, a reveal or spoiler is `lost`, and "help" is never the word for either |
| 40 | `history-always-available` | no folder — the ten viewer games' `lib/history.ts` + `PlayArea`, `common/turn-log/useTurnLogPlayerPicker.tsx`, and each game's `Board` | **the turn-history viewer works the same everywhere, and its names say what they mean** — scheduled 2026-09-16 out of `turn-log` F-20 (SKIPPED to it) and the todo that came before it. Today the `#N` handle dies whenever the log on show is not the board's own sequence, and the picker's `boardIsShown` is ONE flag over TWO unrelated conditions. **Coop with one player picked is a bug**: the board is shared, every row in the filtered list is a real event in its history, and the handle dies only because the snapshot builders index by position in the SHOWN list — every one of the seven games already carries a stable id on the row, so resolving it against the unfiltered log is the whole fix. **Compete with an opponent picked is a missing feature**: the board on screen is yours, so their turn has nothing to replay on; mid-game their rows are RLS-hidden anyway, but **at terminal the rows are all there with their feedback colors** and each snapshot builder is pure — what is missing is the decision that a compete log's `#N` may replay SOMEONE ELSE's board, and the UI to say whose board you are looking at while it does. Joel wants it: reading moth's finished wordle as six rows of text where the game can draw it as a board is the gap. **setgame is the existence proof** that the coop half is reachable — its own comment says it offers the viewer on every row whatever the filter, because its snapshot is the `board_after` stored on the event rather than a fold over one player's sequence, so "any row can be opened without knowing whose board it belonged to". **wordiply is the other odd one**: a turn log with no viewer at all, so its rows have no `#N`. Joel: *"setgame and wordiply shouldn't need to be different around this stuff."* **The naming comes with it**, which is why F-20 is skipped here rather than worked: `boardIsShown` reads as always-true (there is always a board shown) where the value means *"the log shows every move made on the board you're looking at, and nothing else"* — and whether the right name is `canOpenHistory` (what the seven call sites ask) or `logMatchesBoard` (what the value is) depends on whether an unmatched log becomes openable |
| 41 | `z-index` | no folder — `core-css/base.css` → THE Z- LAYERS, twelve games' board stylesheets, `playArea.module.css`, `shared/grid-and-drag`, `shared/rank-ladder`, `shared/board-cursor` | **the stacking order, end to end** — scheduled 2026-09-16 out of `turn-log` F-12, which is SKIPPED because this area catches it. Audited from turn-log: 38 z-index declarations, six reading a rung; **thirty local literals ≤ 5 that are legitimate** by base.css's own rule (layering inside a component's own stacking context); and **two that are not** — the drag ghost at `1000` in bananagrams and `100` in scrabble, the same element at two numbers, both fixed-position and genuinely page-level, with `--z-ghost` (1200) sitting unread for both. The area exists because ONE decision governs all of it: **do boards become sealed stacking contexts?** `--z-board`, `--z-board-question` and `--z-ghost` are declared and read by NOTHING, and `vocabularies.test.ts` says `--z-board` gaining a reader is the signal that boards got sealed — so whichever game's area converted first would answer it for all twelve, silently. Meanwhile `base.css` says the play surface is 1000 and it is 0–5 everywhere, so the ladder documents a state the app is not in. Area-shaped rather than a sweep: changing a stacking order changes what paints over what, so it wants looking at per surface, and it starts from a design decision rather than a mechanical rule |
| 42 | `word-list` | `word-list` | the alphabetical finds readout — common, not a family's: it takes its rows as a prop |
| 43 | `word-entry` | `word-entry` | the typed-word box and its row. No `<input>`; keystrokes come off the window |
| 44 | `terminal` | `terminal` | what shows when a game ends. `terminalMessage` is here but `feedback` owns its words |
| 45 | `reveal` | `reveal` | showing the answer after the end — and [the reveal-solution taxonomy](../docs/deferred.md) it has to build |
| 46 | `info-sheet` | `info-sheet` | the info column: its mobile sheet, its switch, the bordered panel its readouts wear, the whose-turn line (`TurnStatusLine`, moved in from `turn-log` 2026-09-12), and its action row (`InfoActionsRow` + its test + its stylesheet, moved in from `game-page` 2026-09-15, all `cs-unmet`). `turnText` is here but `feedback` owns its words |
