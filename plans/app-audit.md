...
| 42 | `word-list` | `word-list` | **OPEN 2026-09-20, then PAUSED the same day** behind the re-opened `board-marks` (row 35) and [one-mark-hook.md](one-mark-hook.md); it resumes when that lands. Nothing is abandoned: seven files `cs-audited-word-list`, seventeen findings in `plans/areas/word-list.md`, none worked; the prose pass is still the next step. The alphabetical finds readout — common, not a family's: it takes its rows as a prop |
