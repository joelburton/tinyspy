...
| 46 | `info-sheet` | `info-sheet` | **OPEN — audited 2026-09-18** (Joel: *"this is the roster. do the audit"*), fifteen files `cs-audited-info-sheet`; `turnText` stays `cs-blessed-feedback` as evidence. The info column: its mobile sheet, its switch, the bordered panel its readouts wear, the whose-turn line (`TurnStatusLine`, moved in from `turn-log` 2026-09-12), and its action row (`InfoActionsRow`, moved in from `game-page` 2026-09-15). Seventeen findings in `plans/areas/info-sheet.md`, none worked: ten prose and conformance (the marker pass, a switch docstring arguing from a sheet width the same commit abolished, "used to" in seven files, consumer rosters in five files and two docs, a stylesheet crediting a hook that renders nothing of it, a one-paragraph `doc.md`), **one measured bug** — the terminal whose-turn line renders a plain space and is 0px tall, so the play→terminal flip reflows the column in every turn-order game while five places say it cannot — and six with a decision: where the info column's shared readout classes live (the folder table says here, the code says `game-page`; recommended: move the readout family here as a lowercase sheet), the row's `Exclude<Outcome, 'error'>` (recommended: drop it, per outcomes.md's own ruling), the `useInfoSheetOpen` rename code-conventions.md already made, `metricLabel` optional for nobody, fourteen pending literals (one real question: the column's unnamed `0.95rem`), and `infoPanel.headerRow` as the heading pattern by hand |
