...
**Brand: FreeBee.** The codename `spellingbee` is what the code says everywhere —
...
that is `src/spellingbee/`, its two SQL files, and `docs/games/spellingbee.md`.)*

## Findings

*(IDs are `F-spellingbee-1`, `F-spellingbee-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/spellingbee.md` → Deferred,
