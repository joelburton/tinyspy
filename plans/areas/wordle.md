...
**Brand: WordNerd.** The codename `wordle` is what the code says everywhere —
...
that is `src/wordle/`, its two SQL files, and `docs/games/wordle.md`.)*

## Findings

*(IDs are `F-wordle-1`, `F-wordle-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/wordle.md` → Deferred,
...
- `e2e/wordle-keyboard.e2e.ts` is RED and it is NOT a flake — line 162 reads `--ink-on-dark-color`; the token is `--ink-onDark-color` (camel)
