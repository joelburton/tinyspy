...
**Brand: SnakeBox.** The codename `letterboxed` is what the code says everywhere —
...
that is `src/letterboxed/`, its two SQL files, and `docs/games/letterboxed.md`.)*

## Findings

*(IDs are `F-letterboxed-1`, `F-letterboxed-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/letterboxed.md` → Deferred,
