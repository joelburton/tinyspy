...
**Brand: TinySpy.** The codename `codenamesduet` is what the code says everywhere —
...
that is `src/codenamesduet/`, its two SQL files, and `docs/games/codenamesduet.md`.)*

## Findings

*(IDs are `F-codenamesduet-1`, `F-codenamesduet-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/codenamesduet.md` → Deferred,
...
- `CluePanel` needs a name that says what it is — and the first answer was WRONG: it is not the AI suggester, and it is not a floating panel
- `Board.module.css` sets `ui-monospace, Menlo, monospace` — the most consequential mono use, because it is a play surface rather than a form
