...
**Brand: SyrupSwap.** The codename `waffle` is what the code says everywhere —
...
that is `src/waffle/`, its two SQL files, and `docs/games/waffle.md`.)*

## Findings

*(IDs are `F-waffle-1`, `F-waffle-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/waffle.md` → Deferred,
...
- `SolutionReveal` sets monospace twice so the revealed grid lines up — the alignment need is real, monospace as the answer is not obvious
