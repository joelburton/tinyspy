...
**Brand: PaulPath.** The codename `strands` is what the code says everywhere —
...
that is `src/strands/`, its two SQL files, and `docs/games/strands.md`.)*

## Findings

*(IDs are `F-strands-1`, `F-strands-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/strands.md` → Deferred,
