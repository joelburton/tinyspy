...
**Brand: WordKnit.** The codename `connections` is what the code says everywhere —
...
that is `src/connections/`, its two SQL files, and `docs/games/connections.md`.)*

## Findings

*(IDs are `F-connections-1`, `F-connections-2`, … — §21 → Areas. Every heading states its
...
genuinely OUT of the sprint's scope goes to `docs/games/connections.md` → Deferred,
