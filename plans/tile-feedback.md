...
So: **the keys wear the disabled look and the keyboard stays readable.**
SHIPPED 2026-09-22, Joel's call — the same treatment as a keyboard that is not
your turn, which is the app's existing word for "no move to make right now",
rather than a second look meaning the same thing. The `gameOver` prop is gone
from `<GuessKeyboard>` along with the `visibility: hidden` it drove; both games
already disable their keyboard at terminal, so both moved by deleting a prop.
The tension the reversal named — reading as inactive without dulling the very
letter colors it exists to show — is answered by `--chrome-disabled-opacity`
fading the whole surface uniformly, which leaves every tone still legible
...
- **Done 2026-09-15: the band flash reaches the player whose guess made it**, per
  the default above.
- **Done: a wrong or near guess takes the FLASH as well as the verdict**, per
  the default — `BoardCol` flashes attention on every verdict and starts the
  shake once it has faded, so the flash hands the piece back its color. (Listed
  "open" until 2026-09-19; the code had answered it.)
- Keep the good case's tiles unmarked: they become the band, which says more.
- **Ruled 2026-09-19, all NO** (the tf2 pass): no dashed hint mark on the board
  — connections' hint names a tile in the info column and stays there; no
  in-flight dim shown to the rest of a coop table, which would take a second
  broadcast event; no arrival animation when four tiles become a band, so the
  flash is the whole of it.

### codenamesduet · shape 1

**tf2, 2026-09-23** — see the roster row. What follows is the section as it
stood before the pass, kept for the proposals it records.

**Before.** The one turn-based game, and the one with no shared board marks at
...
|---|---|---|---|
| **wordle** | **tf2** | 2026-08-16, board marks 08-17; **tf2 2026-09-22** | first through: the in-flight dim, the verdict ring in its pill's tone, hover-as-shadow, blue history, the keyboard as a control surface. Then the four board-scope marks; the keyboard was withdrawn at terminal and now stays, disabled (2026-09-22). It went first because it needed the fewest decisions — no selection, no hint, no cursor. **tf2 found the board already conformed** and changed nothing about what it draws: the refusal mark became `useMark` on the published `WORD_ANSWER_MS` (it had two `useState`s, an effect and a hand-tuned 900), the identity dot is EXEMPT (above), and the two dims compounding to 27.8% were looked at and kept |
...
| wordle | 19 → 21 → **20** | 59 → 59 | a wash: lost `.busy` + the reject ring, gained `.letter` + `.inFlight`. tf2 took `.blank` back off — its body restated the grid's own tokens four rules above it |
...
| codenamesduet's bystander tan `--codenamesduet-neutral` `#b4986e` | **2.74:1** | 2026-09-23, at codenamesduet's pass (Joel: *"it look fine"*) |
