...
--
-- club_id is denormalized from common.games.club_id for RLS-
-- efficient is_club_member(club_id) checks without a join.
--
...
-- is_club_member(club_id). History is club-wide — any club member
-- can see every game in the club, regardless of whether they sat
-- down at this specific game. The earlier "is_player_in_game"
-- check is gone; "is in this game" is now a gameplay question
-- (handled by require_game_player at the RPC layer for actions),
...
begin
  ...
  -- common.games + game_players rows, returns canonical id.
  new_id := common.create_game(target_club, 'tinyspy', player_user_ids);
  ...
  -- cards live here as columns now (not in a side table).
  insert into tinyspy.games (
    id, club_id, status, current_clue_giver, turns_remaining,
    user_a_id, user_b_id, key_card_a, key_card_b, setup
  ) values (
    new_id, target_club, 'active', 'A', s_turns,
    seat_a, seat_b, to_jsonb(a_view), to_jsonb(b_view), setup
  );
...
revoke execute on function tinyspy.create_game(uuid, jsonb, uuid[]) from public;
grant execute on function tinyspy.create_game(uuid, jsonb, uuid[]) to authenticated;
...
begin
  ...
  elsif g_row.status = 'sudden_death' and revealed_label <> 'G' then
  ...
  if end_outcome is not null then
    ...
    perform common.end_game(
      ...
      jsonb_build_object(
        'outcome', end_outcome,
        'turns_used', (g_row.setup->>'turns')::int - g_row.turns_remaining
      ),
      player_results
    );
...
-- (No clear_active_on_termination trigger. Terminal-transition
-- cleanup of common.club_active_game now happens inside
-- common.end_game, which submit_guess calls explicitly at the
-- moment it decides the game is over. Single write path is
-- cleaner than the trigger-on-status-change side effect.)
