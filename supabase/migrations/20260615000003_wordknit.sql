-- ============================================================
-- wordknit — Connections-style word-grouping puzzle
-- ============================================================
--
...
--
-- wordknit ships as a coop/compete PAIR via the sibling-manifest
-- pattern (mirroring psychicnum): one schema, one folder, two
-- `common.gametypes` rows ('wordknit_coop' / 'wordknit_compete'),
...
-- matched categories) lives on wordknit.players + wordknit.guesses
...
-- in create_game.)
--
-- wordknit.games.id is FK'd to common.games(id) — the canonical
...
-- denormalized onto wordknit.guesses so the mode-aware partial
...
create table wordknit.games (
  ...
  -- ('wordknit_coop' / 'wordknit_compete') by construction in
  -- create_game.
  mode text not null
...
-- ============================================================
-- wordknit.players — per-player tracking
-- ============================================================
...
--
-- Filtered to gametype in ('wordknit_coop', 'wordknit_compete')
-- (defensive; common.games.id ↔ wordknit.games.id is one-to-one
-- by FK, but the join condition doesn't say "and only wordknit,"
...
--
--   1. common.create_game(target_club, 'wordknit_<mode>',
...
--   3. INSERT one wordknit.players row per player_user_ids entry
...
-- ============================================================
-- wordknit.submit_guess — record a submission (mode-aware)
-- ============================================================
...
-- ============================================================
-- wordknit.submit_timeout — countdown expiry handler (mode-aware)
-- ============================================================
