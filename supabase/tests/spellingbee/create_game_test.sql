...
select is(
  ...
  'spellingbee_coop',
  'common.games.gametype = spellingbee_coop (mode routes the suffix)'
);
...
select is(
  ...
  'spellingbee_compete',
  'compete: common.games.gametype = spellingbee_compete'
);
...
-- ============================================================
-- (12) Auth: dee (outsider) cannot create a spellingbee game
-- ============================================================
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"required_band": 0}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
...
select isnt(
      ...
      pg_temp.spellingbee_setup() || '{"required_band": 1}'::jsonb,
      array['ada11111-1111-1111-1111-111111111111'::uuid,
            'bea22222-2222-2222-2222-222222222222'::uuid],
      'coop',
      pg_temp.spellingbee_board()
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"required_band": 7}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
  '{"type":"not-ok","severity":"fault","field":"_","dbcode":"PN160",
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"required_band": 4, "legal_band": 3}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"required_band": 2, "legal_band": 7}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
  '{"type":"not-ok","severity":"fault","field":"_","dbcode":"PN161"}'::jsonb,
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"required_band": "three"}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
...
select pg_temp.envelope_is(
  spellingbee.create_game((select handle from club),
    pg_temp.spellingbee_setup() || '{"legal_band": "5.5"}'::jsonb,
    array['ada11111-1111-1111-1111-111111111111'::uuid],
    'coop',
    pg_temp.spellingbee_board()),
...
select isnt(
      ...
      pg_temp.spellingbee_setup() || '{"required_band": 4, "legal_band": 6}'::jsonb,
      array['ada11111-1111-1111-1111-111111111111'::uuid,
            'bea22222-2222-2222-2222-222222222222'::uuid],
      'coop',
      pg_temp.spellingbee_board()
...
-- ============================================================
-- Defensive: the routing is `'spellingbee_' || mode`. Verify both
-- strings land on common.gametypes (cross-checked in
-- clubs_gametypes_test, but also here so a spellingbee-only regression
-- doesn't slip past).
...
select is(
  (
    select array_agg(gametype order by gametype)
     ...
     where gametype like 'spellingbee%'
  ),
  array['spellingbee_compete', 'spellingbee_coop'],
  'common.gametypes carries both spellingbee_coop + spellingbee_compete'
);
