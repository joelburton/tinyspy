...
--   1. a clue — `took_turn` false; its word and count, nothing else;
--   2. a guess — `took_turn` true exactly when it ends the turn: a bystander in
--      ordinary play. An agent, and any guess in sudden death, spend nothing;
...
select is(
  ...
  '{"took_turn":false,"clue_word":"TOOLS","clue_count":2,
    "guess_position":null,"guess_result":null}'::jsonb,
  'a clue is logged with its word and count, and takes no turn'
);
...
select is(
  ...
  jsonb_build_object('took_turn', false, 'clue_word', null, 'clue_count', null,
    'guess_position', (select a_agent from cells), 'guess_result', 'G'),
...
select is(
  ...
  '{"took_turn":false,"clue_word":null,"clue_count":null,
    "guess_position":null,"guess_result":null}'::jsonb,
...
select is(
  ...
  '{"took_turn":true,"clue_word":null,"clue_count":null,
    "guess_position":null,"guess_result":null}'::jsonb,
  'a pass is logged against the turn it ended, and takes a turn'
);

-- ─── Turn 2: bea clues, ada turns over a bystander ───
select submit_clue((select id from g1), 'HAMMER', 1);
select pg_temp.as_user('ada11111-1111-1111-1111-111111111111');
...
select is(
  ...
  jsonb_build_object('took_turn', true, 'clue_word', null, 'clue_count', null,
    'guess_position', (select b_bystander from cells), 'guess_result', 'N'),
...
select throws_ok(
  format($$ insert into codenamesduet.events
            ...
            values (%L, 'ada11111-1111-1111-1111-111111111111', 'clue', false, 1, 'A', 'AGAIN', 1) $$,
         (select id from g1)),
  '23505', null,
...
select throws_ok(
  format($$ insert into codenamesduet.events
              (game_id, user_id, kind, took_turn, turn_number, seat, clue_word, clue_count, guess_position)
            values (%L, 'ada11111-1111-1111-1111-111111111111', 'clue', false, 9, 'A', 'WORD', 1, 3) $$,
         (select id from g1)),
  '23514', null,
...
-- ─── Sudden death: nothing spends the budget ───
select pg_temp.as_user('ada11111-1111-1111-1111-111111111111');
...
-- ada guesses, judged on bea's key: an agent, then a bystander that loses.
select pg_temp.as_user('ada11111-1111-1111-1111-111111111111');
...
select is(
  (select array_agg(guess_result || ':' || took_turn order by id) from codenamesduet.events
    where game_id = (select id from g2)),
  array['G:false', 'N:false'],
  'in sudden death neither an agent nor the losing bystander takes a turn'
);
