...
select is(
  ...
  16::bigint,
  'common.gametypes contains sixteen rows (tinyspy + 2 psychicnum + 2 connections + 2 freebee + monkeygram + 2 waffle + 2 wordle + 2 stackdown + 2 scrabble)'
);

select is(
  (select array_agg(gametype order by gametype) from common.gametypes),
  array['connections_compete','connections_coop','freebee_compete','freebee_coop','monkeygram','psychicnum_compete','psychicnum_coop','scrabble_compete','scrabble_coop','stackdown_compete','stackdown_coop','tinyspy','waffle_compete','waffle_coop','wordle_compete','wordle_coop'],
...
select is(
  ...
  array['connections_coop','freebee_coop','monkeygram','psychicnum_coop','scrabble_coop','stackdown_coop','waffle_coop','wordle_coop'],
...
select lives_ok(
  $$ select common.set_club_gametypes(
       ...
       array['tinyspy', 'connections_coop', 'freebee_coop']) $$,
...
select is(
  (select array_agg(gametype order by gametype)
     from common.clubs_gametypes
    where club_handle = (select handle from club)),
  array['connections_coop', 'freebee_coop', 'tinyspy'],
