...
select pg_temp.create_club('Psychic rp', array['ada','bea']) as handle;
...
select (psychicnum.create_game(
  (select handle from club),
  jsonb_build_object('max_guesses', 3, 'word_count', 8, 'band', 3,
...
-- PN253 is common.require_game_player's refusal.
