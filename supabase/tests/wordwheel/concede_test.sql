...
-- the compete-only mode guard, and that the wrapper delegates (marks
-- the caller conceded, keeps the game going while others race, and —
-- via common.concede — ends it as a collective loss when the last racer
-- drops out). The full common.concede matrix is in common/concede_test.sql.
-- ============================================================
...
select common.create_club('Wheel concede', array['ada', 'bea', 'cade']) as handle;
...
select throws_ok(
  format($$ select wordwheel.concede(%L) $$, (select id from gc)),
  'P0001',
