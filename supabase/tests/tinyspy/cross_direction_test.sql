...
--   5. Every guess is logged to tinyspy.guesses (a word can appear twice).
-- ============================================================
...
from tinyspy.games g where g.id = (select id from g1);
...
select set_config('request.jwt.claims', '', true);
select is(
  (select neutral_a and neutral_b from tinyspy.words
...
select is(
  (select count(*)::int from tinyspy.guesses where game_id = (select id from g1)),
  4,
