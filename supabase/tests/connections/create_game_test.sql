...
--
-- Setup payloads use `pg_temp.connections_setup(puzzle_id, timer?)`
...
select lives_ok(
  format(
    $$ select connections.create_game(%L, pg_temp.connections_setup(%L::uuid, '{"kind":"countup"}'::jsonb), array['ada11111-1111-1111-1111-111111111111'::uuid, 'bea22222-2222-2222-2222-222222222222'::uuid], 'coop') $$,
    (select handle from club), (select id from puzzle)
  ),
...
select is(
  (select default_setup->'timer'->>'kind' from common.clubs_gametypes
    where club_handle = (select handle from club) and gametype = 'connections_coop'),
  'countdown',
  'saved defaults: connections saves the full timer shape (10-min countdown from the happy-path call)'
);
...
-- The partial unique index on (club_handle) where is_current_view = true
