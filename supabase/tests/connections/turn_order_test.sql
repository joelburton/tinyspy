...
-- (4) A DUPLICATE tile set does NOT advance. It's bea's turn; bea re-submits
-- ada's exact tiles → PN301, a race. Nothing is written, so the pointer stays
-- on bea: only an ACCEPTED move advances it.
select pg_temp.as_user('bea22222-2222-2222-2222-222222222222');
select pg_temp.envelope_is(
  connections.submit_guess((select id from g),
                           array['ALPHA','ANGEL','APPLE','ARROW']::text[], 'wrong', null),
  ...
  'turns: a duplicate tile-set is a race'
);
reset role;
select is(
  (select current_turn_user_id from common.games where id = (select id from g)),
