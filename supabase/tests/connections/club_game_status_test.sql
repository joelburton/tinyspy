...
select is(
  (select club_handle from connections.club_game_status
...
select is(
  (select is_terminal from connections.club_game_status
    where game_id = (select id from g)),
  false,
...
-- both connections.games and common.games, both gating on club
...
select * from connections.create_game(
  (select handle from club),
  pg_temp.connections_setup(
    (select id from connections.puzzles where source_id = 'TEST-NULL-DATE')
  ),
  array['ada11111-1111-1111-1111-111111111111'::uuid,
        'bea22222-2222-2222-2222-222222222222'::uuid], 'coop'
);
