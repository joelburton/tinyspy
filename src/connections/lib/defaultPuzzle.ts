 ...
 *
 * @param puzzles        date-descending {id, puzzle_date} rows (non-NULL dates)
...
export function resolveDefaultPuzzle(
  puzzles: ReadonlyArray<{ id: string; puzzle_date: string }>,
  finishedDates: ReadonlySet<string>,
  savedPuzzleId: string,
): string | null {
  if (puzzles.length === 0) return null

  const savedDate = puzzles.find((p) => p.id === savedPuzzleId)?.puzzle_date
  const baseDate = savedDate ?? puzzles[0].puzzle_date
  ...
  if (finishedDates.has(baseDate)) {
    ...
    const later = puzzles.filter((p) => p.puzzle_date > baseDate)
    const nextDay = later[later.length - 1]
    if (nextDay) pickDate = nextDay.puzzle_date
  }

  return puzzles.find((p) => p.puzzle_date === pickDate)?.id ?? null
}
