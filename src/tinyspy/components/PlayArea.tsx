...
/**
 * Tinyspy's play surface — the seat header, clue panel, 5×5 board,
 * game log. Cross-cutting chrome (title, timer when one lands,
 * Pause, Back-to-club, chat) lives on the common `<GamePage>`.
 *
 ...
 * gives a clue or makes a guess on their machine, our view updates
...
export function PlayArea({ session, gameId }: Props) {
  ...
  return (
    <GamePage gameId={gameId} session={session} gametype="tinyspy">
      {() => {
        ...
        // Phase derivation: a turn is in "guess phase" iff a clue
        // already exists for games.turn_number. The submit_clue RPC
        // enforces the one-per-turn unique constraint, so we can
        // trust this at the client level.
        const currentTurnClue =
          clues.find((c) => c.turn_number === game.turn_number) ?? null

        // derivePhase is pure and unit-tested in
        // src/lib/phase.test.ts — see there for the full
        // clickability / phase matrix.
          ...
          <div className={cls(styles.boardWrap, inSuddenDeath && styles.suddenDeath)}>
            ...
            <div className={styles.boardGrid}>
              {words.map((w) => {
                ...
                // Post-game we render two stripes per cell (A's
                // view, B's view) so you can review what each card
                // "actually was" on both keys.
                ...
                // For the post-game stripes, we want A's label on
                // top and B's on bottom regardless of who's looking.
                // So we re-derive each from whichever of my/peer
                // happens to belong to that seat.
