...
describe('historySnapshot', () => {
  it('shows the board that row recorded, not the live one', () => {
    ...
    expect(historySnapshot(rows, 0)!.board).toEqual([1, 2, 3, 4, 5, 6])
    expect(historySnapshot(rows, 1)!.board).toEqual([70, 71, 72, 4, 5, 6])
  })

  it('rings the cards that turn was about', () => {
    expect(historySnapshot(rows, 0)!.historyLitCards).toEqual([5])
    expect(historySnapshot(rows, 1)!.historyLitCards).toEqual([1, 2, 3])
  })
  ...
  it('returns null for a turn that is not there', () => {
    expect(historySnapshot(rows, 9)).toBeNull()
    expect(historySnapshot([], 0)).toBeNull()
  })
})
