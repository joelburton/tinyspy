...
describe('historySnapshot', () => {
  it('shows the board that row recorded, not the live one', () => {
    ...
    expect(historySnapshot(rows, 11)!.board).toEqual([1, 2, 3, 4, 5, 6])
    expect(historySnapshot(rows, 12)!.board).toEqual([70, 71, 72, 4, 5, 6])
  })

  it('rings the cards that turn was about', () => {
    expect(historySnapshot(rows, 11)!.historyLitCards).toEqual([5])
    expect(historySnapshot(rows, 12)!.historyLitCards).toEqual([1, 2, 3])
  })

  it('names the turn, and says how far a hint went', () => {
    expect(historySnapshot(rows, 11)!.historyLabel).toBe('Turn 1 — hint (1 of 3)')
    expect(historySnapshot(rows, 12)!.historyLabel).toBe('Turn 2 — set claimed')
  })

  it('returns null for a row that is not there', () => {
    expect(historySnapshot(rows, 99)).toBeNull()
    expect(historySnapshot([], 11)).toBeNull()
  })
})
