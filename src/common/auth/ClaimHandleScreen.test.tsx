...
describe('ClaimHandleScreen', () => {
  ...
  it('offers a sign-out escape so a stranded user is never stuck', async () => {
    // A user landing here on a stale session — the DB reset under them — has
    // no other way off: no page renders behind the needsClaim gate, so the
    // menu that holds Log out everywhere else isn't mounted.
    mockSignOut.mockResolvedValue({ error: null })
    // The escape ends in a HARD redirect to "/". Signing out alone leaves a
    // stale session stuck here, because the auth listener doesn't re-render.
