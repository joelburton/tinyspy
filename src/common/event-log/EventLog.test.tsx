...
/**
 * Tests for <EventLog> — the shared panel every game's log wears.
 *
...
const panel = (n: number) => (
  <EventLog heading="Turns" picker={picker} shown={Array.from({ length: n })}>
    {rows(n)}
  </EventLog>
)
...
const panelWithCount = (entries: number, count: number) => (
  <EventLog heading="Turns" picker={picker} shown={Array.from({ length: entries })} entryCount={count}>
    {rows(entries)}
  </EventLog>
)

describe('EventLog — scrolling to the newest entry', () => {
