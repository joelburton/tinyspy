// cs-audited-buttons

import { IconShare } from '../icons/icons'
import { StandardButton, type PurposeButtonProps } from './StandardButton'

/**
 * Share-a-move button — broadcasts your in-progress (staged) board to your coop
 * teammates, who see it in a read-only preview (scrabble's "show a move"). The
 * A blue outline at secondary weight, NOT the filled primary: it's a helpful
 * side action, not the main move, and it matches the Swap button beside it. **Icon-only** by default (the share glyph reads on its
 * own); `label` is the aria-label + tooltip. Default label "Show move".
 */
export function SharePreviewButton({
  name = 'Show move',
  icon = IconShare,
  tone = 'normal',
  label = null,
  ...rest
}: PurposeButtonProps) {
  return <StandardButton name={name} icon={icon} tone={tone} label={label} {...rest} />
}
