...
/**
 * `<TimerField>`'s behavior, pinned because nothing pinned it.
 *
...
describe('TimerField', () => {
  it('offers the three modes as one radio group', async () => {
    ...
    render(<TimerField value={{ kind: 'none' }} onChange={() => {}} />)
    await open(user)
  ...
  it('keeps the MM:SS box inert until Down is the choice', async () => {
    ...
    const { rerender } = render(<TimerField value={{ kind: 'none' }} onChange={onChange} />)
    await open(user)
    ...
    rerender(<TimerField value={{ kind: 'countdown', seconds: 600 }} onChange={onChange} />)
  ...
  it('reports the other two modes without a duration', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<TimerField value={{ kind: 'countdown', seconds: 600 }} onChange={onChange} />)
    await open(user)
