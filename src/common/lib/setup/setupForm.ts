 ...
 * wrapper, the body renders `values` and signals edits via `set`.
 *
 * `values` and `set` are `unknown`-typed here so `GameManifest`
 * can stay non-generic (the registry holds `GameManifest[]`,
 * which can't carry per-game type parameters). Each game's
 * setup component casts once at the top and is fully typed inside.
 */
