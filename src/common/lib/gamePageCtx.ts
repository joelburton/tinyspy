 ...
 *
 * **Its own module because its readers are its own.** 32 files import it and
 * all 32 are a game's components; exactly one of them also imports
 * `GameManifest`, whose 22 readers are the manifests and the club surfaces that
 * list them. Declaring a game and playing one are different moments with
 * different audiences (plans/areas/game-lib.md → `F-game-lib-12`).
 */
