...
export type LocalFeedbackApi = {
  ...
  /** Show a message. A `timed` message (`mode: { kind: 'timed' }`) auto-clears
   *  after `dismiss.ms` (default `LOCAL_FEEDBACK_DISMISS_MS`), re-arming the timer
   *  on each call; a `sticky` / `manual` / `permanent` message stays until `clearLocalFeedback()`
 ...
 * flash), `manual`, or `permanent`. This is the single mechanism every game uses — it
...
export function useLocalFeedback({ locked = false }: LocalFeedbackOptions = {}): LocalFeedbackApi {
  ...
  const showLocalFeedback = useCallback((msg: GenericFeedbackMsg) => {
    ...
    if (msg.mode.kind === 'timed') {
      const ms = msg.mode.ms ?? LOCAL_FEEDBACK_DISMISS_MS
