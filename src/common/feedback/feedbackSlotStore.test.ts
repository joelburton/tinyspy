 ...
 * to the newest, showing replaces the live message of the same rank, a timer
 * kind retracts itself, `dismiss` and `close` each remove the top only when
 * its kind leaves that way, and `destroy` clears every timer.
 */
...
describe('createFeedbackSlot — which message shows', () => {
  ...
  it('ties within a rank go to the newest', () => {
    ...
    // Same rank → newest wins (and replaces; see below).
    const third = FeedbackMessage.note('third')
    slot.show(third)
    expect(slot.getTop()).toBe(third)
  })

  it('showing a message replaces the live message of the SAME rank', () => {
    const slot = createFeedbackSlot('local')
    slot.show(FeedbackMessage.result('lost', 'Not a word'))
    slot.show(FeedbackMessage.result('won', 'Correct'))
    expect(slot.peek()).toHaveLength(1)
    expect(slot.getTop()?.text).toBe('Correct')
    // …and an acknowledgment, which shares the result's rank, replaces it too.
    slot.show(FeedbackMessage.acknowledgment('neutral', 'Peel!'))
    expect(slot.peek()).toHaveLength(1)
    expect(slot.getTop()?.text).toBe('Peel!')
  })

  it('a different rank does not replace: the hint is still there under "Not a word"', () => {
    const slot = createFeedbackSlot('local')
...
describe('createFeedbackSlot — how a message leaves', () => {
  ...
  it('replacing a timed message cancels its timer, so the old clock cannot kill the new one', () => {
    const slot = createFeedbackSlot('global')
  ...
  it('dismiss() removes the top only when it leaves by gesture', () => {
    ...
    slot.dismiss()
    expect(slot.getTop()?.kind).toBe('standingNote')
  })
