 ...
 * `check`), bunch size, the 3-way word check with its two difficulty
 * bands (`word_check` / `dict_2` / `dict_3plus`), where a dumped tile
 * goes (`dump_to_bag`), and the shared `timer` mode. A countdown that
...
export type BananagramsSetup = {
  ...
  hand_size: 15 | 21
  /** How many tiles the bunch holds for this game, 1..144. The full
   *  Bananagrams set is 144; a smaller bunch (a random subset) makes a
   *  shorter game. MUST be ≥ `playerCount × hand_size` so the deal is
   *  possible — `bunchSizeError` enforces it in the dialog, and
  ...
  dict_3plus: number
  /** Where a dumped tile goes. `false` (default) = back into the bunch (it may
   *  be drawn again). `true` = to the out-of-play "bag", so the bunch depletes
   *  (the game ends sooner) — though a dump can top up from the bag when the
