...
describe('hintShortfallText', () => {
  ...
  it('is singular at one word to go', () => {
    expect(hintShortfallText(1)).toBe('1 word needed for a hint')
  })

  it('is plural above one', () => {
    expect(hintShortfallText(2)).toBe('2 words needed for a hint')
    expect(hintShortfallText(3)).toBe('3 words needed for a hint')
  })
})
