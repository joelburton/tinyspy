...
describe('historySnapshot', () => {
  it('is a filter: the board at turn N is the theme finds among rows 0..N', () => {
    expect(historySnapshot(ROWS, 11).found).toEqual([
      { path: [[0, 0], [0, 1]], isSpangram: false },
    ])
    // Rejects and hint words never reach the board.
    expect(historySnapshot(ROWS, 13).found).toEqual(historySnapshot(ROWS, 11).found)
  })

  it('the boundary is INCLUSIVE: viewing a find shows that find placed', () => {
    expect(historySnapshot(ROWS, 14).found).toHaveLength(2)
    expect(historySnapshot(ROWS, 14).found[1]).toEqual({
      path: [[3, 0], [3, 1]],
      isSpangram: true,
    })
  })
  ...
  it('lights the viewed turn even when it changed nothing', () => {
    ...
    expect(historySnapshot(ROWS, 12).historyLitTiles).toEqual([[1, 0], [1, 1]])
    expect(historySnapshot(ROWS, 12).found).toHaveLength(1)
  })

  it('describes the turn in the log wording, 1-indexed', () => {
    expect(historySnapshot(ROWS, 11).historyLabel).toBe('#1 APPLE — theme word')
    expect(historySnapshot(ROWS, 12).historyLabel).toBe('#2 ZQ — too short')
    expect(historySnapshot(ROWS, 14).historyLabel).toBe('#4 SPANNER — spangram')
    expect(historySnapshot(ROWS, 15).historyLabel).toBe('#5 APPLE — already found')
  })

  describe('a spent hint', () => {
    it('re-rings its revealed cells as a HINT, not as a traced route', () => {
      const snap = historySnapshot(ROWS, 16)
    ...
    it('names the act without naming the word', () => {
      expect(historySnapshot(ROWS, 16).historyLabel).toBe('#6 Hint — a word was revealed')
    })
    ...
    it('leaves the board exactly as the finds before it left it', () => {
      ...
      expect(historySnapshot(ROWS, 16).found).toEqual(historySnapshot(ROWS, 15).found)
    })

    it('carries no hintCoords on a guess turn', () => {
      expect(historySnapshot(ROWS, 11).hintCoords).toBeNull()
    })
  })
