...
describe('customLettersError', () => {
  ...
  it('requires BOTH fields when either is filled', () => {
    expect(customLettersError({ ...base, custom_center: 'e' })).toEqual(onLetters(/both|eight other/i))
    expect(customLettersError({ ...base, custom_letters: 'abcdfghi' })).toEqual(onLetters(/both|center/i))
  })

  it('rejects a multi-character center', () => {
    expect(customLettersError({ ...base, custom_center: 'ab', custom_letters: 'cdfghijk' })).toEqual(onLetters(/center must be a single letter/i))
  })

  it('rejects the wrong count of other letters', () => {
    expect(customLettersError({ ...base, custom_center: 'e', custom_letters: 'abcd' })).toEqual(onLetters(/eight other letters/i))
  })

  it("allows the letter 's' in either field (word wheel spends a tile per use)", () => {
    // Unlike spellingbee, 's' is an ordinary letter here.
    expect(customLettersError({ ...base, custom_center: 's', custom_letters: 'abcdfghi' })).toEqual({})
    expect(customLettersError({ ...base, custom_center: 'e', custom_letters: 'abcdfghs' })).toEqual({})
  })
  ...
  it('accepts repeated letters (the wheel is a multiset)', () => {
    ...
    expect(customLettersError({ ...base, custom_center: 'a', custom_letters: 'abcdfghi' })).toEqual({})
    // 'a' repeated in the outer letters → two outer a-tiles.
    expect(customLettersError({ ...base, custom_center: 'e', custom_letters: 'aabcdfgh' })).toEqual({})
  })
})
