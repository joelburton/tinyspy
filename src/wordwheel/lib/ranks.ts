/**
 * wordwheel rank ladder — TypeScript port of
 * `~/wordwheel-ws/shared/ranks.js`.
 *
 ...
 *     `wordwheel.submit_word` (Phase 2 RPC). The RPC's
 *     `wordwheel._rank_idx` computes the same number via integer
 ...
 *  integer win-check in `wordwheel._rank_idx` and the bar fill in
