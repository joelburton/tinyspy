...
/**
 * The 5×5 codenamesduet board. Owns its own per-tile render logic and
...
export function BoardGrid({
  ...
  return (
    <>
      ...
      {/* .board wrapper + .grid mirror psychicnum/connections (the shared
          "board" shape — the single place a future framed board would live);
          the tiles themselves are the shared `.tile`/`.tileWord` chrome, with
          the keycard/triangle/pending OVERLAYS layered on via `.overlayTile`
          (which makes the tile a positioning context for them). */}
