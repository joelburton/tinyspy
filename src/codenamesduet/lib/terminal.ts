 ...
 *  the shared `TerminalMessage` shape: `pillText` +
...
export function buildTerminalMessage({
  ...
  playState: string
  // WHY it ended — `common.games.status.reason`, written by the RPC that
  // ended the game: `assassin` and `turns` (submit_guess), `timeout`
  // (submit_timeout). The club-list label reads the same column.
