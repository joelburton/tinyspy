...
/**
 * The one-character role a cell occupies on a key card:
 *   - 'G' green agent (counts toward the 15 to find)
 *   - 'N' neutral / bystander
 *   - 'A' assassin (game over if revealed)
 *
 * Each key card (one per seat) is a length-25 array of these letters, indexed
 * by board position 0..24. The two cards are stored as the `key_card_a` and
