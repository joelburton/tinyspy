...
**Don't reveal the solution on a loss.** Eight games keep their answer covered when you lose (and on a manual end), opening it only on a clean win or when someone presses Reveal: **waffle** (the grid), **wordle** (the word), **stackdown** (the six words), **psychicnum** (the three secrets), **crosswords** (the author's grid), **codenamesduet** (the partner's key card), **strands** (where the words hide), and **letterboxed** (the seeded two-word solution). Force-reveal at loss time and the replay becomes theater — you already know where the tree ends, and `replay_board` re-runs the *same* board in every one of these games. Under the friends-only trust model nothing needs re-shielding — the server already unshields at terminal, so "may they see it?" is purely a **display** question. It has exactly one answer, in one place: **`common.games.solution_revealed`**.
