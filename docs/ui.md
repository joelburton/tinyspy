...
**Game-end UI** — the info column's row is `game-page/InfoActionsRow` in every state, terminal included: the game's actions, with `over`'s outcome line beside them once there is one. `common/terminal/` holds the words (`terminalMessage`) and the celebration. The verdict itself is in-page — the below-board pill — and the one modal in play is `<CelebrationBlockingModal>`, popped only at the moment of a win. `<GamePage>` binds `act-back-to-club`, which the row places as an `<ActionButton>`. See [Terminal results](#terminal-results--the-moment-vs-the-record) above for the full contract.
