...
**Which win counts is per-game.** Most games celebrate the coop win (`playState === 'won'`; coop-only by the states vocabulary, since compete writes `won_compete`). Scrabble and bananagrams celebrate the **compete** win instead — they can afford the self-vs-other test on first render, and bananagrams' coop has no win at all. Thirteen of fourteen games celebrate; wordiply has no win state to celebrate (its verdict leads with `Ended:`).
...
**Don't reveal the solution on a loss.** Seven games keep their answer covered when you lose (and on a manual end), opening it only on a clean win or when someone presses Reveal: **waffle** (the grid), **wordle** (the word), **stackdown** (the six words), **psychicnum** (the three secrets), **crosswords** (the author's grid), **codenamesduet** (the partner's key card), and **strands** (where the words hide). Force-reveal at loss time and the replay becomes theater — you already know where the tree ends, and `replay_board` re-runs the *same* board in every one of these games. Under the friends-only trust model nothing needs re-shielding — the server already unshields at terminal, so "may they see it?" is purely a **display** question. It has exactly one answer, in one place: **`common.games.solution_revealed`**.

- **`common.end_game`** sets it on any win (you produced the solution to get there) and at any ending of a gametype that never hides its answer — `common.gametypes.hides_solution`, a registry fact rather than a per-ending decision, so the word-find trio / connections / wordiply end revealed without anyone remembering to say so. Their FEs read the flag too, so "should they see it?" reads the same everywhere rather than living in per-game display logic.
- **`common.reveal_solution(target_game)`** is the explicit control, and **the only one** — there is no mid-game "give up and show me". Every game has the same two-step order: End the game (which ends it for everyone), then Reveal. (waffle and wordle used to offer a combined mid-game give-up; removed 2026-08-03.) **Terminal-only, server-side**: "ended for everyone" is `is_terminal`, so a compete player who conceded or ran out *cannot* reveal while the others race — no per-game "am I locally done?" reasoning anywhere in the path. Their row still shows the control, disabled, tooltipped **"Can't reveal until all end"**: the row must not change shape when the last racer finishes, and "not yet" beats a control that vanished. Any game player may fire it; idempotent.
- **`common.reset_game`** clears it, so a replay of the same board starts blind on every client. That's the payoff of a common column over per-game state: waffle and wordle each had to remember to clear a local flag on Restart, and the two games that got the gate next promptly forgot to.
- **It's shared.** One player reveals and the group's boards open together, arriving on the same realtime refetch as the rest of the row — a post-mortem is something the friends do on the call, not five private decisions.

Games with no solution (bananagrams, scrabble) simply never set it, the way `current_turn_user_id` is inert for the free-for-all games.

**Every gated game offers the reveal twice** — a `RevealButton` in the terminal action row *and* a game-menu item — so a player who's scrolled past the row, or who is on the mobile layout where the info column is off-canvas, can still get to it. The menu item is disabled until terminal and disables itself once the solution is showing.

**Two rules decide the details.** First, *auto-show on a clean win only when the board **cannot** differ from the solution.* waffle / wordle / stackdown / psychicnum have one answer, so winning means you've already seen it. **crosswords is the deliberate exception:** rebuses and quantum clues mean the players' grid can legitimately differ from the author's, so it never overwrites what they entered — Reveal is opt-in there even after a win. Don't "correct" that into consistency. Second, *the club-list title follows the same gate* — a title that spells the answer would spoil the game from the outside, which is exactly what wordle's `_sync_title` did until 2026-08-02. Every game's title-writer now names only what the players actually earned (stackdown: the words they cleared; waffle: the best board's correct words; psychicnum + codenamesduet: board words, never the key).
...
**All fourteen games have it** (2026-08-03). The three that didn't were each opted out for a good local reason, and each reason lost to the same argument — a player who can't find Restart where every other game puts it concludes the app is broken, not that this game is special:
...
fourteen games are v3**, with bananagrams and crosswords the two documented layout
...
**Rollout.** Complete — **all fourteen games are v3**, so every game-move / end /
