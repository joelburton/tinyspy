...
**Which win counts is per-game.** Most games celebrate the coop win (`playState === 'won'`; coop-only by the states vocabulary, since compete writes `won_compete`). Scrabble and bananagrams celebrate the **compete** win instead — they can afford the self-vs-other test on first render, and bananagrams' coop has no win at all. Twelve of thirteen games celebrate; wordiply has no win state to celebrate (its verdict leads with `Ended:`).
...
**Don't reveal the solution on a loss.** Six games keep their answer covered when you lose (and on a manual end), opening it only on a clean win or when someone presses Reveal: **waffle** (the grid), **wordle** (the word), **stackdown** (the six words), **psychicnum** (the three secrets), **crosswords** (the author's grid), and **codenamesduet** (the partner's key card). Force-reveal at loss time and the replay becomes theater — you already know where the tree ends, and `replay_board` re-runs the *same* board in every one of these games. Under the friends-only trust model nothing needs re-shielding — the server already unshields at terminal, so "may they see it?" is purely a **display** question. It has exactly one answer, in one place: **`common.games.solution_revealed`**.
