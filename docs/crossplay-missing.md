...
| **NYT overlay-PNG analysis** | `nyt.ts` decodes theme circles-on-shaded + author word-break bars from the raster overlay PNG on themed puzzles. | ✅ **DONE** — pure `nytOverlay.ts` detector (pinned to real NYT overlay fixtures) + `npm:pngjs` decode in the edge fn. Both circles **and** bars render on the board + both PDFs (bars via `create_game` seeding template marks into the live cells — marks are a live-cell concept here). | crosswords.md §5 |
