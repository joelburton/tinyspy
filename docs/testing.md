...
- **a subscription that names the wrong table** — `events-realtime.e2e.ts`, one test per game: the page is opened and then a row is written from Node through that game's own RPC, so only the postgres-changes event can make the log's `#N` appear. A `{ schema, table, filter }` typo fails silently — no error, just a game that stops filling — and nothing that mocks the client can see it.
