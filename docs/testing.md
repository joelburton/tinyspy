...
- **a subscription that names the wrong table** — `events-realtime.e2e.ts`, one test per game: the page is opened and then a row is written from Node through that game's own RPC, and the log's `#N` has to appear without the page doing anything. What that proves is a LIVE BINDING on the named table: a `{ schema, table, filter }` typo makes the channel join fail, so neither the event nor the attach-confirmation refetch behind it ever arrives and the row never shows. It does not separate the two — `useRealtimeRefetch` refetches on the attach as well — and it doesn't need to. The typo fails silently otherwise — no error, just a game that stops filling — and nothing that mocks the client can see it.
- **the turn-history overlay** — the `*-history.e2e.ts` specs: clicking an event-log `#N` replays it on the board with the frame/banner, and the shared exit paths (key / board click / any click) work. Overlay + no-reflow properties jsdom can't see.
...
desktop/mobile/PDF is hundreds of states — more than anyone opens by hand, so cross-game
