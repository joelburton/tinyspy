...
- `bananagrams.concede(target_game)` — **a player drops out of the race** (replaces the old whole-table `end_game`). bananagrams is compete, so conceding is a **real loss** for the conceder — but unlike a whole-game stop it marks JUST the caller out (`progress.conceded = true`) and the **others keep racing** (`peel` already counts/deals only active players). `require_game_player`; rejects a non-`playing` game (`game is not in progress`) and a double-concede (`you have already conceded`) with `P0001`. After marking the caller out it checks who's left: **≥ 1 active** → the game stays `'playing'` (the conceder just watches the terminal-look locally); **0 active** (the LAST racer conceded, including a solo `N = 1` game) → the whole game ends as a **collective loss** the same shape as the timeout — `play_state='lost'`, `status={outcome:'conceded'}` (NO `winner_username`), every player `{"won": false}`. The conceder is recorded a loss automatically when the game finally ends (a peel-win / timeout / all-conceded terminal marks every non-winner `{"won": false}`) — no mid-game result write. The `progress` write IS the realtime nudge for the `useProgress` channel; the all-conceded terminal's `common.end_game` wakes the modal. Locks the gametype row (serializes against a concurrent peel, so a last-concede and a peel-win can't both write terminal). pgTAP: `concede_test.sql`. `save_player_board` also no-ops for a conceded caller (their board is frozen).
