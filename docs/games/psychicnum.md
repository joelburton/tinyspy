...
### `psychicnum.submit_guess(target_game uuid, guess text) → jsonb`

The only mid-game guess action. The guess must be one of the board words (compared case-folded — the player clicks a tile or types a board word). There are three secrets; players win by finding all three, so a correct guess does not end the game by itself — only the one that completes the set does. It answers an envelope — `won` for a hit, `lost` for a miss, `warning` for a word already guessed — with `{ verdict, found_all }` in `data` (the table under "What it answers" below).

**The answer is the caller's verdict on their own guess, never the game's fate.** The budget-exhausting guess answers by what the guess did — a hit that empties the budget is still `won` — and every way the game ends (the exhausted budget, timeout, concede, a compete opponent finishing) reaches the FE by realtime, so the pill never flashes a loss over a correct guess.
...
The FE side: `src/psychicnum/lib/setup.ts` (the `PsychicnumSetup` type) and `src/psychicnum/components/SetupForm.tsx` (the form body, lazy-loaded inside the common `SetupGameModal`). The server is the canonical authority for what shapes are accepted — the TypeScript narrowing is advisory.
...
src/psychicnum/
  ...
  components/
    PlayArea.tsx          Two-column composition on the SHARED PlayArea scaffold
                          ...
                          PlayArea.module.css now holds only .inputMessage):
                            ...
                            <WordEntryArea> (the shared capture-entry control: icon Delete +
                              WordEntryInput + icon Submit + keyboard) + submit_guess RPC — during play
    ...
    PlayArea.module.css   (+ `.bigEntry` — psychicnum's one entry tweak: a 2rem font
                          on the shared <WordEntryArea>, since a single guess word reads large)
                          The word entry is the SHARED common/word-entry/WordEntryArea
                          (icon Delete + WordEntryInput + icon Submit + the capture keyboard) —
    ...
    BoardCol.module.css   below-board slot (turn-viewer banner / guess entry, which draws
                          the local feedback slot's top message). Owns the guess dispatch
                          (submit_guess) + the local board shuffle. PlayArea hands it the
                          board to render (live results OR a history snapshot) + `isViewingHistory`.
  ...
  lib/
    ...
    capitalize.ts         Sentence-cases a raw RPC error message for the own-move pill.
                          (The message itself is `FeedbackMessage.result`, shared.)
...
| What does the UI look like | [`src/psychicnum/components/PlayArea.tsx`](../../src/psychicnum/components/PlayArea.tsx) (word entry is the shared [`common/word-entry/WordEntryArea.tsx`](../../src/common/word-entry/WordEntryArea.tsx)) + `GameEventLog.tsx` alongside; the coop-win celebration is the shared `common/terminal/CelebrationBlockingModal.tsx` |
