...
A NYT-Connections-style word-grouping puzzle. The third gametype family in this monorepo (after tinyspy and psychicnum), now registered as a sibling-manifest pair — `wordknit_coop` + `wordknit_compete`. WordKnit was the first place in this codebase to introduce several patterns: FE-evaluated rules, shared selection state via Supabase Realtime Broadcast (coop only), and the "pause the game when a peer disconnects" pattern.
...
WordKnit exports two manifest entries from one folder, mirroring psychicnum's split:

| field                | `wordknitCoopGame`     | `wordknitCompeteGame`     |
|----------------------|------------------------|----------------------------|
| `gametype`           | `wordknit_coop`         | `wordknit_compete`          |
...
Unlike tinyspy and PsychicNum — where the server holds a secret and validates moves against it — wordknit's board (categories + tile order) is **publicly readable** by every club member, in both coop and compete modes. The FE has the answer key. The `submit_guess` RPC trusts the FE's verdict (correct / oneAway / wrong + `matched_category_rank`) and just records it, applying atomicity for the shared state (per-player `mistake_count` on `wordknit.players`, mode-aware one-correct-per-rank idempotency via partial unique indexes on `guesses`).
...
`common.games.play_state` carries wordknit's lifecycle enum, mode-aware:
...
|--------------------------------------------|-------------------------------------------------------------|----------------------------------------------------------------------|
| **gametype string**                        | `'wordknit_coop'`                                           | `'wordknit_compete'`                                                 |
| **`wordknit.games.mode`**                  | `'coop'`                                                    | `'compete'`                                                          |
| **manifest `numberOfPlayers`**             | `[1, 6]` (solo OK)                                          | `[2, 6]` (needs ≥1 opponent)                                         |
| **`wordknit.players.mistake_count` per row**| Always equal across rows (lock-step decrement)             | Independent per row (only the guesser's row increments)              |
| **`wordknit.guesses` RLS**                 | Club-wide visible                                           | Caller-only — `using (... and guesses.user_id = auth.uid())`         |
| **`wordknit.players` RLS**                 | Club-wide visible                                           | Club-wide visible (same — opponents see each other's mistake counts) |
...
- The `wordknit.games` table (modulo the `mode` value).
- The `wordknit.players` table structure (one row per player; only the update mechanics differ).
- The `wordknit.guesses` table rows (modulo the `mode` denorm + RLS).
...
### `wordknit.create_game(target_club text, setup jsonb, player_user_ids uuid[], mode text) → table(id uuid)`

The one entry point. **One RPC for both modes** — the `mode` parameter:

- Routes the gametype string to `'wordknit_coop'` or `'wordknit_compete'` on `common.games.gametype`.
- Lands on `wordknit.games.mode` for RLS branching.
...
Verifies caller is a club member, validates `setup.puzzleId` (must be a uuid that exists in `wordknit.puzzles`) and `setup.timer` shape (see [Timer](#timer-browser-side-no-server-sync)), loads the puzzle's categories, shuffles the 16 tiles into `board.tileOrder`, builds the title as `"#<source_id> <nyt_date> (<TILE1>/<TILE2>)"` where TILE1/TILE2 are the first 2 alphabetical tiles across all 16. Calls `common.create_game(target_club, 'wordknit_<mode>', player_user_ids, title, setup, setup)` which inserts the `common.games` header (`is_current_view=true`, `play_state='playing'`, with `setup` persisted on `common.games.setup`), then inserts the `wordknit.games` row with `mode` + `puzzle_id`, then inserts one `wordknit.players` row per player_user_ids entry with `mistake_count=0`. The board is a copy of the puzzle's categories + this game's shuffled tileOrder; the puzzle row stays pristine.
...
The only mid-game action. Validates the payload shape (4 tiles, valid result enum, rank present iff correct), then branches on `wordknit.games.mode`. `SELECT FOR UPDATE` on the gametype row serializes concurrent submissions in both modes.
...
- `wrong` / `oneAway` → insert row; UPDATE every `wordknit.players` row `mistake_count = mistake_count + 1`; if hits 4 → `play_state='lost'`, all players `{won: false}`.

**Compete branch:**
- Caller eliminated check (`wordknit.players.mistake_count >= 4` for caller) raises P0001 'you are eliminated from this game'.
...
- `wrong` / `oneAway` → insert row; UPDATE only the caller's `wordknit.players` row; check `MIN(mistake_count)` across all players ≥ 4 → `play_state='lost_compete'`, all `{won: false}`. Caller-just-eliminated-but-others-alive lets the game continue.
...
Idempotent — a second concurrent call on the already-terminal game raises `P0001 "game is not in progress"`, which the FE swallows. See [Timer](#timer-browser-side-no-server-sync).
...
- **`games`** + **`players`** are club-wide visible: `using (common.is_club_member(club_handle))` (games) / EXISTS via `wordknit.games` join (players). Every club member sees every player's mistake_count in both modes — that's the "opponents see remaining mistakes but not guesses" property.
...
- **`guesses`** is mode-aware, mirroring `psychicnum.guesses_select`:
  ...
  Coop: any club member sees any guess. Compete: club members only see their own guesses. The `guesses.mode` denorm is what lets the policy branch without joining to `wordknit.games` on every visibility check.
...
```
src/wordknit/
  manifest.ts             TWO GameManifest entries (wordknitCoopGame + wordknitCompeteGame)
  ...
  hooks/
    useGame.ts            Owns postgres-changes (games / guesses / players) on a stable
                          per-game channel `wordknit:${gameId}` (stable because coop's
...
Two channels — one common-side, one wordknit-side — both stable-named because each carries broadcasts that need to merge across peer tabs:
...
WordKnit is the first place in this codebase that uses Realtime Broadcast and Presence (everything else uses only Postgres Changes). The pattern is worth documenting because it'll repeat for future coop games with transient shared state.
...
| What does the tile grid look like | [`src/wordknit/components/TileGrid.tsx`](../../src/wordknit/components/TileGrid.tsx) (per-tile self/peer attribution; peer-frame degenerate-unused in compete) |
...
| How shared selection works | [`src/wordknit/hooks/useGame.ts`](../../src/wordknit/hooks/useGame.ts) (the `apply` callbacks + `toggleTile` + selection-events broadcast; `broadcast()` short-circuits to local-only in compete) |
