...
- **`lib/history.ts`** — wordle's snapshot semantics for the shared turn-history viewer (mechanics in [playarea.md → Turn-history viewer](../playarea.md#turn-history-viewer)): the replay is pure + unit-tested, ADD-style like psychicnum (a guess only ADDS a colored row, so a past board is simply the first N guess rows — no fold), boundary **inclusive** (viewing turn N shows the board after N landed, N's row ringed history-yellow). Keyed by **log position** (= the displayed board row, 0-based; the viewer only ever replays the board you're looking at, where log position and board row line up 1:1). The below-board slot is the shared feedback system (`useFeedbackSlot('local')`, docs/ui.md → Feedback pill): BoardCol shows the soft rejects into it as `FeedbackMessage.result`, and PlayArea's three standing conditions — the verdict, out of the race, whose turn — are effects on it.
