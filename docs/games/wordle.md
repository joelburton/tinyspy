...
- **SetupForm** (the shared `CoopStyleField` first — the opt-in turn-by-turn coop pacing + its first-turn picker, self-gating to nothing for compete / solo; then the guesses dropdown + two shared `DifficultyField`s — answer source with a "0: Wordle" option, and legal-guess band whose floor follows the answer; the manifest's `validate` gates Start until legal ≥ answer — + `TimerField`), **Help**.
