...
- **GameTurnLog** — the guess history as the shared `<TurnLog>` table: one `<tr>` per guess (outcome bar `neutral`, green on the solving guess; `#n`; the guess as five colored letter-squares in the `.main` column; the guesser's `<ActorTag>` in the right-aligned `.who` column). Its header carries a small understated **"whose guesses" dropdown** (the shared `<TurnLog headerAction>` slot): coop with 2+ players is one "Team"; every other case lists the players (viewer first + default + "You" when they're playing; a spectating club member sees the player's name instead) — the "see opponents' boards" affordance, with compete opponents RLS-hidden during play (the log reads "Hidden until game ends.") and revealed at terminal.
