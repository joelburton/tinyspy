...
- **`replay_board(game)`** — restart the SAME game — same word — from scratch for everyone. Two FE entry points, one handler (the waffle arrangement): the **"Replay board"** game-menu item (both modes, any state) and the terminal action row's **`RestartButton`** (left of Back-to-Club). Resets `wordle.players` (zeroed, unsolved), clears `wordle.guesses`, then `common.reset_game` (un-terminal, fresh initial status, clear results + concede). The target **re-shields on its own** (`_target_for` gates on `is_terminal`, which the reset clears). The FE confirms mid-game only; at terminal there's nothing to lose. On the FE side the reset must land **entirely blank**: BoardCol drops its in-flight `pending` word + typing buffer when the live rows shrink (rows only ever shrink on a reset), else the finished run's last guess would resurrect as an uncolored top row and block input. pgTAP: `replay_test.sql`.
