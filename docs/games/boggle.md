...
- **Build step:** `npm run boggle:wordlist` queries `common.words` for **all**
  ...
  drops. It reads the **table**, not `~/src/gamelist/words.tsv` (that file is only
...
- **Generated, not committed.** The asset is git- and eslint-ignored, and
  **regenerated by `npm run deploy`** before `supabase functions deploy`, and by
  `gmake deploy-funcs` — which depends on it, always building from the LOCAL stack
  because the hosted `common.words` isn't seeded until several steps later. Run
  it manually (`gmake g-boggle-trie`) before `supabase functions serve` locally.
...
- `gmake deploy-funcs` regenerates the asset **from the local stack**
