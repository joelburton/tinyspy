...
- **Build step:** a script queries the **`common.words` table** for the clean
  required-eligible rows (`american, crude=0, slur=0, slang=0, len 3–<max board
  len>`), selecting `word, difficulty`, all bands present (so any pickable band can
  be built), and writes the bundled asset (~0.85 MB gzip). It reads the **table**,
  **not** `~/src/gamelist/words.tsv` — that file is solely the `common.words`
  importer's input; nothing else should depend on it.
...
2. **Bundled dictionary asset** generator (queries the `common.words` table) +
   `boggle-build-board` edge function (prove cold-start + generation latency
   end-to-end).
