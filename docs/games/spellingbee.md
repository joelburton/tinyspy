...
- **`lost_compete`** — terminal. Compete only: the countdown expired before anyone hit the target (`status.outcome='timeout'`), or every racer conceded (`status.outcome='conceded'` — `common.concede`'s gametype-suffix rule appends `_compete` for a compete sibling, so all-conceded lands here, not on plain `lost`).
...
| `tests/spellingbee/coop_target_test.sql` | The opt-in coop win: team reaches `setup.target_rank` → `play_state='won'` / `status.outcome='target'` / everyone `{won:true}`, and a later `submit_word` is rejected; the clock expiring with a target set + unreached → `'lost'` / `outcome='timeout'`; no target → `'ended'`; manual End with a target unreached → `'ended'` (stopping ≠ losing). |
