...
> for three. Word/board **validation** is opt-in (the "Check if board is legal
> to win" setup option); off by default, we trust players — any placement, even
> gibberish, counts. The throwaway UX prototype in `monkeygram-ui/` (gitignored)
> is where the board feel was settled.
...
- **Word/connectivity validation is opt-in** (setup: "Check if board is legal to win"). Off (default) → the classic trust-the-friends rule: place all your tiles (even scattered, even gibberish) and peel a dry bunch to win. On → the winning peel only counts if your tiles form **one connected grid of real words** (at a chosen dictionary obscurity); otherwise the offending tiles flash **red** until you fix them. Mid-play is never validated — only the peel that would end the game.
...
- `monkeygram._win_blockers(board, max_difficulty) → int[]` — the board validator (plain `language sql`, used by peel only when `check_legal` is on). Returns the 0-indexed cells that block a legal win, or `{}` for a valid grid. Blockers = tiles **not in the main 4-connected mass** (a recursive flood-fill from the top-left-most tile — diagonal touches don't connect) ∪ tiles of any **2+ run that isn't a real word** (`common.words` at `difficulty ≤ max_difficulty`; single tiles aren't words, so never dictionary-checked).
- `monkeygram.peel(target_game) → jsonb` — the draw/endgame, and the game's *win* terminal. `require_game_player`; rejects unless the hand is empty (`placed == length(tiles)`). Then: if the bunch can't refill the whole table (`length(pool) < players × peel_count`), it's a **winning peel** — **but if `setup.check_legal` is on it first runs `_win_blockers`; a non-empty result means the board isn't legal, so it leaves the game in progress and returns `{result: 'illegal', invalid_cells}`** for the FE to paint red. Otherwise the peeler **goes out and wins** (`end_game('won', …)`, returns `{result: 'won'}`). If the bunch *can* refill, **every player draws `peel_count`** from the front of the pool, the pool advances, `status.pool_remaining` updates (`{result: 'dealt'}`). A continuing peel is never validated — you're not winning yet. `peel_count` from setup (default 1). Locks the gametype row up front so concurrent peels serialize; a peel on a non-`playing` game is rejected (`game is not active`).
...
- **SetupForm**: `hand_size` (15 / 21, default 21) + `bag_size` (number, 1–144, default 144 — fewer tiles = a shorter game) + **check_legal** (a checkbox; when on, reveals a **dictionary** picker, 2–6 obscurity bands Common…Expert, default 4) + the shared `TimerField` (none / count-up / countdown MM:SS, default none). A countdown that runs out ends the race as a collective loss (`submit_timeout`). The bag must hold a starter hand per player: the form shows the live "deals N" figure, and `bagSizeError` (shared with the gate) feeds the manifest's `setupForm.validate` so the dialog **disables Start with a reason** until `bag_size ≥ playerCount × hand_size`. Manifest compete-only; solo is N = 1.
...
5. **Polish / future** — hand sort/group, optional elapsed timer. (Word/board validation shipped as the opt-in "Check if board is legal to win" — see `_win_blockers` + peel.)
