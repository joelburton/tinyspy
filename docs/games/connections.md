...
The **Restart** button in the terminal action row + the "Replay board" menu item, both driven by the shared **`useStandardGameActions`** hook (which also owns End + Concede here; only **New game** stays bespoke, since connections walks a dated archive rather than re-rolling a board). Resets the working state on the SAME game row: the frozen `board` (the categories AND this game's shuffled tileOrder) stays, so it's the same sixteen tiles in the same arrangement solved again. Any game player, from a finished game OR mid-game (mid-game confirms — it wipes the group's progress).
