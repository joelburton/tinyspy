...
The **Restart** button in the terminal action row + the "Restart" menu item, both driven by the shared **`useStandardGameActions`** hook (which also owns End + Concede here; only **New game** stays bespoke, since connections walks a dated archive rather than re-rolling a board). Resets the working state on the SAME game row: the frozen `board` (the categories AND this game's shuffled tileOrder) stays, so it's the same sixteen tiles in the same arrangement solved again. Any game player, from a finished game OR mid-game (mid-game confirms — it wipes the group's progress).
...
The **End button** in the info-column action row fires this — the manual, **neutral** stop, shown in **coop** (compete shows **Concede** instead — see below). Where `submit_timeout` writes a "you lost" terminal, `end_game` writes `play_state='ended'` with status `{outcome: 'manual', mode}` and every player `{won: false}`: the friends agreed to quit, so nobody won and nobody lost. The FE renders a neutral "Game ended" below-board verdict (the shared `gameEndedTerminalMessage`), not a red loss verdict, and no celebration; `labelFor` learns `'ended'` in both manifests. Any current game player can call it (same `require_game_player` gate as `submit_guess`).
...
connections.puzzles {
  ...
  puzzle_date    date unique     -- NYT publication date; drives the FE picker
...
[`supabase/scripts/import-connections-puzzles.ts`](../../supabase/scripts/import-connections-puzzles.ts), run via `gmake g-connections-puzzles`. Fetches the connections.json from Eyefyre's repo (or `--file <path>` for offline), maps the upstream `{id, date, answers:[{group, members}]}` shape to our `{source_id, puzzle_date, categories:[{rank, name, tiles}]}` shape, upserts on `source_id` with `ignoreDuplicates: true`. Re-runs are no-ops on already-imported rows.
...
| The pause-on-disconnect pattern | [`src/common/pause-suspend/pause.ts`](../../src/common/pause-suspend/pause.ts) + [`src/common/pause-suspend/PauseOverlay.tsx`](../../src/common/pause-suspend/PauseOverlay.tsx) + [`src/common/pause-suspend/PauseBoundary.tsx`](../../src/common/pause-suspend/PauseBoundary.tsx) |
