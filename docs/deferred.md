...
- **Timer counts wall-clock time while no one's on the game (bug).** Today: start a game with a 10-minute timer, leave it at 9:50, navigate home, come back five minutes later — the timer shows 4:50, not 9:50. The wall clock kept ticking even though nobody was watching. The pause infrastructure (presence + manual) only tracks pauses observed by clients currently on the channel; once everyone leaves, the gap is invisible. **Joel's preferred fix is a UX reframe, not a code patch alone:** there should be no casual "return home" link from inside a game. Instead a **"Suspend game"** affordance that (a) flips the game's status / sets an explicit `paused_at` timestamp before navigating away, (b) shows a confirm warning in non-solo games ("this pauses the game for everyone"), and (c) makes resuming an explicit re-entry. Code-side support needs some persistent record of "the game wasn't being played from T1 to T2" — see the bug-report discussion for the options trade matrix (heartbeat-write, mount/unmount-write, server-canonical timer). For now, the bug is the documented behavior; players who walk away during a count-down lose the time. The fix is part of the broader "games are commitments, you don't casually exit" UX intent — different from typical online games where mid-match quitting is expected.
