...
- **`<game>/theme.css` was to become `brand.css`, and never did.** The argument (2026-08-20): thirteen of the sixteen per-game files are nothing but `:root { …tokens }`, and `brand` is the same forcing function as the `--<game>-` prefix — a line that is not brand gets asked about on the way in. Against it: the per-game convention is role names (`manifest.ts`, `logo.svg`, `theme.css`), and `docs/tokens.md → Themes` documents `theme.css`. Not decided either way; if the name stays, say so where the file layout is drawn.
...
- **Literal radii → tokenize by *semantic intent*** (2026-07-01 review §3.3; the
  ...
  *is* (a card → `lg`, a panel → `md`, a chip → `sm`), which is a human judgment.
  ...
  is converted** (tokens.md → The a/b/c rule). A
...
- **Ungated `:hover` on tappable board elements.** A touchscreen keeps `:hover` on the last-tapped element until you tap somewhere else, so a hover-only style sits there after every move looking like state. Only strands' tiles and spellingbee's hexes are gated with `@media (hover: hover)` — the same gate, for the same reason, as the tooltip bubble ([`common/tooltips/doc.md`](../src/common/tooltips/doc.md)). Still ungated: the shared `.tile` in [`game-page/playArea.module.css`](../src/common/game-page/playArea.module.css), `.key` in [`GuessKeyboard.module.css`](../src/shared/onscreen-keyboard/GuessKeyboard.module.css), and the boards of stackdown, wordwheel, boggle, letterboxed and setgame. `src/common/core-css/todo.md` asks whether to gate `:hover` app-wide, which would settle all of these at once.
