...
The channel-qualified feedback split shipped — **local** feedback is `useLocalFeedback` (a near-input `<GenericFeedbackPill>`, validity tones, never a player color) and **group/peer** feedback is `usePeerFeedback` → the header `<PageHeaderStatusSlot>` (the actor's color disc), two separate channels so neither clobbers the other. The naming convention (`Global`/`Local`/`Generic`, never bare "feedback") lives in [code-conventions.md](code-conventions.md).
...
- **The two channels are the same machinery, written twice, and only one of them is a hook** (found 2026-08-28 while building the envelope pill mapping; not blocking that sprint or the CSS one). `useLocalFeedback` holds the state, the fault branch and the auto-clear timer; the **global** side is four `useState` / `useCallback` / `useMemo` blocks inline in `GamePage.tsx:198-298` doing the same three jobs. The hook's own docstring says they differ only in *where the host renders it*. Concretely:
  ...
  - **`usePeerFeedback` is not the twin of `useLocalFeedback`** — it is a peer-event-stream narrator (a seen-set bootstrap that fires a pill per new item) which *feeds* the global sink. The filenames promise a symmetry that isn't there.
