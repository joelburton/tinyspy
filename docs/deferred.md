...
- **Who owns a `.module.css`, and how a shared one is read.** The understanding the naming implies — *"`Foo.module.css` is about the look of `Foo`; I can change it worrying only about `Foo`"* (Joel, 2026-09-14) — is true for most of the repo and silently false where a second component imports one. The repo already has the escape hatch and has never written it down: **a stylesheet meant to be read by others is named in lowercase**, because it is not a component's. Ten files do it and every one is multi-importer — `historyViewer.module.css` (19 importers), `modalActions.module.css` (6), `infoPanel.module.css` (4), `foundWordsPlayArea.module.css` (4), `pickers.module.css` (4), then `gridCursor`, `dragGhost`, `typedWord`, `clubFilters`. [common-folders.md](common-folders.md) states both halves separately (*"leading-cap is a component … lowercase is everything else"*, then *"`.module.css` is a stylesheet"*) without joining them.
  ...
  - **The offender under it** is `turn-log/TurnLog.module.css` (eleven games' `GameTurnLog.tsx`), filed in that folder's `todo.md`. `game-page`'s sheet was the other, and was renamed to `playArea.module.css` on 2026-09-16 — no `PlayArea` component exists in that folder, which is what let five concerns collect in it; subdividing them is still open in its `todo.md`. Same-folder siblings reading a component's sheet are a milder, separate case, and there are 24 of those.
  - **`composes:` is the other half, and is probably the better answer where a game EXTENDS a shared look** rather than just wearing it. A game's own module writes `composes: row from '…/turnLog.module.css'` and adds its own declarations; the game's `.tsx` then imports only its own stylesheet, always. Joel, 2026-09-14: *"not having the component itself know which parts of its style comes from one module vs another is a real win."* The repo uses `composes` nowhere today (one stylesheet mentions having declined it in favor of custom properties), so adopting it is a decision about the shape of our CSS, not a local edit.
...
- **`dismiss` may want a shared component, not just a shared class.** The icon-only ✕ appears on toasts, pills, banners, floating panels and game cards with **five** implementations (`Toast .close`, `FeedbackPill .close`, `historyViewer .bannerExit`, `FloatingPanel .closeButton`, `CurrentGameCard .deleteButton`). They agree on being neutral and disagree on everything else — size, glyph, hover, whether there's a border. Now that the neutral `<button>` means none of them is undoing anything, the differences left are real and small enough to be worth a single component. Not urgent; noticed while sorting the fourteen kinds ([ui.md](ui.md#what-a-button-is-the-fourteen-kinds)).
...
- **Each game's below-board reserve is a hand-tuned constant, and none is checked.** Eleven games size their board's height as `100svh − var(--game-chrome-height) − <their own number>` — `− 3.5rem` for waffle, `− 15rem` for wordle, `− 8.5rem` for stackdown, `− 4.4rem`/`− 6.1rem` for scrabble, and so on — each standing for "everything else in my column" (the entry row, the feedback slot, a mobile status bar). They are the same construction as `--game-chrome-height` was before it was composed on 2026-09-15: a total someone added up once. A number that is too big wastes space no test can see, and too small overflows. Measuring them needs a per-game probe of the slack INSIDE the board column, which is a level below the page-fits-the-viewport check that now guards the shell. Surfaced by the game-page audit while fixing the shell's own 1px.
- **letterboxed is ~5px over at short viewport heights.** Measured 2026-09-15 across all fifteen solo-playable games at 1128×617: every other game is exactly at the shell's figure, letterboxed is 5px past it. The same ~5px `short-viewport.e2e.ts` recorded in prose in an earlier pass and left alone. It is the one per-game discrepancy in the set, so it is likely one of the hand-tuned reserves above rather than anything shell-wide.
