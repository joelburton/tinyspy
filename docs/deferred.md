...
- **No recovery from a LOST realtime event.** A channel can report `SUBSCRIBED` and then never deliver `postgres_changes` — reproducibly, for the first channel after the Realtime tenant restarts ([realtime-lost-events.md](realtime-lost-events.md)). The existing defence, `useCommonGame`'s refetch on every `SUBSCRIBED`, cannot help: it already fired, before the channel went deaf. So a client sits connected, rendering a game that has silently stopped updating, with nothing on screen saying so — the partner's moves simply never arrive. Fixing it means a staleness check of some kind (a periodic re-read, or re-verifying terminal state after your own move lands), which is new shared realtime plumbing and a design decision, not a patch. Caught by one e2e spec on 2026-08-06; deliberately left alone rather than papered over with a longer timeout or a Playwright retry.
