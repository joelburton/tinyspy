...
- **Crosswords replay.** The only game with neither a `replay_board` RPC nor a recorded reason not to have one — codenamesduet and bananagrams both opt out deliberately (the board *is* the secret / there's no puzzle to re-run). Crosswords is simply unaddressed: decide whether a solved-or-abandoned puzzle is worth re-running (against the same grid, with the answers half-remembered) before building it.
