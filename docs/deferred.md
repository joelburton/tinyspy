...
- **Drop the dead `TerminalCopy.outcome` field (actionable now).** The end-states sweep retired the shared `<GameOverModal>` from every game (the verdict is carried in-page by the below-board pill + the info-column outcome line; a win worth marking pops `<CelebrationDialog>` instead), and the modal + `TerminalModal` + `useTerminalModal` were deleted. `outcome: 'won' | 'lost'` was that modal's green-vs-red input and **nothing reads it now** — but all thirteen `buildOver()`s still populate it, several with a comment explaining that `'won'` only ever meant "not a loss". Dropping it is mechanical (one field on the type, one line per game) and was left out of the sweep only to keep each game's commit to its own copy. Do it in one pass across the roster.
