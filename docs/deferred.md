...
- **Retire the `-bg` half of the outcome vocabulary with `color-mix`.** `common/theme.css` pairs each outcome tier with a `-border` and a `-bg`; `won-bg` and `lost-bg` are used by ClubGameCard; `active-bg` lost its last reader when the setup Calendar was deleted (2026-08-13) and is now a recorded completeness slot. The open idea is to derive those from the `-border` token via `color-mix` and drop the hand-picked `-bg` values entirely. *(The narrower half of this item is done: `near-bg` was dead — and its comment falsely credited connections's one-away flash, which actually reads `-border` + `-strong` — so it's deleted. `current-bg` stays: it's a deliberate vocabulary-completeness slot, now recorded in `VOCABULARY_COMPLETENESS` in [`src/cssTokens.test.ts`](../src/cssTokens.test.ts), which is where that policy is enforced rather than argued.)*
