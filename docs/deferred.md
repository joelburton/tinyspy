...
- **Club-level game-list editor.** Today every newly-created club is auto-populated with every registered gametype in `common.clubs_gametypes` (via `claim_username` for solo clubs and `create_club` for friend clubs). There's no UI for a club to opt out of a gametype it doesn't want, and there's no auto-propagation to existing clubs when a new gametype is registered later (DB-admin INSERT handles that case under the alpha prior). A future "club settings" surface could let members manage their own m2m rows.
...
- ~~**Click-to-define popover + "look up any word" shortcut.**~~ **SHIPPED 2026-06-18.** Common word-lookup feature, documented in [`common.md → Word definitions`](common.md#word-definitions-click-to-define--lookup). `common.definitions` cache + `cache_definition` RPC (migration `20260622000000`), the `define` Edge Function (read-through cache → Wiktionary), the scrabble-defs seed import (`npm run defs:import`), and the freebee FE wiring (clickable `WordList` rows + the `~` shortcut). pgTAP `common/definitions_test.sql`; FE `parseDefinition.test.ts`. Possible follow-ups: surface the lookup in other word games as they land; richer Wiktionary rendering (etymology/IPA, currently dropped).
