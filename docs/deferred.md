...
**A deferral that lives inside one game lives in that game's doc**, under a `## Deferred` heading. Work tends to happen game-by-game, so the item should be in the file you already have open. The sorting key is **which file you'd edit to do the work**, not which game surfaces it.
...
| [codenamesduet](../src/codenamesduet/todo.md) | the register lives in the game folder's `todo.md` since its area opened 2026-09-23 |
...
- **Thirty-three game RPCs still answer a deleted game as a fault.** The rule is [envelopes.md → a missing game row is PN485](envelopes.md#how-sql-builds-one): a missing `<schema>.games` row is `common._raise_game_deleted`, asked before `require_game_player`. `end_game`, `submit_timeout` and `replay_board` do this in every game, and `spellingbee.submit_word` since 2026-09-23. These raise a fault of their own instead — `BUG: … with no <game> row` or *"That game no longer exists"* — and several gate on membership first, so a deleted game there answers *"You are not in this game"*: `bananagrams.dump`, `bananagrams.peel`, `boggle.submit_word`, `codenamesduet.get_clue_context`, `.pass_turn`, `.submit_clue`, `.submit_guess`, `connections.submit_guess`, `crosswords.export_solution`, `.reveal_solved_word`, `letterboxed.clear_chain`, `.log_hint_or_spoiler`, `.submit_word`, `.undo_word`, `psychicnum.request_hint`, `.request_spoiler`, `.submit_guess`, `scrabble._commit_exchange`, `._commit_pass`, `._commit_word`, `.get_ai_context`, `.get_suggest_context`, `setgame.record_hint`, `.submit_set`, `stackdown.reveal_next_hint`, `.reveal_next_word`, `.submit_word`, `strands.spend_hint`, `.submit_path`, `waffle.submit_swap`, `wordiply.submit_guess`, `wordle.submit_guess`, `wordwheel.submit_word`. Each conversion retires its code, makes its handler read `constraint_name` if it does not (the helper raises `lost`, and `raiseCodes.test.ts` refuses a caller that drops it), adds its function to `gameDeletedFirst.test.ts`'s list, and wants one pgTAP case that deletes the game and calls it. Cross-cutting because one change runs through every game's SQL file; any area may take its own share.

- **Who owns a `.module.css`, and how a shared one is read.** The understanding the naming implies — *"`Foo.module.css` is about the look of `Foo`; I can change it worrying only about `Foo`"* (Joel, 2026-09-14) — is true for most of the repo and silently false where a second component imports one. The repo already has the escape hatch and has never written it down: **a stylesheet meant to be read by others is named in lowercase**, because it is not a component's. Ten files do it and every one is multi-importer — `historyViewer.module.css` (19 importers), `modalActions.module.css` (6), `infoPanel.module.css` (4), `foundWordsPlayArea.module.css` (4), `pickers.module.css` (4), then `gridCursor`, `dragGhost`, `typedWord`, `clubFilters`. [common-folders.md](common-folders.md) states both halves separately (*"leading-cap is a component … lowercase is everything else"*, then *"`.module.css` is a stylesheet"*) without joining them.
  - **The rule to state**: capital means one component's, and only that component (and its test) may import it; lowercase means a shared sheet anyone may. It is exactly guardable — an import of `Foo.module.css` from anything but `Foo.tsx`/`Foo.test.tsx` fails — which is the kind of rule this repo already enforces a dozen of.
  - **Both named offenders are now fixed.** `event-log/EventLog.module.css` was split by reader on 2026-09-16 (F-event-log-16): the thirteen classes only its own components draw stay under the capital name, and the six a game puts on its own rows moved to `gameEventLog.module.css` — measuring which side read what is what showed a rename alone would have called thirteen private rules shared. `game-page`'s sheet was the other, and was renamed to `playArea.module.css` on 2026-09-14 — no `PlayArea` component exists in that folder, which is what let five concerns collect in it. One of the five has since left: the info column's box and rows moved to `info-sheet/infoCol.module.css` on 2026-09-18, on the line that the PlayArea root div's classes stay and the column's go. Subdividing the remaining three is still open in `game-page`'s `todo.md`. Same-folder siblings reading a component's sheet are a milder, separate case, and there are 24 of those.
  - **`composes:` is the other half, and is probably the better answer where a game EXTENDS a shared look** rather than just wearing it. A game's own module writes `composes: row from '…/eventLog.module.css'` and adds its own declarations; the game's `.tsx` then imports only its own stylesheet, always. Joel, 2026-09-14: *"not having the component itself know which parts of its style comes from one module vs another is a real win."* The repo uses `composes` nowhere today (one stylesheet mentions having declined it in favor of custom properties), so adopting it is a decision about the shape of our CSS, not a local edit.
  - **No owning area**, which is why it is here: the convention is repo-wide, while each violation belongs to the folder that holds it. Surfaced by the `setup-form` audit, where `SetupDisclosure` was drawing its only class from `game-page/playArea.module.css` (F-setup-form-10).
...
- ~~**No recovery from a LOST realtime event.**~~ **Fixed (2026-08-06).** The mechanism turned out to be measurable — `SUBSCRIBED` is only the join ack, and events committed before the server's `system` "Subscribed to PostgreSQL" attach confirmation are dropped — which made the fix a patch after all, not new plumbing: every postgres_changes hook now refetches again when the attach is confirmed (`common/realtime/postgresAttached.ts`), closing the window. Pinned by `e2e/realtime-deaf-window.e2e.ts` (written failing, flipped by the fix). Full story: [realtime-lost-events.md](realtime-lost-events.md).
- **Contract slots that nobody declares.** Common reads a family of custom properties that no common file declares and that each game fills on its own layout — `--grid-gap`, `--tile-font-min` / `-max` / `-factor`, `--max-tile-width`, `--max-board-size`, `--board-units-*`, `--local-feedback-min-height`, `--swap-box-min-height`, `--stats-col-gap` / `--stats-max-width`, `--rank-text`, `--cols` (measured 2026-08-20, re-checked 2026-09-19: still true). A slot is the mechanism working as designed — the shared `.tile` reads only tokens so a game restyles by re-setting one — but a slot is a blank, not a decision, and it fails backwards: a token nobody READS is clutter, a slot nobody FILLS invalidates the whole declaration that reads it, silently. `cssTokens.test.ts` cannot see it, because each name IS defined, in some game's file; what nothing checks is that every game mounting the reading component defines it. Two things owed, neither decided: declare the family in one place (a `--board-*` block in the scaffold, so a game filling it sees the whole set), and a guard that a game which mounts the reader sets the slot. `--info-col-width` is the one that IS owned by common (`playArea.module.css` says a game must declare it) and is the model.
- **`<game>/theme.css` was to become `brand.css`, and never did.** The argument (2026-08-20): thirteen of the sixteen per-game files are nothing but `:root { …tokens }`, and `brand` is the same forcing function as the `--<game>-` prefix — a line that is not brand gets asked about on the way in. Against it: the per-game convention is role names (`manifest.ts`, `logo.svg`, `theme.css`), and `docs/ui.md → Themes` documents `theme.css`. Not decided either way; if the name stays, say so where the file layout is drawn.
- **Four guards strip comments by regex, and a `/*` inside a string literal can blank real code.** `src/guards/` has four hand-rolled comment strippers — `rawStorage.test.ts`, `cssClasses.test.ts:55`, `cssTokens.test.ts:40`, and `callSiteShape.test.ts`'s cruder line-prefix test — and all of them treat `/* … */` as a comment wherever it appears. A string literal containing `/*` therefore blanks everything to the next close, which for a scanning guard means **passing while a violation ships**: the one failure mode a guard must not have. **It is reachable, and a glob is the ordinary way** — measured 2026-09-03 by running `rawStorage`'s stripper over all of `src/` against the TypeScript parser's own comment ranges: 38 lines in 8 files are blanked that the parser says are code, and 0 go the other way. Seven of the eight are one-line and self-contained (the guards' own regex literals, a `'/*'`/`'*/'` string pair in `dbCallWrapped`); the eighth is `cssTokens.test.ts`, where `'themes/*.css'` in a bucket's `where:` hides the twenty-five lines after it. Every hidden region is inside `src/guards/`, so nothing a guard looks for is hidden today, but the argument that a `/*` in a string is contrived was wrong. The *other* half of the same hole is already closed — `cssClasses:58`, `vocabularies:75` and now `rawStorage` all use `(^|[^:])//` so a URL cannot swallow the rest of a line. **Filed rather than fixed** (the `utils` audit, 2026-09-03): fixing one stripper with a real tokenizer would make a single guard exact and leave four implementations that disagree, which is worse than the hole. It is worth doing as ONE change to all four or not at all. `typescript` is already a direct devDependency (`~6.0.2`), so `ts.createScanner` needs nothing new — but **no guard uses the TS API today**, so adopting it is a decision about the shape of `src/guards/` as a whole. **There is no owning area:** `src/guards/` is on no row of the app audit's areas table, which is why this is here rather than in a folder's todo file.

- **The per-game section of [keyboard-shortcuts.md](keyboard-shortcuts.md) goes, game by game.** Its key tables are hand-kept twins of the generated key list in Help and will drift; the sentences between them (crosswords' rebus and number-jump popups, letterboxed's letter-refusal rule, setgame's card grid, bananagrams' type-over swap) are that game's and belong in its `docs/games/<game>.md`. Joel, 2026-09-11: "get rid of these; they're not needed and will definitely drift" — "as we hit the game". So as each game's area opens, move its prose into its doc and drop its block; the "Per-game board keys" heading goes with the last one. The global sections above it stay.
- **The history viewer's banner ✕ is its own `<button>`; consider `<CloseButton>`.** The shared dismiss glyph landed 2026-08-26 in the CSS sprint's `forms` area — `common/buttons/CloseButton.tsx`, converted at the floating-panel titlebar, the toast and the feedback pill. The banner's exit is the one left. **It is now a one-file question**: the `event-log` area gave the banner a component (`common/event-log/HistoryBanner.tsx`, 2026-09-16), so the `✕` (U+2715) that used to be typed by hand into nine `BoardCol.tsx` is written once. What made this urgent — nine copies agreeing by luck — is gone; what is left is a look decision, since `<CloseButton>` is a `StandardButton` with a hover wash and an `em`-sized box, and the banner's exit is a bare glyph with its own padding. Swapping it changes how the ✕ reads in ten games, so it wants a look rather than a sweep. **Not in the family:** letterboxed's `.chainRemove`, which is an *undo* ("Take back WORD") that merely looks like a close.
...
- **Literal radii → tokenize by *semantic intent*** (2026-07-01 review §3.3; the
  ...
  *is* (a card → `lg`, a panel → `md`, a chip → `sm`), which is a human judgment.
  ...
  is converted** (ui.md → The a/b/c rule). A
...
- **Should the `--secondary-*` slot tokens be renamed `--tone-*`?** Raised during the button-taxonomy work (shipped 2026-08-18) and never settled. `.secondary` is a *treatment* and the slot holds a *tone*, so the current name describes the caller rather than the contents — and it now has a `--primary-*` twin with the same shape, which makes the asymmetry easier to see: `--button-slot-primary-color` reads as "the filled treatment's fill", which is what it is, while `--button-slot-secondary-color` reads as "the outline's ink" only because you know what `secondary` means. A rename touches the five files that write or read `--button-slot-*`: both themes (`themes/daylight.css`, `themes/midnight.css`), `buttons/StandardButton.module.css` (which both declares the slots per tone and reads them), and the two raw-`<button>` sites that re-set a slot to borrow a tone — `feedback/FeedbackPill.module.css` and `definitions/WordEditDialog.module.css`. Strands' `HintBar` is NOT among them: it reads the `--button-caution-*` family tokens directly, not the slots.

- **`dismiss` may want a shared component, not just a shared class.** The icon-only ✕ appears on toasts, pills, banners, floating panels and game cards with **five** implementations (`Toast .close`, `FeedbackPill .close`, `historyViewer .historyBannerExit`, `FloatingPanel .closeButton`, `CurrentGameCard .deleteButton`). They agree on being neutral and disagree on everything else — size, glyph, hover, whether there's a border. Now that the neutral `<button>` means none of them is undoing anything, the differences left are real and small enough to be worth a single component. Not urgent; noticed while sorting the fourteen kinds ([ui.md](ui.md#what-a-button-is-the-fourteen-kinds)).
...
- **Eight controls override the disabled fade with their own number.** `--chrome-disabled-opacity` moved 0.5 → 0.75 on 2026-08-18 (the reasoning is at the token: the missing hover and the tooltip carry "disabled", so the fade only has to make the dead one findable in a row — fading harder cost reading the label). Eight components predate that and still hard-code their own, every one of them someone finding 0.5 too harsh and picking a number in isolation — which is the problem the global just solved centrally:
  ...
  | `0.6` | `.button:disabled` | `common/anagram-finder/AnagramDialog.module.css` |
  | `0.6` | `.saveButton:disabled, .deleteButton:disabled` | `common/definitions/WordEditDialog.module.css` |
  | `0.6` | `.key:disabled` | `shared/onscreen-keyboard/GuessKeyboard.module.css` |
...
The channel-qualified feedback split shipped — **local** feedback is the below-board slot (a near-input `<FeedbackPill>`, outcome colors, never a player color) and **group/peer** feedback is the header's global slot (the actor's disc, fed by `usePeerFeedback`), two separate slots so neither clobbers the other. Both are one store, `createFeedbackSlot`, ranking `FeedbackMessage`s by kind ([ui.md → Feedback pill](ui.md#feedback-pill)); the naming rule (a slot instance is `localFeedbackSlot` / `globalFeedbackSlot`, never bare "feedback") lives in [code-conventions.md](code-conventions.md).

- ~~**The two channels are the same machinery, written twice.**~~ **Done 2026-09-12** by the `feedback` area's redesign: one slot store for both, a message's kind deciding how it leaves, no `locked`, no per-channel timer.
...
- **Does a fault still want a modal AND a pill?** (raised 2026-08-31, unresolved.)
  ...
  gets both, and `FeedbackMessage.notOk` says so deliberately, because filtering
  it out would leave the board showing a stale answer after the modal is dismissed.
...
- **Boggle compete "dupes-cancel" scoring.** The *authentic* paper Boggle rule: a word found by more than one player scores zero for everyone. Boggle deliberately does the opposite today — the `boggle.found_words` PK is `(game_id, user_id, word)`, dedup is per-player, so two players who independently find the same word both keep it. **Far future because the question is whether we want the rule at all** (2026-08-03), not how to build it; the build is a scoring change in `submit_word` / `_finish` plus a setup flag, and it's what would license the **⦻ marker** in the shared [`WordList`](../src/common/word-list/doc.md). If it's ever picked up, the thing to decide first is the honesty problem: compete finds are private mid-game (RLS scopes `found_words` to you until terminal), so a word can't show as canceled while you play — you'd see it accepted and watch it zero out at scoring. Either that reveal *is* the fun, or the mode should hide the running score.
