...
- **No double-submit guard on New game** (2026-07-31 review, C5a). None of the thirteen New-game handlers disables itself or drops a second call while the RPC is in flight. Two quick clicks make two fresh games: the first sits orphaned in the club list (`create_game` un-currents it) and peers get two invitation toasts. Self-healing at friends-scale, which is why it's deferred, but it's real. Unlike Restart there's no shared hook to fix it in — New game stays per-game *by design* (see the "New game is NOT here" note in [`useStandardGameActions.ts`](../src/common/hooks/game/useStandardGameActions.ts)), so this is thirteen edits or a new shared wrapper, which is the whole reason it's still here. *(The Restart half was fixed 2026-08-02: `useStandardGameActions.restart` now holds an in-flight ref.)*
