...
**The items that need a database migration are indexed in [`db-work.md`](db-work.md)** — a working queue we're going through while `db:reset` is still free (the alpha prior in [`CLAUDE.md`](../CLAUDE.md)). Each entry there is a pointer plus the concrete shape of the migration; the rationale stays in this file or the game's own doc. Delete `db-work.md` once it's empty.
...
| [boggle](games/boggle.md#11-deferred) | word-list freshness via Storage · a "check board" helper |
