...
- **No double-submit guard on New game / Restart** (2026-07-31 review, C5a). None of the thirteen New-game handlers — nor `useStandardGameActions.restart` — disables itself or drops a second call while the RPC is in flight. Two quick clicks make two fresh games: the first sits orphaned in the club list (`create_game` un-currents it) and peers get two invitation toasts. Self-healing at friends-scale, which is why it's deferred, but it's real. The fix belongs in the shared hook (an in-flight ref, like the pattern each game already uses for its own submit) rather than thirteen times over; New game would need the same guard where it stays per-game.
