...
- **lost** — the 7th wrong guess landed. Terminal.
...
### `psychicnum.create_game(target_club uuid) → table(id uuid)`

Caller must be a club member. Picks a random target 1–10, inserts the game row in `active`, upserts `common.club_active_game` pointing at it.
...
- `'lost'` — caller's wrong guess was the 7th; status flipped to `lost`.
