...
| *any key* | **Exits the turn-history viewer** back to the live board, and is consumed (so the same press doesn't also play a move). Every game with an event log has the viewer. Clicking anywhere exits too. |
...
| A `<StandardForm>` — setup, the club forms, the word dialogs | `Enter` | **Commit it, from anywhere in it.** The browser submits from a text box alone, which leaves a form of checkboxes and radios — a game's setup — with no key that finishes it. Three controls keep their own `Enter` instead: a button (it presses that button, which is how a setup dialog's puzzle pickers and its Cancel work), a textarea (a newline), and a native `<select>`. It goes through the commit button, so a Start the setup has disabled stays disabled. |
