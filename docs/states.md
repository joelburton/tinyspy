...
- **Non-terminal, MULTIPLAYER**. The suspend-confirm MODAL (a real modal: backdrop-blocked board, dialog-owned keyboard — `SuspendConfirmationBlockingModal`, a wrapper over the shared `ConfirmationBlockingModal`). On accept, ALL viewing members (not just the leaver) move to the club page and the game stops being current.
