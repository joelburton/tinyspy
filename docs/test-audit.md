...
> **Update — addressed** (recommendation #7). testing.md's scope paragraph was
> rewritten to describe the real buckets (realtime/presence/auth, responsive
> layout, real-PDF print, the history overlay, game-specific browser behavior).
> All the named holes are filled: mobile (boggle/wordwheel/wordiply), print
> (boggle/wordwheel), history-viewer (waffle/stackdown), and a concede-flow spec.
> The snapshot below is pre-implementation.
...
7. ~~**Round out e2e to its de-facto scope**~~ **DONE.** Shipped: **mobile** specs
   for boggle/wordwheel/wordiply (`*-mobile.e2e.ts`, the shared info-sheet recipe);
   **print** smokes for boggle/wordwheel (`*-print.e2e.ts` — both have printers;
   wordiply/waffle don't); **history-viewer** specs for waffle + stackdown
   (`*-history.e2e.ts`, on new `seedWaffleSwap` / `seedStackdownFirstWord`
   fixtures — the latter pins the game's own board row to the fixture board so a
   real submit_word can seed a move without touching the shared library); a
