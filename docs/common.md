...
- `src/gametypes.ts` is the one exception — it imports every game's manifest by definition.

The payoff is the **removability invariant**:

> Any game must be removable in three actions: delete its folder, delete its line from `src/gametypes.ts`, drop its schema.
...
- A gametype's supported player-count range is declared on its manifest (`numberOfPlayers: [min, max]`). The shell decides whether to surface "Start X" in a given club by checking the range against the club's member count. A fixed-seat range like codenamesduet's `[2, 2]` never appears in a solo club; open-N ranges like `[1, 6]` appear in solo clubs and in clubs up to the cap; a future game with `[3, 5]` would only appear in clubs sized 3-5. ClubPage lists no row for a gametype with no `clubs_gametypes` row, and dims the row (the count as its tooltip) when the row exists but the count is out of range — see `playerCountFits` / `playerCountLabel` in `src/common/manifest/gameManifest.ts`.
- Mode-specific UX (coop vs compete) does NOT live in the setup form — see [The sibling-manifest pattern](#the-sibling-manifest-pattern) below. Each variant is its own registered gametype with its own row in the start list; setup choices stay shape-uniform across siblings.
...
- its own manifest entry in `src/gametypes.ts` (`psychicnumCoopGame`, `psychicnumCompeteGame`)
- its own URL prefix (`/g/psychicnum_coop/<id>`, `/g/psychicnum_compete/<id>`)
- its own row in the ClubPage's start list
...
| field          | purpose                                                                     |
|----------------|-----------------------------------------------------------------------------|
| `baseGametype` | The family key. Equals `gametype` for single-mode games. Used to group siblings — the gametype filter over the club page's "Your games" collapses a pair into one choice. |
...
- **`common.game_scratchpads`** (table) + **`common.set_scratchpad(target_game, p_owner_id, p_body)`** (RPC) — one row per pad, a per-row `version` bumped on write. `owner_id null` = the **shared** coop pad; a user id = a **private** per-player compete pad. RLS gates a row to game members (and, for a private pad, its owner). The RPC guards on membership only: the notes outlive the game, so a finished game's pad stays writable.
...
| table | purpose |
|---|---|
| `profiles` | One row per auth user. Holds `username` (unique, CHECK-validated against the canonical regex `^[a-z][a-z0-9-]{2,14}$` — 3–15 chars, capped for mobile legibility) + `color` (from the 8-entry palette CHECK). Materialized by the `common.claim_username` RPC the user calls themselves on first sign-in (no trigger; see [Username claim flow](#username-claim-flow)). `username` is immutable in v1; `color` and `sounds_enabled` are editable together via the caller-scoped `common.update_profile` RPC (the "Edit profile" dialog). `sounds_enabled` is the player's one switch for every sound the app plays (the turn bell, the win jingle), read by `common/sounds/playSound`; it defaults to true, which is how every account that predates it got it on. Also carries `ai_member` — true for scrabble's three AI opponents, who are ordinary accounts in every other respect (a profile, a color, a solo club) and are marked so nothing has to recognize a bot by its name; `can_edit_words` — the dictionary-curation flag, granted by hand in the database — and `theme`, a **reserved** free-form nullable `text` column nothing reads yet, held for a future theme picker; NULL means "use the app default"; constrain it once real theme names exist. No UPDATE policy — every write goes through an RPC. Its FK to `auth.users` is ON DELETE **RESTRICT** — the top of the deletion firewall (see below). |
...
| `gametypes` | The registered-gametype list (`gametype text PK`, `min_players smallint`, `default_enroll boolean`). Authoritative SQL-side mirror of `src/gametypes.ts`. Each gametype's schema migration registers itself with an `INSERT ... ON CONFLICT DO NOTHING` (a seed row, so it stays on the migration side), declaring its `min_players` (the lower bound of the manifest's `numberOfPlayers` — coop/solo games register 1, two-player games 2). Sibling families register one row per variant — psychicnum's migration inserts both `psychicnum_coop` and `psychicnum_compete`. `min_players` is the one fact the server needs from the player-count range: `common.default_gametypes_for_club` uses it to keep solo clubs out of two-player games. `default_enroll` (default true) says whether a NEW club is enrolled automatically — psychicnum's pair registers **false** (it's the architecture-exercise toy, not a game a fresh club should find on its Start list); off-by-default, not banned, since `set_club_gametypes` can opt a club in. Permissive SELECT — gametype identifiers aren't sensitive. |
| `games` | The universal game-record header. One row per game-playing across all gametypes. Holds `club_handle`, `gametype`, `title`, the **view-state pair** (`is_current_view`, `paused`), the **play-state pair** (`play_state` text + `is_terminal` boolean), `status` jsonb (the gametype-specific listing-label payload), `started_at`, `ended_at`, and `last_active_at` (stamped by a BEFORE UPDATE trigger on **every** write to the row — moves, the `is_current_view` shelve/resume flip, pauses, the terminal write — so the club games list orders + dates by when a game was last *touched*, not when it started; see [last_active_at](#last_active_at)). (The game clock lives in its own table, `common.timers` — see below.) `current_turn_user_id` (nullable) is the common turn-order pointer for opt-in turn-by-turn coop games — null for free-for-all (the default); see [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games). Per-gametype detail (board, secret, current turn) lives on `<gametype>.games`, which shares an id with this row via FK. The `title` is a short human-readable label built by each gametype's `create_game` at insert time (see [Title formulas](#title-formulas)) — the FE renders it beside the gametype's logo in club game lists (`CurrentGameCard`). The two state pairs and their orthogonality are written up in [`states.md`](states.md). |
| `game_players` | M2M between games and profiles, recording who played each game. Frozen at game-create time — distinct from `clubs_members`, which is current membership of the club. The `result jsonb` column carries each player's outcome (won/lost flag, score, etc.), populated by `common.end_game` at terminal transition. `conceded` / `conceded_at` are the per-player drop-out flag (see [Concede](#concede--per-player-drop-out)), and `locally_terminal` says this player is DONE while the game plays on (see [Done, but not out](#done-but-not-out--locally_terminal)) — the per-player terminal state that exists *before* `end_game`, because peers must see who bowed out and the flag distinguishes "Quit" from "Lost" at game-over. `turn_seat` (nullable) is the player's position in a turn-order game's rotation (see [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games)). |
| `clubs_gametypes` | M2M between clubs and gametypes. Row existence answers "is this club allowed to play this gametype?" — the FE filter for which rows the club's start list offers. Seeded at club-creation via `common.default_gametypes_for_club` (friend clubs via `create_club` get every `default_enroll` gametype — so not psychicnum; solo clubs via `claim_username` additionally drop the `min_players > 1` rows, since their lone member can't field a two-player game). Members edit the set afterward through the "Edit club" dialog (`common.set_club_gametypes`). Sibling-variant pairs get one row per gametype string (`psychicnum_coop` AND `psychicnum_compete` both land here when the club is created). A `default_setup jsonb` column on each row carries the friends' last-used setup choices for that (club, gametype) — auto-written by `common.create_game` on every successful start, read by the FE on dialog-open and merged under the manifest's static defaults. Sibling variants get independent defaults (coop's last `setup.guesses` choice doesn't affect compete's). Each gametype decides what fields of its setup are per-club preferences vs per-game decisions (e.g., codenamesduet strips `first_clue_giver_user_id` before saving since the seat picker is per-game); the per-game `create_game` RPC controls what gets passed as the `saved_default` argument. See `deferred.md` for the setup-shape evolution policy. |
| `messages` | Per-club chat. Single persistent thread per club, spans games and gametypes within the club's lifetime. 1–1000 character constraint on `content`. **Important-prefix:** a message starting with `!` is treated as important — `<Chat>` force-opens the chat panel for every other player when one arrives (the panel stays open until they dismiss it), and `<ChatBody>` styles the message text differently. Useful for "Hey everyone — I'm back!" or "Pause please." Implemented in `Chat.tsx` (`startsWith('!')` check on the latest message) + `ChatBody.tsx` (display strip the `!` + apply `.importantContent` class). |
...
The flip is the club's durable "current game" pointer: it drives the ClubPage's current-game card (and the abandoned-pointer heal). It does NOT pull anyone into the game — being added to a game pops a join invitation instead (see [Joining a game — the invitation toast](#joining-a-game--the-invitation-toast)).
...
The game clock is **`common.timers (game_id, ticks, last_tick)`** — its own table (not a column on `common.games`, so the per-second tick UPDATE doesn't churn the games realtime stream). `ticks` is an **additive** count of whole seconds of *active play*. Every actively-playing client calls **`common.tick_timer`** once a second; its conditional (`now() - last_tick >= 1 second`) advances `ticks` by at most 1 per real second — which dedupes across players (concurrent same-second calls no-op) and makes a pause/idle gap cost **+1 on resume, not the gap**. The FE (`useGameTimer`) derives the display from `ticks`: countdown shows `max(0, duration - ticks)`, countup shows `ticks`. Pause and "nobody viewing" need no bookkeeping at all — they're just seconds where nobody calls tick_timer, so the clock stops. **Robust by construction:** because the count only moves while someone is actively ticking, a navigate-away / crash / tab-kill just stops it — there's no "remember to record the gap" write to miss, which is the failure an accumulator cannot avoid. A pause costs ±~1s (the resume tick is `+1`, not the gap), and the display second-flips carry network latency as jitter — invisible for friendly word games; local interpolation between ticks is an easy add if perfectly-smooth display ever matters.

The FE half — the poll, the merge rule, who fires the timeout — is [`src/common/timer/doc.md`](../src/common/timer/doc.md).

### Title formulas

`common.games.title` is `not null` and `length(trim(title)) > 0`-checked. Each gametype's `create_game` builds the value at insert time and passes it as the `title` argument to `common.create_game`. Choosing a formula is a per-gametype call — there's no universal-good answer ("just list the players" fails inside a single club where every game has the same 2-3 players). Each formula lives in that gametype's SQL — its `create_game`, and the move RPCs or `_sync_title` that rewrite it; [game-status-labels.md](game-status-labels.md) describes the title beside the status line it is read with.

Three rules run across all of them. **A title names the game after its content** — the board's words, the puzzle's date, the answer — since that's what a player recognizes a game by; two games take a pure identifier instead (`#` + the first six hex of the game's uuid): bananagrams because it has nothing shareable to name (private grids, private hands), and **setgame because a stable HANDLE is worth more than a live count** — its sets-found total is public and would have made a legal title, but a title that changes every few seconds can't be used to REFER to a game, while "look at #A3F19C" can be said to another player and searched for. **A title may be a readout**: nothing stops a gametype's own RPCs rewriting it mid-game — it's just a column on `common.games` — so games with nothing to say at create time seed a `'New game'` placeholder and fill it in from play (scrabble's first words played, stackdown's cleared words, wordle's latest guess, waffle's correct words). A mode that holds the placeholder for a whole race — wordle compete, waffle compete — seeds `'New compete'` instead, since that's the label a club list actually sits on. A rewriting gametype derives the title from state in a single `_sync_title` helper that every transition calls, rather than assigning it move by move; that's what makes a timeout, a concede, a manual end and a **replay** all land on the right string — a replayed game must stop advertising the answer. **A title can only carry what every player already sees**, because the column is club-wide readable: wordle compete and stackdown compete hold the placeholder while private guesses are in flight, waffle compete waits for the terminal reveal, and psychicnum names itself after board words rather than its secrets.

The grammar every status line speaks lives in [`statusLabel.ts`](../src/common/manifest/statusLabel.ts)'s header — the leading words, the devices, and what a line may say — and `npm run report:labels` prints what every game actually says. One of those rules is a constraint on what the SERVER writes, which is why it is named here too: a status line may only say what every player already sees, and `status` is club-wide readable, so a compete game's private per-player progress must not be written there in the first place — several compete labels are a bare `Playing` for exactly that reason.

The "no gametype in the title" rule: titles never embed `"connections"` / `"codenamesduet"` etc. because `CurrentGameCard` already shows the gametype's **logo** beside the title. A title that named the game would just echo the logo.

### last_active_at

`common.games.last_active_at` (timestamptz, defaults to `now()` at insert) dates a game by **when it was last touched**. The club games list orders by it (the `club_handle, last_active_at desc` index) and `CurrentGameCard` renders it — so a long-suspended game sorts and reads by its last activity, not its `started_at` (which, for a game shelved weeks ago, is unhelpful).
...
5. `perform common.end_game(target_game, 'ended', jsonb_build_object('reason', 'manual', …), player_results);` — the **uniform terminal `play_state` is `'ended'`** and the **status carries `outcome: 'manual'`**, across all gametypes.
6. **Realtime touch.** `common.end_game` writes only `common.games`. A game whose FE `useGame` subscribes to `<schema>.*` tables (most of them) won't see that write, so `end_game` must also do a no-op self-write on a subscribed `<schema>` row (e.g. `update <schema>.games set club_handle = club_handle where id = target_game;`) to produce a WAL entry the subscription wakes on. The terminal flip itself (the verdict pill / action-row swap, off `isTerminal`) rides `useCommonGame`'s `common.games` subscription regardless, but board/progress refetches (and post-terminal reveals like Waffle's solution) need the touch.
7. `revoke execute … from public; grant execute … to authenticated;`

**FE side.** End game is the `act-end-game` action, bound once per game by [`useStandardGameActions`](../src/common/game-page/useStandardGameActions.ts): the registry carries its words, its glyph, its ⌥⌫ and its question (`END_GAME_CONFIRM` — never `window.confirm`; see [ui.md → Confirm modals](ui.md#confirm-modals--never-windowconfirm)), the shared run asks it mid-game, and the body is `db.rpc('end_game', { target_game: gameId })`. It's **placed twice** — an info-column action-row `<ActionButton>` and a row in the game's menu via [`buildGameMenu`](../src/common/menu/gameMenu.ts) — but there is one binding, so the two cannot disagree about whether it applies. A race places `act-concede` instead — and where the race can ALSO stop the whole table, that one action carries both endings: its question offers "Concede" and "End for everyone" and says what each does, rather than the board drawing two red buttons that can only name them. The **pause overlay** carries a third placement, bound separately by `GamePage` (the play area is unmounted while paused, so its binding is gone) — the escape from a wedged presence-pause. See [ui.md → GamePage menu](ui.md#gamepage-menu). Either way the terminal rendering (`buildOver` + `manifest.labelFor`) must treat `play_state === 'ended'` / `status.reason === 'manual'` as a **neutral** result — never the red "you lost" branch. The message is shared outright: [`gameEndedTerminalMessage(mode)`](../src/common/terminal/terminalMessage.ts) returns the "Game ended" / "Game over" pair with `outcome: 'neutral'`, which colors both the below-board verdict and the info-column outcome line ([ui.md → Terminal results](ui.md#terminal-results--the-moment-vs-the-record)).

**The answers.** `end_game` and `submit_timeout` share a shape, because they ask the same two questions: one `ok` — `{ result: 'ended' }` — and three refusals, none of them raised by a game's own file. **PN486 "Game over"** (`race`, `noted`) when `play_state` is already terminal, from `common._raise_game_over()`, shared by every site across the two RPCs; **PN485 "That game was already deleted"** when the game's own row is gone, from the helper `replay_board` introduced; and `require_game_player`'s PN252 / PN253. Both RPCs check the row BEFORE the membership gate, for the reason [ui.md → Terminal results](ui.md#terminal-results--the-moment-vs-the-record) gives under Restart.

**The two RPCs differ only in who is looking.** `end_game` is a person clicking, so a lost race gets a pill. `submit_timeout` fires from every connected client on the same countdown edge, so in a four-player game three arrive to find the work done — `GamePage` swallows PN486 into an info log and surfaces everything else. That is a call-site decision about an audience, not a difference in the answer.
...
**The answer is an envelope** ([envelopes.md](envelopes.md)) with one `ok` — `{ result: 'conceded' }` — and four refusals, all four raised in `common`, since the wrappers add no rejection of their own: **PN481** a game that does not exist and **PN484** a concede in coop are faults; **PN482** "Game over" and **PN483** "Already conceded" are races, both of them the frontend's own gates (`isTerminal`, `myConceded`) losing to the subscription that feeds them. Both races carry `constraint = 'noted'`, because they are news rather than a setback. Whether the drop-out also ENDED the game is deliberately not in the answer: that reaches every client, the conceder's included, by subscription.

**FE side** mirrors `end_game`: the registry's question, then `runRpc(db.rpc('concede', { target_game }))` (every game has an own-schema `concede`, so the call is uniform). Every game reaches it through [`useStandardGameActions`](../src/common/game-page/useStandardGameActions.ts), which binds `act-concede` once and asks the question in the shared run — so no game can forget to ask, and none writes the three-branch chain itself. The info-column action row places both `act-concede` and `act-end-game` and lets each hide itself in the mode that isn't its own, so **Concede** shows in compete and **End** in coop; the OpponentStrip marks a conceder "out"; and the conceder gets a "You conceded" locally-terminal look (the same "I'm done, others race" branch elimination games already had) with their input disabled. bananagrams is the origin of the pattern; its per-player `concede` was promoted here in the 2026-07 sweep.

### Done, but not out — `locally_terminal`

**A racer can stop racing without conceding, and the presence-pause has to know.** The roster the pause watches is `not conceded and not locally_terminal`: a player nobody is waiting for must not stop the game for the people still playing by closing their tab. `common.game_players.locally_terminal` is that flag, written by `common._set_locally_terminal` from inside the gametype RPC that detects the local terminal, and cleared by `common.reset_game` with `conceded`.

**It is not a second spelling of `conceded`.** A conceder walked away and forfeits any win; a locally terminal player may be the WINNER — in the best-style races (wordle, waffle, strands — [win-lose.md](win-lose.md)) the first player it fires for is the one who solved, and the table stays open so a ranking can decide. Marking them "conceded" instead would forfeit the win they just earned, which is why this is its own column rather than an overload.

**Who sets it, and when:** connections at the fourth mistake · psychicnum when the budget is spent · wordle on a solve or the last guess · waffle on a solve or the last swap · strands on a solve · wordiply on the fifth guess. Compete only — coop has no local terminal, since the game ends for everyone at once. The games on `common.concede` need none of it: there the only way to stop racing is to win, which already ends the game.

### Turn-order — opt-in turn-by-turn for coop games

Coop games are **free-for-all** by default: every player may act at any time, and for find-many-words games (spellingbee, boggle) that parallelism *is* the game. But discrete-move coop games (one call = one turn-worthy action) can opt into **turn-by-turn** play, where moves rotate through the players in order instead of firing whenever — same rules, same shared board/budget, only *who may act right now* changes. Eight games carry it today (psychicnum, wordle, connections, waffle, wordiply, scrabble-coop, strands, letterboxed — the strongest fit on the roster: the chain hands off natively, and there an undo costs the undoer's turn); the whole mechanism is **common**, so more games can adopt it with a two-line wiring each.

**Data model (both nullable; null = free-for-all, the default, so nothing changes for games that don't opt in):**

- `common.games.current_turn_user_id uuid` (`references common.profiles(user_id) on delete set null`) — whose turn it is right now. Null ⇒ not a turn game. Directly comparable to `auth.uid()` server-side and `session.user.id` client-side.
- `common.game_players.turn_seat int` — the player's 0-based position in the rotation. Assigned at create-time when turn-order is on; null otherwise. (`joined_at` can't serve as the order — every row shares the create transaction's `now()`.)

**Three common `_`-prefixed helpers** (security-definer, revoked from public — called only from within game RPCs, like the concede helpers; all inert when the pointer is null):

| helper | when it's called | what it does |
|---|---|---|
| `common._assign_turn_order(target_game, first_user_id)` | once, at **create-time**, by a coop `create_game` when `setup->>'coop_style' = 'turns'` | seats the rotation (seat 0 = the chosen first player, the rest shuffled via `order by random()`) and sets `current_turn_user_id` to that first player |
...
(psychicnum, connections, waffle, wordle, scrabble, wordiply, strands, letterboxed) resets
...
- `setup.first_turn_user_id: string (uuid)` — who goes first (only meaningful for `coop_style = 'turns'`). A member id, so like codenamesduet's `first_clue_giver_user_id` it's **stripped from `default_setup`** server-side — a specific person isn't a reusable club preference.

**Realtime is free.** `_assign_turn_order` / `_advance_turn` UPDATE `common.games`, which every peer's `useCommonGame` already subscribes to → `load()` refetches → the turn flips for everyone. `common.games` is already in `supabase_realtime`, so the [publication invariant](supabase.md) is untouched (no new channel, no new table).

**The FE seam.** `useCommonGame` selects `current_turn_user_id` (on the exported `CommonGame` type) and derives `isMyTurn = current_turn_user_id == null || current_turn_user_id === session.user.id` in its return shape; both reach every game via `GamePageCtx` (`isMyTurn` / `currentTurnUserId`). Two shared components serve the turn games: **`SetupCoopStyleSection`** (`common/setup-form/` — the coop-only "Co-op" setup disclosure that writes the two keys and picks the first player from the *selected* roster; it re-seeds on deselect and hides for compete/solo) and **`TurnStatusLine`** (`common/info-sheet/` — the "Your turn" / "Waiting for ● Name…" info-column line, rendered only in turn games and inert-but-height-holding at terminal). The wording itself lives once in **`common/info-sheet/turnText.tsx`**: `waitingForText()` is the "Waiting for ● Name…" node, rendered by `TurnStatusLine` (desktop) and by **`FeedbackMessage.waiting(member)`** — a standing note each game's whose-turn effect shows into the below-board feedback slot while it isn't your turn, the answer to "whose turn?" on a phone, where the info column is off-canvas ([ui.md → Feedback pill](ui.md#feedback-pill)). It shows only when it ISN'T your turn (a permanent "Your turn" would evict the own-move results you need exactly then — the signal is the note leaving), and its rank puts it under the verdict, out-of-race and a result. Each game's form filters `SetupBodyProps.members` by the form's `player_user_ids` before handing them to `SetupCoopStyleSection`, so its first-player picker only offers people who'll actually play.

**scrabble is the reconciliation case.** scrabble *compete* keeps its **own** seat system (`scrabble.games.current_seat` + `scrabble._advance_seat`); scrabble-*coop* turn-order uses the common pointer. Two turn mechanisms in one game is a deliberate, accepted coexistence — unifying compete onto the common primitive is out of scope.

**A gametype may seat a bot only where something pokes it to move.** A bot is an ordinary account (`profiles.ai_member`) with a `game_players` row, so this rotation will hand it the turn like anyone — and then nothing happens, because a bot has no client. Something has to notice and call the move RPC: scrabble's `PlayArea` pokes an edge function whenever the current seat is an AI one. So a gametype wanting bots needs that poke before it seats one, or the table stalls on the bot's turn with no error to explain it. It cannot happen today: scrabble's bots are compete-only, and scrabble compete rotates by its own seat pointer rather than this one.
...
**`common.clubs.is_solo` is that prefix, said once.** A stored generated column — `handle like '=%'` — so the convention is the database's to know and callers ask for the answer instead of re-deriving it. It is also what lets PostgREST sort by it: the home list orders `is_solo.desc, created_at.desc` (Postgres sorts `false` before `true`, so descending puts solo on top) and the FE renders the rows as handed. Not writable — an INSERT naming it raises `428C9`.

Solo clubs are the anchor for solo-game-mode play (boggle, crosswords) and per-user stats. The HomePage lists each user's solo club alongside their regular clubs, marked by a "Solo" badge on the row and always sorted to the top — once the user knows where their solo space is, "Start connections alone" is a normal flow inside that club rather than a separate UI shape. Most game logic doesn't distinguish solo clubs from regular ones; the `club_handle` is just non-null in both cases.

Coop gametype variants (those declaring `mode: 'coop'`) with `min === 1` list normally in a solo club's start list. Compete variants declare `min === 2`, so a solo club is not enrolled in them at creation, and a row enrolled later is dimmed — a 1-player race against no opponent is degenerate. Coop with a countdown timer is the right "race against a clock" UX for a solo player. Fixed-seat gametypes like codenamesduet's `[2, 2]` stay multi-member-only the same way.
...
| `common.require_valid_timer(timer jsonb) → void` | Canonical timer-shape validation. Argument is the timer *subobject* (typically `setup->'timer'`), not the full setup blob, so the helper doesn't assume a specific nesting. Its five raises (PN035–PN039) are all **faults**, naming the column `timer`: absent timer, absent kind, a kind that isn't none/countup/countdown, a countdown with no seconds, and seconds outside 1..3600. They're faults because the timer control cannot produce any of them — the kind comes from three radios and the seconds box keeps the last valid value — so arriving here means a bug, a hand-built request or a corrupt saved default. Use in every gametype's `create_game` that exposes a timer setup option. |
...
| `common.update_state(target_game uuid, play_state text, status jsonb) → void` | Mid-game state-write helper for the duplicate-write discipline. Each gametype's submit_* RPC calls this on every state-affecting move (after writing its own per-gametype counters) so `common.games.play_state` + `status` stay current for the club-page listing label. (`last_active_at` is stamped by the games trigger, not here.) `is_terminal` is forced to false; use `common.end_game` for terminal transitions. **`status` MERGES (`\|\|`), it does not replace** — a caller passes only the keys it changes. The replace-everything version silently dropped any key a later write forgot to repeat, and did: codenamesduet seeded `greens_found` at create and the first `_end_turn` erased it. |
| `common.end_game(target_game uuid, play_state text, status jsonb, player_results jsonb) → void` | The terminal-transition counterpart. Sets `ended_at = coalesce(ended_at, now())`, writes the terminal `play_state` + `is_terminal = true` + `status` jsonb on `common.games` (`last_active_at` follows via the games trigger), and writes each player's `result` jsonb from `player_results` (keyed by user_id). Use at the moment a gametype's RPC decides the game is over (4 mistakes in connections, assassin in codenamesduet, etc.). Does NOT clear `is_current_view` — see the view-state section above. **`status` MERGES**, like `update_state`: a terminal write states what the ENDING adds and the mid-game readouts survive under it. Every terminal write should still state its own `reason` explicitly — under a merge, an omitted key inherits, so "no reason" can't mean anything. Says nothing about the solution: whether a player is looking at it is a local display choice in the FE (see [Revealing the solution](#revealing-the-solution)). |
...
| `common.concede(target_game uuid) → jsonb` | **Player-callable (granted to authenticated).** The generic concede for NON-elimination compete games: calls `_set_conceded`, then ends the game as a collective loss (`play_state 'lost_compete'` — or plain `'lost'` for a single-mode gametype; `status.reason 'conceded'`, every `{"won": false}`) iff no non-conceded player remains — otherwise the game continues for those still racing. Elimination / turn-based games use `_set_conceded` + their own `_maybe_finish_compete` instead. See [Concede](#concede--per-player-drop-out). |
| `common._assign_turn_order(target_game uuid, first_user_id uuid) → void` | Internal (revoked). Seats a turn-order coop game at create-time — seat 0 = the chosen first player, the rest shuffled — and points `current_turn_user_id` at them. Called by a coop `create_game` when `setup.coop_style = 'turns'`. See [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games). |
| `common._advance_turn(target_game uuid) → void` | Internal (revoked). Advances the turn pointer to the next player by `turn_seat` (wraps; skips conceded). No-op when the pointer is null. Call it only after an ACCEPTED, non-terminal move. See [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games). |
| `common._require_turn(target_game uuid, caller uuid) → void` | Internal (revoked). Raises `P0001 'not your turn'` when the game is a turn game and `caller` isn't the current player; no-op for free-for-all / compete / solo. Call it in the move RPC right after the row lock + caller resolution. See [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games). |
...
| `common.unset_current_view(target_game uuid) → jsonb` | Last-viewer-leaves view-state write fired by `useCommonGame`'s cleanup-on-unmount when the local presence snapshot was just-me (or empty). Clears `is_current_view` and answers `ok` with `{"result": "cleared"}`. Idempotent on the `where is_current_view = true` guard, and idempotent one level up too: a game that has been deleted comes back `ok` / `noted` (PA001), since it has no pointer to leave behind — the one `ok` a caller tells apart by `dbcode`, since it arrives through a raise and so carries no `data`. (An abandoned pointer the leave-race misses is healed from the club page — see `useClubPresence`.) |
| `common.tick_timer(target_game uuid) → int` | The game clock's one writer. Every actively-playing client calls it once a second; advances `common.timers.ticks` by at most 1 per real second (the `now() - last_tick >= 1s` conditional dedupes across players and makes pause/idle gaps cost +1, not the gap) and returns the current count. See [The game clock](#the-game-clock) above. |
...
begin
  ...
  perform common.require_valid_timer(setup->'timer');  -- when this gametype has a timer
  ...
  -- the per-gametype files in supabase/sql/ for examples.
...
**There is no server-side answer to "may they see it?" any more, and that's the design.** Seeing the solution is a **local, per-player display choice**, made in the FE with `useSolutionReveal` (`common/reveal/useSolutionReveal.ts`) and worked through `act-reveal`'s two faces — boxed-eye *Reveal*, `EyeOff` *Hide*. The UX rationale is [`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record); in short:

- **Personal.** One player looking doesn't open the answer on a partner who is still turning the loss over. That is the whole reason it changed.
- **Temporary.** Hide brings back the board as the game actually ended — which matters most for the games whose reveal *rewrites* the board (crosswords, strands, waffle, connections), where the old permanent reveal destroyed the only record of where the players got to.
- **Unpersisted.** Nothing is written, so nothing rides realtime, nothing has to be un-written on a replay, and a reload lands back on the board as it ended.

**Nothing autoreveals on a loss, or on a game the friends agreed to stop.** The one thing that opens an answer by itself is having produced it: the six games you can only finish by solving start their solver — and only their solver — with it shown ([`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record); the predicate is `solvedByMe`). A game whose readout is the word list is the other end of the range: its missed words arrive in the list at game over, so there is no reveal control to gate.

**What the server still owes is the SHIELD**, and it's a different question: each gametype's column-grant + `_x_for()` helper hands the solution over at **`is_terminal`** — over for *everyone*. That's the load-bearing part, and compete is where it bites: a player who conceded, was eliminated, or finished early sits there locally-done while the rest are still playing, and could read the answer out. So the gate can't key on any per-player doneness (`supabase/tests/strands/compete_test.sql` pins both halves on the sharpest case, a race that deliberately doesn't end on first solve). Which player is *looking* never needed protecting.
...
See the comment block above the policy in [`supabase/sql/common.sql`](../supabase/sql/common.sql) for the longer reasoning.
...
A game's players are a **subset** of the club, picked at create time (the `SetupGameModal` player checklist, defaulting to all members; the creator's own checkbox is locked on — you can't start a game you're not in, a rule the dialog holds up and `create_game` does not check — nor does it check that the club is enrolled in the gametype; the start list offers only enrolled ones and `ClubPageLoader` refuses a `?new=` for any other, and a hand-built request would be seated) and frozen into `common.game_players`. The authz model splits cleanly along that line:
...
- `games` — new games + `is_current_view` flips (set/unset_current_view, the suspend-broadcast cascade) keep the ClubPage games list fresh; `status` jsonb writes from each gametype's `common.update_state` refresh the list labels. (It does not navigate anyone: joining is by the invitation toast, and `game_players` INSERTs are what the global `useGameInvitations` watches.)
- `messages` — chat
...
src/
  ...
  gametypes.ts         THE registry — the only file allowed to import every game's manifest.
  ...
  common/              The shared shell every game builds on — one folder per feature.
```

The `common/` internals — the feature-folder taxonomy + the PURPOSE of
...
| `/c/<handle>` | `ClubPage` |
| `/g/<gametype>/<gameId>` | `<GamePage>`, which mounts the manifest's `PlayArea` (lazy-loaded chunk) as its play surface |
| anything else | `HomePage` (forgiving fallback, not a 404) |

The `/g/<gametype>/<gameId>` shape is what makes multi-game routing work: App.tsx resolves the manifest and hands it to the game route, keyed by `gameId` so navigation between games remounts cleanly (fresh state, no leaked subscriptions). The shell mounts the manifest's lazy `PlayArea` itself, inside the mount log, the error boundary and the Suspense — wrappers that are identical for every game and need nothing the route knows.

`<GamePage>` is the route-level shell — it owns the cross-cutting chrome (header / timer / Pause / Back-to-club, `<PauseBoundary>`, `<Chat>`, the suspend question) and calls `useCommonGame` for the cross-cutting state. The per-game `PlayArea` receives a `GamePageCtx` (the type exported from `src/common/game-page/gamePageCtx.ts` — read it there for the authoritative, doc-commented shape) as its props. Today it carries `session`, `gameId`, `brand`, `title`, `players`, `playState`, `isTerminal`, `timer`, `setup`, `status`, `clubHandle`, and the imperative handles `globalFeedbackSlot`, `menu`, `goToGame` (navigate to another game's page — for a PlayArea that starts a follow-up game, e.g. waffle's "New game" menu item; going back to the CLUB is `menu.actBackToClub`, which knows when to ask first). `playState` mirrors `common.games.play_state` (gametype-specific string); `isTerminal` mirrors `common.games.is_terminal`. The per-game `useGame` is just the postgres-changes subscription for that gametype's own tables — `play_state` lives on `common.games` and arrives via ctx, not on the per-gametype row.

**"Should this survive a pause?" is the rule that decides where state lives.** Because `PauseBoundary` unmounts its children on pause, anything inside the per-game `PlayArea` (component state, `useGame`-local state, form input) resets every time the game pauses. That's deliberate UX — clean slate on resume. State that *must* survive a pause goes either in the DB or in `useCommonGame` above the boundary (members, presence, the clock). State that's specifically transient (connections' shared-tile selections, an in-flight submit form) lives in PlayArea and clears naturally on unmount.
...
ESLint enforces the import-direction rules; see [`eslint.config.js`](../eslint.config.js) for the `no-restricted-imports` configuration. Which folders count as games is **derived** there from `src/gametypes.ts` (and cross-checked against the folders holding a `manifest.ts`), so adding a game stays one line in the registry — the lint list follows, and a mismatch fails `npm run lint` rather than silently unguarding a game.

### Joining a game — the invitation toast

Games seat every player at creation (a `common.game_players` row each), but nobody is dragged into the game. Being added to a game instead pops a **join invitation** — "*Moth* added you to a new *spellingbee* game" + a Join button — wherever the player is in the app. It arrives as a toast, in the same bottom-right stack as every other announcement ([ui.md → Toasts](ui.md)), which is why an open chat panel never hides one. The logic is one global hook, [`useGameInvitations`](../src/common/invitations/useGameInvitations.ts), mounted once in `App.tsx` *after* the claim-handle gate (so it's on every real page, never the login/claim screens); [`<GameInvitations>`](../src/common/invitations/GameInvitations.tsx) renders nothing itself — it mirrors the hook's live invite list into the toast store.

It watches `common.game_players` for INSERTs of the caller's own rows (an invite appears instantly while online) and **re-scans on (re)connect** for non-terminal games the caller is a player in — recovering invites sent while they were offline (rare, but the realtime INSERT alone would miss them). A localStorage **"seen" set** keeps one invite from re-popping; a dismissed invite is recovered via the club page, which still lists the game as the current game. The game the player is currently viewing is filtered out (you're never invited to the game you're in).

**The game waits for invitees.** Presence-pause counts every `game_players` row as an expected player (`computePause`), so a freshly-created game is *paused* — "Waiting for everyone to connect…", over a roster where the invitee who has not arrived is a hollow ring — until each invited player joins. That's the point: a game must not "start" for someone who isn't at their computer, so it genuinely waits for everyone to be present. Friends always join eventually, so there's no decline path. Attribution comes from `common.games.created_by` (the player who clicked Start), recorded by `common.create_game`.

**Nothing else pulls a player in.** ClubPage's subscription only refreshes its games list, and `is_current_view` is the club's durable "current game" pointer (the card + the abandoned-pointer heal) and nothing more — entering a game is always the player's own click. Joining one while mid-play in another simply leaves the first (which pauses for the others).
...
- **Design tokens at `:root`**, split across [`src/common/themes/daylight.css`](../src/common/themes/daylight.css) (colors), [`src/common/core-css/fixed.css`](../src/common/core-css/fixed.css) (colors no theme touches) and [`src/common/core-css/base.css`](../src/common/core-css/base.css) (everything that isn't a color). All other CSS references these via `var(--token-name)`.
...
- `american` / `british` / `canadian` / `australian` — dialect validity. Mostly a *spelling* filter (`color`/`color`); a word like `lorry` is `american=true` too. Default play is `american OR british`.
...
**How a game uses it.** spellingbee defines its slice in `spellingbee.candidate_words`, which takes the game's two bands from its setup (3 and 5 by default): legal = `difficulty ≤ legal band`, required = `difficulty ≤ required band AND american AND NOT slang AND slur = 0 AND crude = 0`, `len ≥ 4`. waffle picks a pre-generated puzzle whose hardest word is exactly the chosen band. Bands a game uses (or offers) are a per-game choice; the list itself holds every band.

**Seed + import.** Public reference data — `grant select` to `authenticated`, with RLS on and a permissive select policy so the table can't fail open. Seeded from the word-list project's working copy `~/src/gamelist/words.tsv` (~283k rows) — read **live** by `gmake all-words`, never vendored into this repo, so it can't silently drift from the source the two projects are developed against in tandem (override the path with the `WORDS_TSV` env var). Loads via psql `COPY` (TRUNCATE + insert; `letter_mask` fills itself as a generated column). Same direct-Postgres load the other reference tables use (fast + reliable for the hosted load; see [`lib/copyLoad.ts`](../supabase/scripts/lib/copyLoad.ts) for why `COPY` beats batched HTTP upserts).
...
- **`definition`** — the def text, or NULL if none yet. **`definition_source`** — one-char provenance: `s` (real gloss) / `e` (auto gloss, "plural of X") / `w` (live Wiktionary) / `m` (manual), or NULL = never looked up. The `s`/`e`/`m` glosses use the upstream word list's compact gloss format (`parseDefinition` acts only on its cross-references); `w` is plain prose. A `w` source with a NULL `definition` is the *negative-cache tombstone* — "looked up, Wiktionary had nothing" — so repeat lookups don't re-hit the API.
...
**Seed + growth.** The seeded glosses ship in the word list itself (`~/src/gamelist/words.tsv`, loaded by `gmake all-words` — see [The word list](#the-word-list-commonwords)). It then grows lazily: the **`common-define` Edge Function** is a read-through cache — reads `common.words` as the caller, and for an in-list word with no definition yet (source NULL) fetches **Wiktionary** (`freedictionaryapi.com`, CC BY-SA) and writes it back via `cache_definition`. Wiktionary won the bake-off over `api.dictionaryapi.dev` (~93% vs ~30% coverage on obscure bonus words, and no aggressive rate-limiting); a transient API failure surfaces an error *without* writing a tombstone, so only definitive empty answers are negatively cached.

**Frontend.** All in [`common/definitions/`](../src/common/definitions/doc.md), so a game wires it in a few lines:
- `useDefinition(word)` — declarative lookup over the `common-define` edge function (through `runEdgeFn`); cancels in-flight results so fast cross-ref chasing never flashes stale text.
- `parseDefinition` — turns a raw def into renderable parts. The stored def text is authoritative and shown **in full**; the parser only *adds* markup — the gloss format's `<word=pos>` / `{word=pos}` cross-refs become clickable `ref` parts (for seeded `s`/`e`/`m` defs). Live Wiktionary text (`source === 'w'`) is plain prose, returned verbatim. Everything else (`[…]` inflection tags like `[n SUPPRESSIONS]`, `/` sense separators, `(YEAR)`) passes through verbatim, so an inflection-only stub still displays its text rather than rendering blank.
- `DefinitionView` — the shared body (heading + parsed def + clickable refs + CC BY-SA attribution when `source === 'w'`); shows "Unknown word." for a word not in the list, "No definition found." for an in-list word Wiktionary had nothing for. A ref click calls `onNavigate` to re-point the lookup in place.
- `DefinitionPopover` (anchored card, click-to-define) and `WordLookupDialog` (FloatingPanel + text box, the shortcut) both embed `DefinitionView` — they differ only in *how the first word is chosen*.

**Click-to-define (app-global).** A surface renders each clickable word as `<DefinableWord word={w}>`, and that is the whole wiring: the component carries the `.definable` class, the native "Click to define" title and the `data-word` e2e handle, and writes the clicked word into a one-slot store that the root's `<DefinitionHost>` renders the card from (the same shape as `ToastHost` / `TooltipHost`). The words are pointer-only, deliberately (`.definable` in `core-css/utilities.css` says why). What stays per-game is only *which* words are clickable — a rejected guess or a theme phrase isn't in the dictionary, so it isn't definable.
...
as `EditProfileModal`). The form edits definition, hint, band, crude, slur,
...
- **Per-club stats.** Solo clubs are the planned anchor for per-user stats. Schema not built; no UI surface yet. **Far future** (2026-08-02) — the shape (which stats, per-club vs global, live-aggregated vs written on `end_game`) stays TBD until there's a concrete want; see [`deferred.md → Far future`](deferred.md#far-future).
- ~~**Auto-propagating a new gametype to existing clubs.**~~ **Decided (2026-08-02): won't do.** Club enrollment in `common.clubs_gametypes` is seeded at creation (`common.default_gametypes_for_club`) and editable afterward via the "Edit club" dialog (`common.set_club_gametypes`). A gametype registered *after* a club exists doesn't auto-add — and won't: the roster is complete, so this is the rare case, and the two answers it already has (the editor, or a per-game backfill in that game's migration, as bananagrams does) are enough.
