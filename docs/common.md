...
**There is no server-side answer to "may they see it?" any more, and that's the design.** Seeing the solution is a **local, per-player display choice**, made in the FE with `useSolutionReveal` (`common/hooks/game/useSolutionReveal.ts`) and worked through the `RevealButton`'s two faces — boxed-eye *Reveal*, `EyeOff` *Hide*. The UX rationale is [`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record); in short:
