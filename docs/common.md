...
`common.games.title` is `not null` and `length(trim(title)) > 0`-checked. Each gametype's `create_game` builds the value at insert time and passes it as the `title` argument to `common.create_game`. Choosing a formula is a per-gametype call — there's no universal-good answer ("just list the players" fails inside a single club where every game has the same 2-3 players). Each formula lives in that gametype's per-feature doc under `### Title formula`; all of them side by side (next to the status lines they're read with) are in [game-status-labels.md](game-status-labels.md).
...
The grammar every status line speaks lives in [`statusLabel.ts`](../src/common/manifest/statusLabel.ts)'s header — the leading words, the devices, and what a line may say — and the per-game strings are tabulated in [game-status-labels.md](game-status-labels.md). One of those rules is a constraint on what the SERVER writes, which is why it is named here too: a status line may only say what every player already sees, and `status` is club-wide readable, so a compete game's private per-player progress must not be written there in the first place — several compete labels are a bare `Playing` for exactly that reason.
...
| `common.end_game(target_game uuid, play_state text, status jsonb, player_results jsonb) → void` | The terminal-transition counterpart. Sets `ended_at = coalesce(ended_at, now())`, writes the terminal `play_state` + `is_terminal = true` + `status` jsonb on `common.games` (`last_active_at` follows via the games trigger), and writes each player's `result` jsonb from `player_results` (keyed by user_id). Use at the moment a gametype's RPC decides the game is over (4 mistakes in connections, assassin in codenamesduet, etc.). Does NOT clear `is_current_view` — see the view-state section above. **`status` MERGES**, like `update_state`: a terminal write states what the ENDING adds and the mid-game readouts survive under it. Every terminal write should still state its own `outcome` explicitly — under a merge, an omitted key inherits, so "no outcome" can't mean anything. Says nothing about the solution: whether a player is looking at it is a local display choice in the FE (see [Revealing the solution](#revealing-the-solution)). |
