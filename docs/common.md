...
**FE side.** End game is a player action the PlayArea wires up: `window.confirm` then `db.rpc('end_game', { target_game: gameId })`. All ten games (now uniformly v3) render the affordance the same way — an **info-column action-row button**, the shared [`<EndGameButton>`](../src/common/components/buttons/EndGameButton.tsx) (disabled at terminal; see [ui.md → Info-column readouts](../ui.md#info-column-readouts)). *Where* the affordance lives is a UI-convergence detail, not a contract: a header-menu variant (`{ id: 'end-game', … }` via `menu.setGameItems`) is still supported by the [`useEndGameMenu`](../src/common/hooks/game/useEndGameMenu.ts) hook, but that hook is **currently unused** — kept as reference should a future game want End-game in the header menu instead of the info column. Either way the terminal rendering (`buildOver` + `manifest.labelFor`) must treat `play_state === 'ended'` / `status.outcome === 'manual'` as a **neutral** result — green "Game ended" copy via `GameOverModal`'s `outcome: 'won'` styling, never the red "you lost" branch. (GameOverModal only has won/lost coloring today; manual end reuses the green one with neutral copy.)
