...
Eight game families hide their answer from a *lost* game so a replay of the same board is a genuine second try (waffle, wordle, stackdown, psychicnum, crosswords, codenamesduet, strands, letterboxed — the UX rationale is [`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record)). Hiding it is a **display** decision, not a shield: each gametype's own column-grant + `_x_for()` helper already unshields the solution the moment the game is terminal, so the client holds it either way.
