...
- A gametype's supported player-count range is declared on its manifest (`numberOfPlayers: [min, max]`). The shell decides whether to surface "Start X" in a given club by checking the range against the club's member count. A fixed-seat range like codenamesduet's `[2, 2]` never appears in a solo club; open-N ranges like `[1, 6]` appear in solo clubs and in clubs up to the cap; a future game with `[3, 5]` would only appear in clubs sized 3-5. ClubPage hides the button entirely when there's no `clubs_gametypes` row, and disables it with a tooltip when the row exists but the count is out of range — see `playerCountFits` / `playerCountLabel` in `src/common/lib/gameManifest.ts`.
