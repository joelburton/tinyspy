...
| `common.unset_current_view(target_game uuid) → jsonb` | Last-viewer-leaves view-state write fired by `useCommonGame`'s cleanup-on-unmount when the local presence snapshot was just-me (or empty). Clears `is_current_view`. Idempotent on the `where is_current_view = true` guard, and idempotent one level up too: a game that has been deleted comes back `ok` / `noted`, since it has no pointer to leave behind. (An abandoned pointer the leave-race misses is healed from the club page — see `useClubPresence`.) |
