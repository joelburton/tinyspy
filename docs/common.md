...
| `gametypes` | The registered-gametype list (`gametype text PK`, `min_players smallint`, `hides_solution boolean`, `default_enroll boolean`). Authoritative SQL-side mirror of `src/games.ts`. Each gametype's schema migration registers itself with an `INSERT ... ON CONFLICT DO NOTHING` (a seed row, so it stays on the migration side), declaring its `min_players` (the lower bound of the manifest's `numberOfPlayers` — coop/solo games register 1, two-player games 2). Sibling families register one row per variant — psychicnum's migration inserts both `psychicnum_coop` and `psychicnum_compete`. `min_players` is the one fact the server needs from the player-count range: `common.default_gametypes_for_club` uses it to keep solo clubs out of two-player games. `hides_solution` says whether this gametype withholds its answer from a game that ended without a win — read by `common.end_game`; see [Revealing the solution](#revealing-the-solution). `default_enroll` (default true) says whether a NEW club is enrolled automatically — psychicnum's pair registers **false** (it's the architecture-exercise toy, not a game a fresh club should find on its Start list); off-by-default, not banned, since `set_club_gametypes` can opt a club in. Permissive SELECT — gametype identifiers aren't sensitive. |
| `games` | The universal game-record header. One row per game-playing across all gametypes. Holds `club_handle`, `gametype`, `title`, the **view-state pair** (`is_current_view`, `paused`), the **play-state pair** (`play_state` text + `is_terminal` boolean), `status` jsonb (the gametype-specific listing-label payload), `started_at`, `ended_at`, and `last_active_at` (stamped by a BEFORE UPDATE trigger on **every** write to the row — moves, the `is_current_view` shelve/resume flip, pauses, the terminal write — so the club games list orders + dates by when a game was last *touched*, not when it started; see [last_active_at](#last_active_at)). (The game clock lives in its own table, `common.timers` — see below.) `current_turn_user_id` (nullable) is the common turn-order pointer for opt-in turn-by-turn coop games — null for free-for-all (the default); see [Turn-order](#turn-order--opt-in-turn-by-turn-for-coop-games). `solution_revealed` is the one common answer to "may the players see the solution?" — see [Revealing the solution](#revealing-the-solution). Per-gametype detail (board, secret, current turn) lives on `<gametype>.games`, which shares an id with this row via FK. The `title` is a short human-readable label built by each gametype's `create_game` at insert time (see [Title formulas](#title-formulas)) — the FE renders it beside the gametype's logo in club game lists (`ClubGameCard`). The two state pairs and their orthogonality are written up in [`states.md`](states.md). |
...
Coop games are **free-for-all** by default: every player may act at any time, and for find-many-words games (spellingbee, boggle) that parallelism *is* the game. But discrete-move coop games (one call = one turn-worthy action) can opt into **turn-by-turn** play, where moves rotate through the players in order instead of firing whenever — same rules, same shared board/budget, only *who may act right now* changes. Seven games carry it today (psychicnum, wordle, connections, waffle, wordiply, scrabble-coop, strands); the whole mechanism is **common**, so more games can adopt it with a two-line wiring each.
...
(psychicnum, connections, waffle, wordle, scrabble, wordiply, strands) resets
...
| `common.end_game(target_game uuid, play_state text, status jsonb, player_results jsonb) → void` | The terminal-transition counterpart. Sets `ended_at = coalesce(ended_at, now())`, writes the terminal `play_state` + `is_terminal = true` + `status` jsonb on `common.games` (`last_active_at` follows via the games trigger), and writes each player's `result` jsonb from `player_results` (keyed by user_id). Use at the moment a gametype's RPC decides the game is over (4 mistakes in connections, assassin in codenamesduet, etc.). Does NOT clear `is_current_view` — see the view-state section above. **`status` MERGES**, like `update_state`: a terminal write states what the ENDING adds and the mid-game readouts survive under it. Every terminal write should still state its own `outcome` explicitly — under a merge, an omitted key inherits, so "no outcome" can't mean anything. Also sets `solution_revealed` when the ending puts the answer on screen: always on a win, and at any ending of a gametype whose `gametypes.hides_solution` is false. |

| `common.reveal_solution(target_game uuid) → void` | The explicit "show us the answer" control behind each game's terminal `RevealButton` + its menu twin. Flips `common.games.solution_revealed`; **terminal-only** (`is_terminal`, i.e. ended for *everyone* — a conceded compete player can't reveal while the others race) and player-only, idempotent. Writes no game state: each gametype's own shield already lifted at terminal, so this is the shared display answer. See [Revealing the solution](#revealing-the-solution). |
...
Seven gametypes hide their answer from a *lost* game so a replay of the same board is a genuine second try (waffle, wordle, stackdown, psychicnum, crosswords, codenamesduet, strands — the UX rationale is [`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record)). Hiding it is a **display** decision, not a shield: each gametype's own column-grant + `_x_for()` helper already unshields the solution the moment the game is terminal, so the client holds it either way.

That decision has exactly one home: **`common.games.solution_revealed`**.

| written by | when |
|---|---|
| `common.end_game` | true on any win (you produced the solution to get there), or at *any* ending of a gametype whose `common.gametypes.hides_solution` is false |
| `common.reveal_solution(target_game)` | the explicit control — terminal-only, player-only, idempotent |
| `common.reset_game` | back to false, so a replayed board starts blind |

**Whether a gametype hides its answer is a REGISTRY fact**, `common.gametypes.hides_solution` — true for the thirteen gametypes of those seven families, false for everything else. It's a property of the game, not a decision each ending re-makes, and that matters: `end_game` reads it once, so no ending path can forget to pass it — including `common.concede`'s all-conceded terminal, which no gametype calls directly and which an earlier per-call-site design silently got wrong. It defaults false ("reveal at any ending"), the safe direction for a forgotten value: a game that shows its answer when it shouldn't is a visible bug someone reports; one that hides an answer for no reason looks like the feature working.

Three things follow, and they're the reason it isn't per-game FE state:

- **The games that never hide their answer set it too.** boggle, spellingbee, wordwheel (their missed-word list *is* the post-game artifact), connections and wordiply (both hold the answer client-side by explicit design) end with the flag true, and their FEs read *it* rather than `isTerminal` — so "should they see the solution?" has one canonical answer for all fourteen games, and changing one of their minds is a registry edit rather than a hunt through components. Games with no solution at all (bananagrams, scrabble) never set it, the way `current_turn_user_id` stays null for the free-for-all games.
- **Reveal is terminal-only, in the RPC.** "Ended for everyone" is `is_terminal`, so a compete player who conceded or ran out of guesses can't open the answer while the others are still racing — and no game needs its own "am I locally done?" reasoning to enforce that.
- **The reveal is shared.** One player reveals and every client's board opens, carried by the same realtime refetch as the rest of the row. A post-mortem is something the friends do together on the call.

`status.outcome = 'revealed'` is a *different* thing and stays: it's the terminal **outcome label** for waffle's and wordle's mid-game give-up ("this game ended *because* someone asked"), sitting beside `'manual'` and `'ended'`. Those RPCs end the game, and `end_game` sets `solution_revealed` on the way through.

pgTAP: [`supabase/tests/common/reveal_solution_test.sql`](../supabase/tests/common/reveal_solution_test.sql).
