...
The four-word grammar every status line speaks (`Playing` / `Won` / `Lost` / `Ended`, parens for a reason, `·` between facts) lives in [`common/lib/game/statusLabel.ts`](../src/common/manifest/statusLabel.ts), and the per-game strings are tabulated in [game-status-labels.md](game-status-labels.md). The same "only what every player already sees" rule applies: `status` is club-wide readable, so a compete game's private per-player progress must not be written there — several compete labels are a bare `Playing` for exactly that reason.
...
**The answers.** `end_game` and `submit_timeout` share a shape, because they ask the same two questions: one `ok` — `{ result: 'ended' }` — and three refusals, none of them raised by a game's own file. **PN486 "Game over"** (`race`, `noted`) when `play_state` is already terminal, from `common._raise_game_over()`, shared by all twenty-nine sites across the two RPCs; **PN485 "That game was already deleted"** when the game's own row is gone, from the helper `replay_board` introduced; and `require_game_player`'s PN252 / PN253. crosswords adds one of its own — **PN487**, End attempted in compete, the mirror of `concede`'s PN484. Both RPCs check the row BEFORE the membership gate, for the reason [ui.md → Terminal results](ui.md#terminal-results--the-moment-vs-the-record) gives under Restart.
