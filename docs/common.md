...
- **`GameScratchpad` / `ScratchpadButton`** (`components/floating-panels/`) — the panel rides on `FloatingPanel` + `useDraggablePanel` (persisted per-game rect), and `scratchpadOpenStore` (`lib/scratchpad/`) mirrors `chatOpenStore`. `<GamePage>` gates the whole thing purely off the manifest `scratchpad` field (`{ enabled; perPlayerInCompete? }`).
