...
**Click-to-define wiring (per-game).** A surface spreads `useDefinePopover`'s `define` onto each word it shows; the words are pointer-only, deliberately (`.definable` in `core-css/utilities.css` says why). This stays per-game because *which* words are clickable is game-specific.
