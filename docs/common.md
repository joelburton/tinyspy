...
- `setup.first_turn_user_id: string (uuid)` — who goes first (only meaningful for `coop_style = 'turns'`). A member id, so like codenamesduet's `first_clue_giver_user_id` it's **stripped from `saved_default`** server-side — a specific person isn't a reusable club preference.
