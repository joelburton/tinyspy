...
**Nothing autoreveals on a loss, or on a game the friends agreed to stop.** The one thing that opens an answer by itself is having produced it: the six games you can only finish by solving start their solver — and only their solver — with it shown ([`ui.md → Terminal results`](ui.md#terminal-results--the-moment-vs-the-record); the predicate is `solvedByMe`). The three word-finding games are the other end of the range: boggle, spellingbee and wordwheel show their missed-word list at terminal and carry **no reveal control at all**, because the word list's KIND filter (*found* / *missed*) already is that control — a Reveal button would be a second, confusing way to switch between the same two lists. If we ever wanted them to withhold it, the change is that filter's **default**, not a new control.
