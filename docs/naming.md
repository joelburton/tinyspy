...
| `created_at` | The `timestamptz` column on every **game-row** table, and on every log table named `events` (every game with a log except codenamesduet: `psychicnum`, `wordle`, `connections`, `waffle`, `wordiply`, `stackdown`, `scrabble`, `strands`, `letterboxed`, `setgame`) — where a row can be a guess, a hint or a spoiler, no one verb names the event. The child tables that log ONE kind of thing name it instead: `guessed_at` (codenamesduet), `found_at`. The verb is more use than a generic timestamp there, and it reads correctly when the row is quoted out of context ("guessed_at 14:02"). |
| `club_handle` | The FK to `common.clubs(handle)` on every `<gametype>.games` table. |
| `target_game` | The conventional name for the game-UUID parameter on every gametype's mutating RPCs (`submit_guess(target_game uuid, …)`). `target_<noun>` is the broader pattern for RPC params pointing at row IDs. Deliberately split (settled in `161488f`): internal revoked-from-public shim helpers take `g_id` instead, so the param name itself signals which surface a function belongs to — `target_game` = client-facing RPC, `g_id` = private helper. |
...
| `cell` vs `tile` | A **cell** is a spot on the BOARD — useful mainly in the games whose boards have empty spots to fill, scrabble and bananagrams, where a tile is placed onto a cell. A **tile** is a letter in a grid. Most games have only tiles: we draw them in squares and color them by theme, and even where we do not (strands' discs), the model is still letters sitting on a board. So a game with no empty spots has no cells, and a prop that marks board pieces is `…Tiles` there. Ruled 2026-09-16, after the history-names sweep imposed one noun on every game and got it wrong twice in opposite directions. |
| `historyId` / `isViewingHistory` / `showHistory` / `exitHistory` / `historyLabel` | The turn-history viewer's vocabulary, identical in every game that has one — the open entry's id (the events row's own id, or codenamesduet's `turn_number`), the boolean, the two verbs, and the banner's text. Named for the FEATURE and not for the shape of the id, because a reader of a `PlayArea` should see "this is about history" without chasing an import; a game that needs a local distinction aliases it (scrabble's `historyTarget`, a union of a past turn and a `peerPreview`). Ruled 2026-09-16, replacing eight spellings (`viewingId`, `viewingIndex`, `viewingSeq`, `viewTarget`, `select`, `selectTurn`, `setViewingIndex`, `viewing`). The rule behind it: **nothing about history or peer-preview carries a name without `history` or `preview` in it** — props, state, CSS classes, tokens and the `act-exit-history` binding alike. |
| `HistorySnapshot` / `historySnapshot` | The turn-history replay shape (type `HistorySnapshot`) and its pure builder (`historySnapshot(rows, index)`) in each game's `lib/history.ts`, consumed by the shared `useHistoryViewer` to render a past turn on the board. Identical across the games with a board-replay viewer. (scrabble is the documented exception — its replay is `historyBoard` in `lib/play.ts`.) |
| `<table>_select` | The SELECT RLS policy naming pattern — `games_select`, `events_select`. Other policy directions follow the same pattern (`<table>_insert` etc.) if/when we ever add them. |
| `useGameTimer`, `PauseBoundary`, `PauseOverlay`, `computePause`, `Chat` | Common hooks / components / helpers. One name each, and no per-game variant: whoever consumes it — a game, or the shell that wraps every game — does so under this exact import. |
...
| `set` | the named collection (`tiles`, `guesses`, `categories`). The game whose whole subject is sets is codenamed `setgame` for this reason, and calls the found thing a **claim** |
