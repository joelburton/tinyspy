...
| `created_at` | The `timestamptz` column on every **game-row** table, and on every log table named `events` (`psychicnum`, `strands`, `letterboxed`, `setgame`) — where a row can be a guess, a hint or a spoiler, no one verb names the event. The child tables that log ONE kind of thing name it instead: `guessed_at` (codenamesduet / connections / wordle / wordiply), `found_at`, `played_at` (scrabble), `submitted_at` (stackdown), `swapped_at` (waffle). The verb is more use than a generic timestamp there, and it reads correctly when the row is quoted out of context ("guessed_at 14:02"). |
