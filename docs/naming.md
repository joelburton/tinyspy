...
| `created_at` | The `timestamptz` column on every **game-row** table. **Child tables do NOT use it** — they name the event: `guessed_at` (codenamesduet / psychicnum / connections / wordle / wordiply), `found_at`, `played_at` (scrabble), `submitted_at` (stackdown), `swapped_at` (waffle). The verb is more use than a generic timestamp, and it reads correctly when the row is quoted out of context ("guessed_at 14:02"). Converged 2026-08-01; this entry used to claim child tables shared `created_at`, which was never true of most of them. |
