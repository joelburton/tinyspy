...
| `created_at` | The `timestamptz` column on every **game-row** table, and on every log table named `events` (`psychicnum`, `wordle`, `connections`, `waffle`, `strands`, `letterboxed`, `setgame`) — where a row can be a guess, a hint or a spoiler, no one verb names the event. The child tables that log ONE kind of thing name it instead: `guessed_at` (codenamesduet / wordiply), `found_at`, `played_at` (scrabble), `submitted_at` (stackdown). The verb is more use than a generic timestamp there, and it reads correctly when the row is quoted out of context ("guessed_at 14:02"). |
